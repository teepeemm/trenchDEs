...
\[
\laplace(e^{at})=\frac{1}{s-a},\quad s>a, \text{\; or \; }
...
\[
\laplace(t^3e^{4t})=\frac{3!}{(s-4)^4}=\frac{6}{(s-4)^4}.\bbox
\]
...
\[
\laplace(\cosh bt)=\frac{s}{s^2-b^2},\quad s>|b|.
\]
...
shows that
\[
\laplace^{-1}\left(\frac{s}{s^2+9}\right)=\cos3t.\bbox
\]
\end{alistSolution}
...
Therefore
\[
\laplace^{-1}(F)=\frac{8}{3}\sin t+\cos t-\frac{4}{3}\sin 2t-\cos 2t.
\]
...
\[
\laplace(f)=\frac{2}{s^2}+\frac{1}{s} +e^{-2s}\left(\frac{1}{s^2}+\frac{1}{s}\right).\bbox
\]
...
\end{align*}
which is the result obtained in \zcref{example:8.4.1}.\bbox
...
-e^{-5s}\left(\frac{2}{s^2}+\frac{11}{s}\right).\bbox
\end{align*}
...
-e^{-\pi s}\left(\frac{3+2s}{s^2+1}\right).\bbox
\end{align*}
...
\[
\laplace^{-1}\left(\frac{e^{-2s}}{s^2}\right)=u(t-2)(t-2).\bbox
\]
...
&=\frac{1}{(s-a)(s-b)}\\
&=\laplace(e^{at})\laplace(e^{bt})=\laplace(f)\laplace(g).
\end{align*}
