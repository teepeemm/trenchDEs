...
$e^{\lambda t}\sin\omega t\leftrightarrow\dst{\frac{s-\lambda}{(s-\lambda)^2+\omega^2}},\,s>\lambda$\\[9pt]\hline
\end{tabular}
\end{center}
...
\item\label{exer:8.4.19} $\dst\frac{H(s)={e^{-2s}}{s-2}}$%&
\item\label{exer:8.4.20} $\dst\frac{H(s)={e^{-s}}{s(s+1)}}$
%\end{tabular}

\item\label{exer:8.4.21} \CGex $\dst\frac{H(s)={e^{-s}}{s^3}+
\frac{e^{-2s}}{s^2}}$
...
$\displaystyle{\frac{\Gamma (p+1)}{s^{(p+1)}}}$ & $(s>0)$ \\[15pt]
  ...
  $t^ne^{at}$  &  $\displaystyle{\frac{n! }{(s-a)^{n+1}}}$  & $(s > 0)$ \\[15pt]
  ($n= \mbox{ integer } > 0$)  & {} & {} \\
  $\cos \omega t$  &
