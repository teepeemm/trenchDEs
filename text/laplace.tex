...
integral}\index{integral!improper} of $g$ over $[a,\infty)$ is defined as
...
\index{Laplace transform!computation of simple}
...
Recall that a limit\index{limit}
\[
\lim_{t\to t_0} f(t)
\]
...
that $f$ has a \indexterm{jump discontinuity}\index{discontinuity!jump}\index{piecewise continuous function!Laplace transform}  at $t_0$,
 and
\[
f(t_0+)-f(t_0-)
\]
...
\begin{rmlist}
\item % (i)
A function $f$ is said to be \emph{\color{blue}piecewise continuous}\index{piecewise continuous function} on a finite
...
\index{inverse Laplace transform!linearity property}
\index{Laplace transform!inverse!linearity property}
\index{linearity!inverse Laplace transform}
...
\index{inverse Laplace transform!rational function}
\index{Laplace transform!inverse!rational function}
\index{rational function!inverse Laplace transform}
...
Some software packages that do symbolic algebra can find partial fraction
expansions\index{partial fraction expansion!finding with software packages}
very easily. We recommend that you use such a package if one is available to
you, but only after you've done enough partial fraction expansions on your own
...
\index{initial value problem!solved with Laplace transform}

\boxit{Laplace Transforms of Derivatives}
\index{Laplace transform!of derivative}
...
\index{Laplace transform!solution of initial value problems!derivative}
...
In \zcref{sec:linear_first_order} we showed that the solution of the
...
\index{Laplace transform!solving second order equation}
\index{Laplace transform!solving initial value problem!second order equation}
\index{second order equation!solving with Laplace transform}
\index{initial value problem!solved with Laplace transform!second order equation}
...
\index{Laplace transform!piecewise continuous function!unit step function}
\index{piecewise continuous function!Laplace transform!unit step function}
...
\index{Laplace transform!piecewise continuous functions}
\index{piecewise continuous functions!Laplace transform}
...
\index{forcing function!with piecewise continuous constant equation}
...
\index{convolution}
\index{Laplace transform!convolution}
...
Until now we haven't been interested in the factorization indicated
...
The \indexterm{convolution}\index{Laplace transform!convolution!defined}
...
\begin{theorem}[The Convolution Theorem]\label{thmtype:8.6.2}
\index{convolution!theorem}\index{Laplace transform!convolution!theorem}
...
\index{Laplace transform!solution of initial value problem!formula}
...
\index{initial value problem!solved with Laplace transform!formula}
...
\emph{\color{blue}convolution integral}\index{convolution!integral}\index{integral!convolution}\index{Laplace transform!convolution!integral}.
The convolution
...
\index{Laplace transform!convolution!transfer function}
...
is called the
\indexterm{transfer function}\index{convolution!transfer function}
of the device. Since
\[
H(s)=W(s)F(s),
\]
...
called the \emph{\color{blue}impulse response}\index{impulse!response}
of the device, for reasons
...
\index{Laplace transform!constant coefficient equation!impulse}
...
zero otherwise. We say that such forces are \emph{\color{blue}impulsive}\index{impulsive force}\index{force!impulsive}.
...
this case that $f$ is an
\emph{\color{blue}impulse function}\index{impulse!function}.
In particular, we
...
ay''+by'+cy=f(t),
\]
$w$ is called the \emph{\color{blue}impulse response}\index{impulse!response}
of the device. Note
that
