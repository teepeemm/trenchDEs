...
terms; that is, we must construct a \indexterm{mathematical
model} for the
...
are  \emph{\color{blue}differential equations}\index{differential equation}. They are the subject of this
book.
...
\index{population growth and decay}%\index{growth and decay}
...
In the
\personHref{Malthus}{Malthusian model}\index{Malthusian model},
 it is assumed that
...
proportionality are  the \indexterm{birth rate} (births per unit
time per individual) and the \indexterm{death rate} (deaths per unit time
...
\personHref{Verhulst}{Verhulst model}\index{Verhulst model}, where \eqref{eq:1.1.2} is
replaced by
...
Equation~\eqref{eq:1.1.4} is  the \indexterm{logistic equation}. You
...
\boxit{Glucose Absorption by the Body}\index{glucose absorption by the body}
...
After you've studied \zcref{sec:linear_first_order},
...
\boxit{Spread of Epidemics}\index{epidemic}\index{spread of epidemic}
...
this equation, we say that it is a \indexterm{second order differential
equation}.

\boxit{Interacting Species: Competition}
\index{competition between species}\index{species!competition between}
\index{interacting species, competition}
...
A \indexterm{differential equation} is an equation that contains one or more
derivatives of an unknown function. The \emph{\color{blue}order}\index{differential equation!order of}\index{order of differential equation} of a
differential equation is the order of the highest
derivative that it contains. A differential equation is an
\indexterm{ordinary differential equation}\index{differential equation!ordinary}
if it involves an unknown function of only one variable, or a
\indexterm{partial differential equation}\index{partial differential equation!defined}\index{differential equation!partial}
if it involves partial derivatives of a function of more
than one variable. For now we'll consider only ordinary differential
equations, and we'll just call them \emph{\color{blue}differential equations}\index{differential equation}.
...
\end{align*}
Although none of these equations are written as in
...
\index{solution of differential equation}
\index{differential equation!solution}

A \emph{\color{blue}solution}\index{differential equation!solution} of a differential equation is a function that
...
\indexterm{solution curve}.
More generally, a curve $C$ is said to be an
\indexterm{integral curve} of a differential equation
if every function
...
\boxit{Initial Value Problems}\index{initial value problem}
...
y'=x^3,\quad y(1)=2.
\]
 We call this an \indexterm{initial value problem}.
The requirement $y(1)=2$ is an \indexterm{initial condition}.
...
These requirements are the \emph{\color{blue}initial conditions}\index{initial condition}.
...
satisfies the differential equation is  the \indexterm{interval of
validity}\index{validity, interval of} of $y$.
...
\index{free fall under constant gravity}

The term \indexterm{initial value problem}  originated in problems
...
\index{first order equation!direction field}
...
y'(x)=f(x,y(x))
\]
for all values of $x$ in some interval, and an integral
curve\index{integral curve} is either the graph of a solution or is
...
number $f(x_0,y_0)$. This is the basis of \emph{\color{blue}the method of direction fields}\index{direction field}.

If $f$ is defined on a set $R$, we can construct a \indexterm{direction
...
\]
form a \indexterm{rectangular grid} (\zcref{figure:1.3.1}). Through each
...
Equations~\eqref{eq:1.3.2} and \eqref{eq:1.3.3} can be reformulated as in
\eqref{eq:1.3.4} with
\[
...
\emph{In \zcref[range]{exer:1.3.1,exer:1.3.11} a  direction
field is drawn for the given equation. Sketch some integral curves.}

\begin{exerciselist}

\item\label{exer:1.3.1}\parbox[t]{\linewidth}{$y'=\dst{\frac{x}{y}}$:\\
...
\item\parbox[t]{\linewidth}{$y'=\dfrac{2xy^2}{1+x^2}$:\\
...
\item\parbox[t]{\linewidth}{$y'=x^2(1+y^2)$:\\
...
\item\parbox[t]{\linewidth}{$y'=\dfrac{1}{1+x^2+y^2}$:\\
...
\item\parbox[t]{\linewidth}{$y'=-(2xy^2+y^3)$:\\
  ...
  width=5.4in,height=3.3in,keepaspectratio]{exer010305}}\bigskip

\item\parbox[t]{\linewidth}{$y'=(x^2+y^2)^{1/2}$:\\
  \includegraphics[alt={A field for -1<x,y<1 with arrows pointing up and right.  The arrows are horizontal near the origin.},
  width=5.4in,height=3.3in,keepaspectratio]{exer010306}}\bigskip

\item\parbox[t]{\linewidth}{$y'=\sin xy$:\\
  ...
  width=5.4in,height=3.3in,keepaspectratio]{exer010307}}\bigskip

\item\parbox[t]{\linewidth}{$y'=e^{xy}$:\\
  ...
  width=5.4in,height=3.3in,keepaspectratio]{exer010308}}\bigskip

\item\parbox[t]{\linewidth}{$y'=(x-y^2)(x^2-y)$:\\
  ...
  width=5.4in,height=3.3in,keepaspectratio]{exer010309}}\bigskip

\item\parbox[t]{\linewidth}{$y'=x^3y^2+xy^3$:\\
  ...
  width=5.4in,height=3.3in,keepaspectratio]{exer010310}}\bigskip

\item\label{exer:1.3.11}\parbox[t]{\linewidth}{$y'=\sin(x-2y)$:\\
  ...
  width=5.4in,height=3.3in,keepaspectratio]{exer010311}}\bigskip
...
population model\index{Verhulst model}
and the spread of an epidemic, we can write both in the form
\[
y'=ay-by^2,\tag{C}
\]
