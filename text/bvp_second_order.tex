...
\index{two-point boundary value problems}
\index{boundary value problem!two-point}
\index{second order equation!two-point boundary value problem}
...
has no zeros on an open interval
$(a,b)$.\index{boundary value problem!two-point!assumptions}\index{boundary value problem!two-point!boundary conditions}\index{boundary value problem!two-point!defined}\index{second order equation!two-point boundary value problem!boundary conditions}\index{two-point boundary value problem!boundary conditions}
From \zcref{thmtype:5.3.1}, if $x_{0}$ is in
...
\rho y(b)+\delta y'(b)=k_{2}.\index{second order equation!two-point boundary value problem!defined}\index{two-point boundary value problem!defined}
\end{equation}


\medskip
The
assumptions\index{second order equation!two-point boundary value problem!assumptions}\index{two-point boundary value problem!assumptions}
stated in this problem apply throughout this section and
...
We call $a$ and $b$ \indexterm{boundary points}.  The conditions \eqref{eq:13.1.4}
and \eqref{eq:13.1.5}  are
\indexterm{boundary conditions}\index{boundary conditions!two-point},
...
Ly=F,\qquad B_{1}(y)=k_{1},\qquad B_{2}(y)=k_{2}.
\end{equation}
This boundary value problem is \emph{homogeneous} if $F=0$
and $k_{1}=k_{2}=0$;   otherwise it's
\emph{nonhomogeneous}\index{boundary value problem!two-point!(non)homogeneous}\index{second order equation!two-point boundary value problem!(non)homogeneous}\index{two-point boundary value problem!(non)homogeneous}.
...
for initial value
problems.\index{second order equation!two-point boundary value problem!solving}
...
This is the
\indexterm{Green's function}\index{boundary value problem!two-point!Green's function}\index{two-point boundary value problem!Green's function}
for the boundary value problem \eqref{eq:13.1.16}.
...
\end{equation}
and find the Green's
function\index{Green's function}\index{second order equation!two-point boundary value problem!Green's function}
for this problem.
\end{example}
...
then find  the Green's function\index{Green's function}
for the problem and use it to write
...
to solve  (A) with
\parbox{.7\linewidth}{\begin{alist*}[columns=2]
\item $F(x)=x^{3/2}$ \quad and
\item $F(x)=x^{5/2}$.
\end{alist*}}
...
to solve  (A) with
\parbox{.5\linewidth}{\begin{alist*}[columns=2]
\item $F(x)=2x^{3}$ and
\item $F(x)=6x^{4}$.
\end{alist*}}
...
\index{Sturm-Liouville problem}\index{Sturm-Liouville problem!defined}
\index{eigenvalue problem!Sturm-Liouville problem}
...
We say that $\lambda$ is an \indexterm{eigenvalue} of  \eqref{eq:13.2.1} if
\eqref{eq:13.2.1} has a nontrivial solution $y$.   In this case, $y$ is an
\emph{eigenfunction associated with $\lambda$}\index{eigenfunction associated with lambda},
or a $\lambda$-\emph{eigenfunction}\index{lambda-eigenfunction}.
\emph{Solving}  the eigenvalue
problem means finding all eigenvalues and associated eigenfunctions
of \eqref{eq:13.2.1}.


\begin{example}\label{example:13.2.1}
Solve the eigenvalue problem\index{Sturm-Liouville problem!solving}
...
problem\index{eigenvalue problem!Sturm-Liouville problem!solving}
...
\personHref{Sturm}{Sturm}-\personHref{Liouville}{Liouville equation}\index{Liouville, Joseph}, and the
eigenvalue problem
...
Sturm-Liouville problem\index{eigenvalue problem!Sturm-Liouville problem!defined}\index{Sturm-Liouville equation}.
...
\begin{gather}
c_{1}+kc_{2}=0\notag\\
(\cosh k+3k\sinh k)c_{1}+(\sinh k+3k\cosh k)c_{2}=0.\label{eq:13.2.17}
\end{gather}
%\end{equation}
...
\begin{gather}
c_{1}+kc_{2}=0\notag\\
(\cos k-3k\sin k)c_{1}+(\sin k+3k\cos k)c_{2}=0.\label{eq:13.2.20}
\end{gather}
%\end{equation}
The determinant of this system is
\begin{align*}
...
\begin{theorem}\label{thmtype:13.2.2}
If
\[
Ly=(p(x)y')'+q(x)y
\]
...
\emph{with respect to}
$r=r(x)$.\index{eigenvalue problem!Sturm-Liouville problem!orthogonality}\index{orthogonality!Sturm-Liouville problem}\index{Sturm-Liouville problem!orthogonality}

\zcref{thmtype:13.1.1}  implies the next theorem.

\begin{theorem}\label{thmtype:13.2.5}
...
\begin{theorem}\label{thmtype:13.2.6}
...
Then
\[
\frac{f(x-)+f(x+)}{2}=\sum_{n=1}^{\infty}c_{n}y_{n}(x)\index{Sturm-Liouville expansion}
\]
