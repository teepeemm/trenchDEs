...
\index{linear second order equation}


\noindent
\textsc{In this chapter} we study a particularly important class of
...
\]
Such equations are said to be \emph{\color{blue}linear}\index{linear differential equation}\index{differential equation!linear}. As in the case of first
order linear equations, (A) is said to be \emph{\color{blue}homogeneous}\index{homogeneous differential equation}\index{differential equation!homogeneous}
if $F\equiv0$, or \emph{\color{blue}nonhomogeneous}\index{nonhomogeneous differential equation}\index{differential equation!nonhomogeneous} if $F\not\equiv0$.

\medskip\noindent
\zcref[S,CAP,font=\scshape]{sec:homog_linear} is devoted to the theory of homogeneous linear
equations.


\medskip\noindent
\zcref[S,CAP,font=\scshape]{sec:const_coeff_homog} deals with homogeneous equations of the
special form
\[
ay''+by'+cy=0,
\]
...
\medskip\noindent
\zcref[S,CAP,font=\scshape]{sec:nonhomog_linear} presents the theory of nonhomogeneous linear
equations.


\medskip\noindent
\zcref[S,CAP,font=\scshape]{sec:undetermined_coeffs_i,sec:undetermined_coeffs_ii}
present  the \indexterm{method of
...
parameters
introduced in \zcref{ch:first_order}.


\medskip\noindent
\zcref[S,CAP,font=\scshape]{sec:reduction_order} deals with \indexterm{reduction of order}, a
...
\medskip\noindent
\zcref[S,CAP,font=\scshape]{sec:variation_param}
deals with the method traditionally called
\indexterm{variation of parameters},
...
\section{Homogeneous Linear Equations}\label{sec:homog_linear}
\index{homogeneous equation!linear second order}
\index{linear second order equation!homogeneous}

A second order differential equation is said to be
\emph{\color{blue}linear}\index{linear differential equation}\index{differential equation!linear}\index{linear second order equation!defined}
if it can be written as
\begin{equation}\label{eq:5.1.1}
y''+p(x)y'+q(x)y=f(x).
\end{equation}
We call the function $f$ on the right a \indexterm{forcing function},
...
\eqref{eq:5.1.1} is \emph{\color{blue}homogeneous}\index{homogeneous differential equation}\index{differential equation!homogeneous} if $f\equiv0$ or
\emph{\color{blue}nonhomogeneous}\index{nonhomogeneous equation}\index{linear second order equation!nonhomogeneous}\index{nonhomogeneous equation!linear second order}
if $f\not\equiv0$. Since these definitions are like
the corresponding definitions in \zcref{sec:linear_first_order} for the linear
...
\zcref{thmtype:2.1.1} gives a formula for the general solution of
\eqref{eq:5.1.2} in the case where $f\equiv0$ and
\zcref{thmtype:2.1.2} gives
...
In \zcref{sec:linear_first_order} we considered the homogeneous equation
...
Since $y\equiv0$ is obviously a solution of \eqref{eq:5.1.3} we call it the
\indexterm{trivial solution}\index{differential equation!trivial solution}\index{trivial solution!homogeneous linear second order equation}.
Any other solution is
\emph{\color{blue}nontrivial}\index{nontrivial solution!homogeneous linear second order equation}\index{differential equation!nontrivial solution}.
Under the assumptions of \zcref{thmtype:5.1.1}, the only solution of
...
continuous on $(-\infty,\infty)$. Therefore \zcref{thmtype:5.1.1}
...
\begin{alistSolution}
\item If $y_1=e^x$ then $y_1'=e^x$ and $y_1''=e^x=y_1$,
so  $y_1''-y_1=0$.
...
\begin{alistSolution}
\item If $y_1=\cos\omega x$ then $y_1'=-\omega\sin\omega x$
and
...
\zcref[S]{thmtype:5.1.1} implies that if $k_0$ and $k_1$ are
arbitrary real numbers then the initial value problem
...
Since $P_0$ has no zeros on these intervals, \zcref{thmtype:5.1.1}
...
\begin{alistSolution}
\item If $y_1=x^2$ then $y_1'=2x$ and $y_1''=2$, so
\[
...
\item If
\begin{align}
y&=c_1x^2+\frac{c_2}{x^2},\label{eq:5.1.16}\shortintertext{then}
y'&=2c_1x-\frac{2c_2}{x^3}\label{eq:5.1.17}\shortintertext{and}
...
Equation}
\index{homogeneous equation!linear second order!general solution}
\index{linear equation!homogeneous second order!general solution}
\index{second order equation!homogeneous linear!general solution}
\index{general solution!second order equation!homogeneous linear}
...
y=c_1y_1+c_2y_2
\]
is a \emph{\color{blue}linear combination of $y_1$ and $y_2$}\index{linear combination of functions}. For
...
\zcref{example:5.1.1,example:5.1.2,example:5.1.3}.
...
\]
 then
\[
y'=c_1y_1'+c_2y_2'\quad\text{and}\quad y''=c_1y_1''+c_2y_2''.
\]
Therefore
\begin{align*}
...
solutions of $\eqref{eq:5.1.18}$ on}\index{fundamental set of solutions}\index{fundamental set of solutions!homogeneous linear second order equation}\index{solution!fundamental set} $(a,b)$ if every solution
...
\emph{\color{blue}general solution of $\eqref{eq:5.1.18}$ on}\index{general solution} $(a,b)$.
...
$(a,b)$ are \emph{\color{blue}linearly independent on}\index{linear independence} $(a,b)$ if neither is a
...
\index{Abel's formula}\index{Wronskian}
\index{homogeneous linear second order equation!Abel's formula}
\index{homogeneous linear second order equation!Wronskian}
\index{linear second order equation!homogeneous!Abel's formula}
\index{linear second order equation!homogeneous!Wronskian}
...
since \zcref{thmtype:5.1.1} implies that \eqref{eq:5.1.21} has a
...
\begin{aligned}
c_1&=\frac{y_2'(x_0)k_0-y_2(x_0)k_1}{y_1(x_0)y_2'(x_0)-y_1'(x_0)y_2(x_0)}\\
...
\[
y''_1 =-py'_1-qy_1\quad\text{and}\quad
y''_2 =-py'_2-qy_2.
\]
...
\personHref{Wronski}{Wronskian of $\{y_1,y_2\}$}.
Formula \eqref{eq:5.1.29} is
\personHref{Abel}{Abel's formula}.
...
\personHref{Cramer}{Cramer's rule}.
...
\begin{alistSolution}
\item Since $p\equiv0$, we can verify Abel's formula
...
\item Again, since $p\equiv0$, we can verify Abel's
formula
by showing that $W$ is constant, which is true, since
\begin{align*}
W(x)&=\begin{vmatrix}
...
&=-\left(\frac{4}{x_0}\right)\left(\frac{x_0}{x}\right)\quad\text{from
\eqref{eq:5.1.31}}\\
&=-\frac{4}{x},
\end{align*}
...
\end{equation}
However, if $W(x_0)=0$,
\zcref{thmtype:5.1.4} implies that $W\equiv0$ on  $(a,b)$.
...
\zcref{thmtype:5.1.1}. (See also \zcref{exer:5.1.24}).
...
We can now complete the proof of \zcref{thmtype:5.1.3}.
From \zcref{thmtype:5.1.5}, two solutions $y_1$ and $y_2$ of
\eqref{eq:5.1.32} are linearly
independent on  $(a,b)$ if and only if $W$ has no zeros on  $(a,b)$.
From \zcref{thmtype:5.1.4} and the motivating comments preceding
...
is linearly independent on $(a,b)$. \hfill\qedsymbol
...
not both zero.
Under the assumptions of \zcref{thmtype:5.1.7}, suppose
...
\end{vmatrix}=0
\]
and \zcref{thmtype:5.1.6} implies the stated conclusion.
\end{proof}
...
\item $\{x+1,  x^2+2\}$
\item $\{ x^{1/2},  x^{-1/3}\}$
\item $\{\dfrac{\sin x}{x},  \dfrac{\cos x}{x}\}$
...
\personHref{Legendre}{Legendre's equation}\index{Legendre's equation}.)
...
\personHref{Bessel}{Bessel's equation}\index{Bessel's equation}.)
...
\item % (b)
Conclude from (a) that if $y_2=uy_1$ where
$u'=K\dfrac{e^{-P(x)}}{y_1^2(x)}$, then  $\{y_1,y_2\}$
is a fundamental set of solutions of (A)
on  $(a,b)$.
\end{alist}

\exercisetext{In \zcref[range]{exer:5.1.10,exer:5.1.23} use the
method suggested by \zcref{exer:5.1.9} to find a second solution
...
\item\label{exer:5.1.21}$(x^2-4)y''+4xy'+2y=0$;  \quad  $y_1=\dfrac{1}{x-2}$

\item\label{exer:5.1.22}$(2x+1)xy''-2(2x^2-1)y'-4(x+1)y=0$;\quad  $y_1=\dfrac{1}{x}$
...
Use   \zcref{thmtype:5.1.1} to show that
...
on  $(a,b)$. Let
\[
z_1=\alpha y_1+\beta y_2\quad\text{and}\quad z_2=\gamma y_1+\delta
y_2,
\]
...
\item % (b)
Use \zcref{exer:5.1.25}(b) to show that these results don't
contradict \zcref{thmtype:5.1.5}, because neither $y_1$ nor $y_2$
...
functions $\{y_1,y_2\}$. Use \zcref{thmtype:5.1.1} to show that
...
\[
y''+p_1(x)y'+q_1(x)y=0 \quad\text{and}\quad
y''+p_2(x)y'+q_2(x)y=0
\]
...
\item\label{exer:5.1.35}Use the method suggested by \zcref{exer:5.1.34} to find a linear
...
\]
 such that
\[
y_1(x_0)=1, \quad y'_1(x_0)=0\quad\text{and}\quad
y_2(x_0)=0,\;  y'_2(x_0)=1.
\]
(\zcref{thmtype:5.1.1} implies that each of these initial value
problems
has a unique solution on $(a,b)$.)
\begin{alist}
\item % (a)
...
initial conditions
\[
y_1(x_0)=1, \quad y'_1(x_0)=0 \quad\text{and}\quad y_2(x_0)=0,
\quad y'_2(x_0)=1.
\]
Then use  \zcref{exer:5.1.37}(c) to write
...
\item\label{exer:5.1.39}Let $x_0$ be an arbitrary real number. Given (\zcref{example:5.1.1})
...
\item\label{exer:5.1.40}Let $x_0$ be an arbitrary real number. Given (\zcref{example:5.1.2})
...
\item\label{exer:5.1.41}Recall from \zcref{exer:5.1.4} that
...
Find solutions of (A)
 such that
\[
y_1(0)=1, \quad y'_1(0)=0\quad\text{and}\quad
y_2(0)=0,\;  y'_2(0)=1.
\]
Then use  \zcref{exer:5.1.37}(c) to write
...
\item\label{exer:5.1.43}\begin{alist}
\item % (a)
...
\index{homogeneous equation!constant coefficient}
\index{constant coefficient equation!homogeneous}
\index{linear second order equation!homogeneous!constant coefficient}

If $a,b$, and $c$ are real constants and $a\ne0$, then
\[
ay''+by'+cy=F(x)
\]
is said to be a \indexterm{constant coefficient equation}.
...
p(r)=ar^2+br+c
\]
is  the \indexterm{characteristic polynomial}\index{polynomial!characteristic}
of \eqref{eq:5.2.1}, and $p(r)=0$
is  the \indexterm{characteristic equation}\index{equation!characteristic}.
...
\begin{enumext}[label=\arabic*,wrap-label={\textsc{Case} #1.},labelwidth=4em]
\item $b^2-4ac>0$, so the characteristic equation has two
distinct real roots.
\item $b^2-4ac=0$, so the characteristic equation has a
repeated real root.
\item $b^2-4ac<0$, so the characteristic equation has
complex roots.
\end{enumext}
...
\index{constant coefficient equation!homogeneous!distinct real roots}
\index{homogeneous equation!constant coefficient!distinct real roots}
...
nonconstant, \zcref{thmtype:5.1.6} implies that the general solution
of \eqref{eq:5.2.4} is
\begin{equation} \label{eq:5.2.6}
y=c_1e^{-x}+c_2e^{-5x}.
\end{equation}

\item We  must determine $c_1$ and $c_2$
...
\]
\zcref[S]{figure:5.2.1} is a graph
of this solution.

\begin{figure}
  \centering
  \includegraphics[alt={A curve for 0<x<5 and 0<y<3 beginning at (0,3) and decreasing toward a horizontal asymptote y=0.},
  width=5.67in,height=3.3in,keepaspectratio]{fig050201}
  \caption{$y=\dfrac{7}{2}e^{-x}-\dfrac{1}{2}e^{-5x}$}
  \label{figure:5.2.1}
\end{figure}
\end{alistSolution}
...
Since $y_2/y_1=e^{(r_2-r_1)x}$ is nonconstant, \zcref{thmtype:5.1.6}
 implies that  $\{y_1,y_2\}$ is a fundamental
set of solutions of $ay''+by'+cy=0$.


\boxit{Case 2: A Repeated Real Root}
\index{characteristic equation!repeated real root}
\index{constant coefficient equation!homogeneous!repeated real roots}
\index{homogeneous equation!constant coefficient!repeated real roots}
...
used in \zcref{sec:linear_first_order} to solve the nonhomogeneous
...
\indexterm{reduction of order} that we'll study in \zcref{sec:reduction_order}.
For other ways to obtain a second solution of \eqref{eq:5.2.8} that's
not a multiple of $e^{-3x}$, see
{Exercises~5.1.\ref{exer:5.1.9}},
{5.1.\ref{exer:5.1.12}}, and {5.2\ref{exer:5.2.33}}.
% 5.2 wasn't present in the last one


If $y=ue^{-3x}$, then
\[
y'=u'e^{-3x}-3ue^{-3x}\quad\text{and}\quad
...
$y_2/y_1=x$
is nonconstant, \zcref{thmtype:5.1.6} implies that   $\{y_1,y_2\}$ is
fundamental set of solutions of \eqref{eq:5.2.8}, and \eqref{eq:5.2.10}
is the general solution.

\item Differentiating   \eqref{eq:5.2.10} yields
...
y=e^{-3x}(3+8x).
\]
\zcref[S]{figure:5.2.2} is a graph of this solution.

\begin{figure}
  \centering
  \includegraphics[alt={A curve for 0<x<5 and 0<y<3 beginning at (0,3) and decreasing toward a horizontal asymptote y=0.},
  width=5.67in,height=3.3in,keepaspectratio]{fig050202}
  \caption{$y=e^{-3x}(3+8x)$}
  \label{figure:5.2.2}
\end{figure}
\end{alistSolution}
...
Since $p(r_1)=0$, $y_1=e^{r_1x}$ is a solution of
$ay''+by'+cy=0$, and therefore of \eqref{eq:5.2.12}. Proceeding as in
\zcref{example:5.2.2}, we look for other solutions of \eqref{eq:5.2.12}
of the form $y=ue^{r_1x}$; then
\[
y'=u'e^{r_1x}+rue^{r_1x}\quad\text{and}\quad
...
\index{characteristic equation!complex conjugate roots}
\index{constant coefficient equation!homogeneous!complex conjugate roots}
\index{homogeneous equation!constant coefficient!complex conjugate roots}
...
is true (see \zcref{exer:5.2.34});     however, there are
...
\[
e^{(-2+3i)x}=e^{-2x}e^{3ix}\quad\text{and}\quad
e^{(-2-3i)x}=e^{-2x}e^{-3ix},
\]
...
u''+9u=0.
\]
From \zcref{example:5.1.2}, the  general solution of this equation is
\[
...
\item Imposing the condition $y(0)=2$
...
\]
\zcref[S]{figure:5.2.3}  is a graph of this function.
\end{alistSolution}

\begin{figure}
  \centering
  \includegraphics[alt={A curve for 0<x,y<2 beginning at (0,2) and decreasing toward a horizontal asymptote y=0.  The curve crosses the x axis near x=0.5 and x=1.5.},
  width=5.67in,height=3.3in,keepaspectratio]{fig050203}
  \caption{$y=e^{-2x}(2\cos 3x+ \dfrac{1}{3}\sin 3x)$}
  \label{figure:5.2.3}
\end{figure}
...
is   positive;
 $\lambda$ and $\omega$ are  the \emph{\color{blue}real}\index{real part}
and \emph{\color{blue}imaginary parts}\index{imaginary part}, respectively, of $r_1$.
Similarly, $\lambda$ and $-\omega$ are the real and imaginary parts of
$r_2$. We say that $r_1$ and $r_2$ are \indexterm{complex conjugates},
which means that they have the same real part and their imaginary
parts have the same absolute values, but opposite signs.

As in \zcref{example:5.2.3}, it's reasonable to
...
\[
e^{(\lambda+i\omega)x}=e^{\lambda x}e^{i\omega x}
\quad\text{and}\quad
e^{(\lambda-i\omega)x}=e^{\lambda x}e^{-i\omega x},
\]
...
\[
y'=u'e^{\lambda x}+\lambda ue^{\lambda x}\quad\text{and}\quad
...
u''+\omega^2 u=0.
\]
From \zcref{example:5.1.2}, the  general solution of this
equation is
\[
...
$y_2/y_1=\tan\omega x$
is nonconstant, so  \zcref{thmtype:5.1.6} implies that
$\{y_1,y_2\}$ is
...
\boxit{Summary}
\index{constant coefficient equation!homogeneous!summary}
\index{homogeneous equation!constant coefficient!summary}
...
\exercises
In \zcref[range]{exer:5.2.1,exer:5.2.12} find the general solution.
...
%
\textsl{In \zcref[range]{exer:5.2.13,exer:5.2.17} solve
the initial value problem.}
...
\item\label{exer:5.2.16}$4y''-4y'-3y=0, \quad  y(0)=\dfrac{13}{12},\quad y'(0)=\dfrac{23}{24}$

\item\label{exer:5.2.17}$4y''-12y'+9y=0, \quad  y(0)=3,\quad y'(0)=\dfrac{5}{2}$

\exercisetext{In \zcref[range]{exer:5.2.18,exer:5.2.21} solve
...
$36y''-12y'+y=0, \quad  y(0)=3,\quad y'(0)=\dfrac{5}{2}$
...
\item % (c)
The statement of
\zcref{thmtype:5.2.1}  is convenient for solving an
initial value problem
\[
...
\[
y=c_1e^{r_1x}+c_2e^{r_2x}, \quad  y=e^{r_1x}(c_1+c_2x),\text{ or }
y=e^{\lambda x}(c_1\cos\omega x+c_2\sin\omega x)
\]
(whichever is applicable) is more complicated. Use (b) to restate
\zcref{thmtype:5.2.1} in a form more convenient for solving
(B).
\end{alist}

\exercisetext{In \zcref[range]{exer:5.2.23,exer:5.2.28} use a method
suggested by \zcref{exer:5.2.22} to solve the initial value
problem.}

\item\label{exer:5.2.23}$y''+3y'+2y=0, \quad  y(1)=-1,\quad y'(1)=4$

\item\label{exer:5.2.24}$y''-6y'-7y=0, \quad  y(2)=-\dfrac{1}{3},\quad y'(2)=-5$

\item\label{exer:5.2.25}$y''-14y'+49y=0, \quad  y(1)=2,\quad y'(1)=11$

\item\label{exer:5.2.26}$9y''+6y'+y=0, \quad  y(2)=2,\quad y'(2)=-\dfrac{14}{3}$
...
by \zcref{exer:5.2.22}  to find a formula for the solution
of
\[
ay''+by'+cy=0, \quad  y(x_0)=k_0,\quad y'(x_0)=k_1.
\]
...
\zcref{exer:5.2.22} to find a formula for the solution of
\[
ay''+by'+cy=0, \quad  y(x_0)=k_0,\quad y'(x_0)=k_1.
\]
...
by \zcref{exer:5.2.22} to find a formula for the solution of
\[
ay''+by'+cy=0, \quad  y(x_0)=k_0,\quad y'(x_0)=k_1.
\]
...
\]
This is
\personHref{Euler}{Euler's identity}\index{Euler's identity}.
\item % (b)
Starting from
\[
...
Use \zcref{thmtype:5.2.1}(c) to verify that if the
...
\section{Nonhomogeneous Linear Equations}\label{sec:nonhomog_linear}
\index{nonhomogeneous equation!linear second order}
\index{linear second order equation!nonhomogeneous}

We'll now consider the nonhomogeneous linear second order equation
\begin{equation} \label{eq:5.3.1}
y''+p(x)y'+q(x)y=f(x),
\end{equation}
where the forcing function\index{forcing function} $f$ isn't  identically zero.
The next
theorem, an extension of \zcref{thmtype:5.1.1}, gives sufficient
...
\index{existence theorem!second order equation}
\index{uniqueness theorem!second order equation}
\index{second order equation!existence and uniqueness theorem}
...
To find the general solution\index{linear second order equation!nonhomogeneous!general solution} of  \eqref{eq:5.3.1}
...
y''+p(x)y'+q(x)y=0
\end{equation}
on $(a,b)$. We call \eqref{eq:5.3.2} the
\indexterm{complementary equation}\index{linear second order equation!complementary equation}\index{nonhomogeneous linear second order equation!complementary equation}
for \eqref{eq:5.3.1}.
...
$y_p$ a
\emph{\color{blue}particular solution}\index{particular solution!nonhomogeneous equation!linear second order}\index{linear second order equation!nonhomogeneous!particular solution}\index{nonhomogeneous linear second order equation!particular solution}
of \eqref{eq:5.3.1}; it can be any solution that we can find, one way or another.
...
\[
y'=y_p'+c_1y_1'+c_2y_2'\quad\text{and}\quad
y''=y_p''+ c_1y_1''+c_2y_2'',
\]
so
\begin{align*}
...
&\quad{}+q(x)(y_p+c_1y_1+c_2y_2)\\
&=(y_p''+p(x)y_p'+q(x)y_p)+c_1(y_1''+p(x)y_1'+q(x)y_1)\\
&\quad{}+c_2(y_2''+p(x)y_2'+q(x)y_2)\\
...
$\eqref{eq:5.3.3}$ on $(a,b)$}\index{general solution}\index{general solution!nonhomogeneous linear second order equation}.


If $P_0$, $P_1$, and $F$ are continuous and $P_0$ has no zeros on
$(a,b)$, then \zcref{thmtype:5.3.2} implies that the general
solution of
...
\]
and apply \zcref{thmtype:5.3.2} with $p=P_1/P_0$, $q=P_2/P_0$, and
$f=F/P_0$.
...
\begin{alistSolution}
\item
We can apply \zcref{thmtype:5.3.2} with $(a,b)= (-\infty,\infty)$,
...
y=1+\cos x+7\sin x.
\]
\zcref[S]{figure:5.3.1} is a graph of this function.

\begin{figure}
  \centering
  \includegraphics[alt={A generic sinusoidal curve.},
  width=5.67in,height=3.3in,keepaspectratio]{fig050301}
...
if
\[
y_p=A+Bx+Cx^2\quad\text{then}\quad
y_p'=B+2Cx\quad\text{and}\quad  y_p''=2C,
\]
so
\begin{align*}
y_p''-2y_p'+y_p
...
\eqref{eq:5.3.10} and  \zcref{thmtype:5.3.2} implies that
\begin{equation} \label{eq:5.3.12}
y=1+3x+x^2+e^x(c_1+c_2x)
\end{equation}
is the general solution of \eqref{eq:5.3.10}.

\item Imposing the initial condition $y(0)=-2$ in
...
y=1+3x+x^2-e^x(3-x).
\]
\zcref[S]{figure:5.3.2}  is a graph of this solution.

\begin{figure}
  \centering
  \includegraphics[alt={A curve for 0<x<2 and -2<y<16 beginning at (0,-2), crossing the x axis near x=0.6, and increasing from there.},
  width=5.67in,height=3.3in,keepaspectratio]{fig050302}
...
\begin{solution}
In \zcref{example:5.1.3},  we verified that $y_1=x^2$ and $y_2=1/x^2$
...
\zcref[S]{thmtype:5.3.2} implies that the general solution of
\eqref{eq:5.3.13} on  $(-\infty,0)$ and  $(0,\infty)$ is
\[
y=\frac{x^4}{6}+c_1x^2+\frac{c_2}{x^2}.\qedhere
\]
\end{solution}
...
\index{linear second order equation!nonhomogeneous!super position principle}

The next theorem enables us to break a nonhomogeneous equation into
simpler parts, find a particular solution\index{particular solution!nonhomogeneous equation!linear second order} for each part, and then
combine their solutions to obtain a particular solution of the
original problem.

\begin{theorem}[The Principle of Superposition]\label{thmtype:5.3.3}
\index{principle of superposition}\index{superposition!principle of}
...
It's easy to generalize  \zcref{thmtype:5.3.3}
to the equation
...
proof similar to the proof of \zcref{thmtype:5.3.3} we can formulate
...
\begin{solution}
The right side $F(x)=2x^4+4x^2$ in \eqref{eq:5.3.17}
...
In \zcref[range]{exer:5.3.1,exer:5.3.6}  find a
particular solution  by the method used in
\zcref{example:5.3.2}. Then find the general
...
\item\label{exer:5.3.7}Show that the method used in \zcref{example:5.3.2}
...
\end{exerciselist}
%
\textsl{In \zcref[range]{exer:5.3.8,exer:5.3.13} find a
particular solution by the method used in \zcref{example:5.3.3}.}

\begin{exerciselistTwoColumn}
\item\label{exer:5.3.8}$x^2y''+7xy'+8y=\dfrac{6}{x}$
\item\label{exer:5.3.9}$x^2y''-7xy'+7y=13x^{1/2}$
\item\label{exer:5.3.10}$x^2y''-xy'+y=2x^3$
\item\label{exer:5.3.11}$x^2y''+5xy'+4y=\dfrac{1}{x^3}$
...
solution in \zcref[range]{exer:5.3.8,exer:5.3.13}
won't yield a particular solution of
\[
...
\item(2)\label{exer:5.3.18}\CGex $y''+8y'+7y=10e^{-2x}, \quad  y(0)=-2,\;
y'(0)=10$

\item(2)\label{exer:5.3.19}\CGex $y''-4y'+4y=e^{x}, \quad  y(0)=2,\quad y'(0)=0$
...
solution in \zcref[range]{exer:5.3.16,exer:5.3.21}
won't yield a particular solution of
\[
y''-7y'+12y=5e^{4x};
\tag{A}
...
x$. In \zcref[range]{exer:5.3.24,exer:5.3.29} use this  to find a
...
solution in \zcref[range]{exer:5.3.24,exer:5.3.29}
won't yield a particular solution of
\[
y''+y=\cos x+\sin x;
\tag{A}
...
\exercisetext{In \zcref[range]{exer:5.3.33,exer:5.3.38} refer
...
\section{The Method of Undetermined Coefficients I}\label{sec:undetermined_coeffs_i}
\index{method of undetermined coefficients}
\index{undetermined coefficients, method of!linear second order equation}
\index{coefficients!method of undetermined}
\index{constant coefficient equation!nonhomogeneous}
\index{linear second order equation!constant coefficient}
\index{linear second order equation!nonhomogeneous!method of undetermined coefficients}
\index{linear second order equation!nonhomogeneous!constant coefficients}
\index{nonhomogeneous linear second order equation!constant coefficients}
\index{nonhomogeneous linear second order equation!undetermined coefficients}
...
From \zcref{thmtype:5.3.2}, the general solution of \eqref{eq:5.4.1}
is
$y=y_p+c_1y_1+c_2y_2$, where $y_p$ is a particular
solution\index{particular solution!nonhomogeneous equation!linear second order}\index{nonhomogeneous linear second order equation!particular solution}
of
...
ay''+by'+cy=0.
\]
In \zcref{sec:const_coeff_homog} we showed how to find $\{y_1,y_2\}$. In this
section we'll show how to find $y_p$. The procedure that we'll use
is called the \indexterm{method of undetermined coefficients}.

Our first example is similar to
\zcref[range]{exer:5.3.16,exer:5.3.21}.
...
\[
 y=2e^{2x}+c_1e^{3x}+c_2e^{4x}.\qedhere
\]
\end{solution}
...
of
\eqref{eq:5.4.2} is a constant multiple of $e^{2x}$ --- it may seem
reasonable to try $y_p=Ae^{4x}$ as a particular solution of
\eqref{eq:5.4.4}. However, this won't work, since we saw in
\zcref{example:5.4.1} that $e^{4x}$ is a solution of the
...
we use the same approach that we used in \zcref{sec:const_coeff_homog} to find a
second solution of
\[
ay''+by'+cy=0
\]
...
y=ue^{4x},\quad
y'=u'e^{4x}+4ue^{4x},\quad\text{and}\quad 
...
\zcref{example:5.4.2}, we look for solutions of \eqref{eq:5.4.6} in the
...
of a constant coefficient equation
\[
ay''+by'+cy=ke^{\alpha x},
\]
...
\zcref{example:5.4.1}.
 (See \zcref[range]{exer:5.4.31,exer:5.4.33}.)
...
Substituting
\[
y=ue^{3x},\quad y'=u'e^{3x}+3ue^{3x},\quad\text{and}\quad
y''=u''e^{3x}+6u'e^{3x}+9ue^{3x}
\]
...
u''+3u'+2u=-1+2x+x^2.
\end{equation}
As in \zcref{exer:5.3.2}, in order to guess a form for a particular
solution of
...
$A$, $B$, and $C$;   thus,
\[
\text{if}\quad u_p=A+Bx+Cx^2\quad\text{then}\quad
u_p'=B+2Cx\quad\text{and}\quad  u_p''=2C.
\]
...
the complementary equation for \eqref{eq:5.4.12}. (See
\zcref{exer:5.4.30}.) Therefore \eqref{eq:5.4.13} does not have a
particular solution of the form $u_p=A+Bx+Cx^2$ that we used
successfully in \zcref{example:5.4.4}, since with this choice of
$u_p$,
\[
u_p''+2u_p'=2C+(B+2Cx)
\]
...
that
\[
u_p'=A+2Bx+3Cx^2\quad\text{and}\quad 
u_p''=2B+6Cx
\]
...
Substituting
\[
y=ue^{-x/2},\quad  y'=u'e^{-x/2}-\frac{1}{2}ue^{-x/2},\quad\text{and}\quad
y''=u''e^{-x/2}-u'e^{-x/2}+\frac{1}{4}ue^{-x/2}
\]
...
\zcref{exer:5.4.30}.) To obtain a particular solution of
\eqref{eq:5.4.15} we integrate twice, taking the constants of integration
to be zero;   thus,
\[
u_p'=-2x+6x^2+12x^3\quad\text{and}\quad 
...
\boxit{Summary}
\index{method of undetermined coefficients!summary}
\index{undetermined coefficients!method of!summary}
...
ay''+by'+cy=e^{\alpha x}G(x),
\]
where $G$ is a polynomial (see \zcref{exer:5.4.30}):
\begin{alist}
\item % (a)
If $e^{\alpha x}$ isn't  a solution of the complementary
equation
...
(See \zcref[range]{exer:5.4.34,exer:5.4.36}.) In Case~(a)
...
in \zcref{example:5.4.4}. In Case~(b)  the equation for
...
coefficients can be found by the method used in
\zcref{example:5.4.5}. In Case~(c)  the equation for $u$
will be of the form
\[
au''=G(x)
\]
...
integration to be zero, as in \zcref{example:5.4.6}.
...
coefficients and \zcref{thmtype:5.3.3}, the principle of
superposition.
\index{principle of superposition}
\index{principle of superposition!method of undetermined coefficients}
\index{coefficients!method of undetermined!principle of superposition}
\index{undetermined coefficients, method of!principle of superposition}
\index{nonhomogeneous linear second order equation!undetermined coefficients!superposition principle}
...
\begin{solution}
In \zcref{example:5.4.1} we found that $y_{p_1}=2e^{2x}$
is a particular solution of
\[
y''-7y'+12y=4e^{2x},
\]
and in \zcref{example:5.4.2} we found that $y_{p_2}=5xe^{4x}$
is a particular solution of
\[
y''-7y'+12y=5e^{4x}.
\]
...
\exercises
In \zcref[range]{exer:5.4.1,exer:5.4.14} find a particular
solution.
...
\end{exerciselistTwoColumn}
%
\textsl{In \zcref[range]{exer:5.4.15,exer:5.4.19} find the general
solution.}
...
\end{exerciselistTwoColumn}
%
\textsl{In \zcref[range]{exer:5.4.20,exer:5.4.23} solve the
...
\exercisetext{In \zcref[range]{exer:5.4.24,exer:5.4.29} use
...
as in \zcref{example:5.4.4}. Conclude that (A) has
...
of the same degree as $G$, as in \zcref{example:5.4.5}. Conclude
...
zero, as in \zcref{example:5.4.6}. Conclude that
(A) has a particular solution of the form
$y_p=x^2e^{\alpha x}A(x)$.
\end{alist}

\exercisetext{\zcref[range,S]{exer:5.4.31,exer:5.4.36}
treat the equations considered in
\zcref[range]{example:5.4.1,example:5.4.6}. Substitute the suggested
...
\zcref[range]{example:5.4.1,example:5.4.6}.}

\item\label{exer:5.4.31}Compare with \zcref{example:5.4.1}:
\[
y''-7y'+12y=4e^{2x};\quad y_p=Ae^{2x}
\]

\item\label{exer:5.4.32}Compare with \zcref{example:5.4.2}:
\[
y''-7y'+12y=5e^{4x};\quad y_p=Axe^{4x}
\]

\item\label{exer:5.4.33}Compare with \zcref{example:5.4.3}.
\[
y''-8y'+16y=2e^{4x};\quad y_p=Ax^2e^{4x}
\]

\item\label{exer:5.4.34}Compare with \zcref{example:5.4.4}:
\[
y''-3y'+2y=e^{3x}(-1+2x+x^2),\quad y_p=e^{3x}(A+Bx+Cx^2)
\]

\item\label{exer:5.4.35}Compare with \zcref{example:5.4.5}:
\[
y''-4y'+3y=e^{3x}(6+8x+12x^2),\quad y_p=e^{3x}(Ax+Bx^2+Cx^3)
\]

\item\label{exer:5.4.36}Compare with \zcref{example:5.4.6}:
\[
...
\begin{alist*}[columns=2]
\item $y''+2y'+y=\dfrac{e^{-x}}{\sqrt x}$
\item $y''+6y'+9y=e^{-3x}\ln x$
\item $y''-4y'+4y=\dfrac{e^{2x}}{1+x}$
\item $4y''+4y'+y=4e^{-x/2}\left(\dfrac{1}{x}+x\right)$
\end{alist*}
...
\item\label{exer:5.4.39}Use the method of \zcref{exer:5.4.38} to evaluate the integral.
...
\item\label{exer:5.4.40}Use the method suggested in \zcref{exer:5.4.38} to
evaluate $\int x^ke^{\alpha x}\,dx$, where $k$ is an arbitrary
positive integer and $\alpha\ne0$.
\end{exerciselist}


\section{The Method of Undetermined Coefficients II}\label{sec:undetermined_coeffs_ii}
\index{method of undetermined coefficients}
...
\eqref{eq:5.5.1}. As in \zcref{sec:undetermined_coeffs_i}, the procedure that we
will use is called the \indexterm{method of undetermined coefficients}.


\boxit{Forcing Functions Without Exponential Factors}
\index{forcing function!nonexponential case}
\index{nonhomogeneous linear second order equation!undetermined coefficients!nonexponential forcing function}
...
\begin{align*}
\dfrac{d}{dx}x^r\cos\omega x
&=-\omega x^r\sin\omega x+rx^{r-1}\cos\omega x\qquad\text{and} \\
\dfrac{d}{dx}x^r\sin\omega x
...
form for $y_p$. For the proof see \zcref{exer:5.5.37}.
...
degree polynomials (constants). Therefore \zcref{thmtype:5.5.1}
...
Since
\[
y_p'=-2A\sin2x+2B\cos2x\quad\text{and}\quad 
...
\begin{solution}
The procedure used in  \zcref{example:5.5.1} doesn't work here;
...
dealing with the second case mentioned in \zcref{thmtype:5.5.1}, and
...
&=-4A\sin2x+4B\cos2x-4y_p \text{ (see \eqref{eq:5.5.6})},
\end{align*}
so
\[
...
\zcref{thmtype:5.5.1} tells us to look for a particular solution of
...
&\quad{}+ \left[B_0+3 B_1-3 A_0-2 A_1+(B_1-3 A_1)x\right]\sin x.
\end{aligned}
\end{equation}
...
\begin{align*}
\phantom{-3}A_1+3B_1&=20\\
-3A_1+\phantom{3}B_1&=\phantom{1}0\\
\phantom{-3}A_0+3B_0+3A_1+2B_1&=16\\
...
\begin{align*}
\phantom{-3}A_0+3B_0&=16-3A_1-2B_1=-2\\
...
general, as follows (\zcref{exer:5.5.36}).
...
\quad\text{and}\quad  y_p''+\omega^2 y_p
\]
...
\begin{solution}
According to  \zcref{thmtype:5.5.1}, we should look for a
...
\begin{align*}
y_p'
&=\left[A_0+(2A_1+B_0)x+B_1x^2\right]\cos x\\
&\quad{}+\left[B_0+(2B_1-A_0)x-A_1x^2\right]\sin x\qquad \text{and}\\
y_p''
&=\left[2A_1+2B_0-(A_0-4B_1)x-A_1x^2\right]\cos x\\
&\quad{}+\left[2B_1-2A_0-(B_0+4A_1)x-B_1x^2\right]\sin x,
\end{align*}
so
\[
...
\index{forcing function!exponential case}
\index{nonhomogeneous linear second order equation!undetermined coefficients!exponential forcing function}

To find a particular solution of
\begin{equation} \label{eq:5.5.15}
ay''+by'+cy=e^{\lambda x}\left(P(x)\cos \omega x+Q(x)\sin \omega x\right)
\end{equation}
when $\lambda\ne0$, we recall from \zcref{sec:undetermined_coeffs_i} that
...
used in \zcref[range]{example:5.5.1,example:5.5.4}. Then
...
u''-7u'+12u=0,
\]
\zcref{thmtype:5.5.1} tells us to look for a particular solution
...
&\quad{}+\left[21A_0+3B_0-6A_1-7B_1+(21A_1+3B_1)x\right]\sin 3x.
\end{align*}
...
\begin{aligned}
3A_1-21B_1&=\phantom{15}0\\
21A_1+\phantom{2}3B_1&=150\\
3A_0-21B_0-7A_1+\phantom{2}6B_1&=\phantom{15}2\\
...
yields
\begin{align*}
3A_0-21B_0&=\phantom{-3}2+7A_1-6B_1=45\\
...
&\quad{}+\left[B_0+(2B_1-2A_0)x-2A_1x^2\right]\sin2x\qquad\text{and}\\
u_p''
&=\left[2A_1+4B_0-(4A_0-8B_1)x-4A_1x^2\right]\cos2x\\
&\quad{}+\left[2B_1-4A_0-(4B_0+8A_1)x-4B_1x^2\right]\sin2x,
\end{align*}
so
\[
...
8B_1&=-16\\
-8A_1&=-\phantom{1}8\\
\phantom{-}4B_0+2A_1&=\phantom{-1}6\\
-4A_0+2B_1&=-\phantom{1}8.
\end{aligned}
\end{equation}
...
\eqref{eq:5.5.20}. (See \zcref{exer:5.5.38}). This leads to the
same system \eqref{eq:5.5.22} of equations for $A_0$, $A_1$, $B_0$, and
$B_1$ that we obtained in \zcref{example:5.5.6}. However, if you try
this approach you'll see that deriving \eqref{eq:5.5.22} this way is
much more tedious than the way we did it in \zcref{example:5.5.6}.

\exercises
In \zcref[range]{exer:5.5.1,exer:5.5.17} find a particular solution.
...
\exercisetext{
In \zcref[range]{exer:5.5.1,exer:5.5.17} find a particular solution and graph
it.}
...
\exercisetext{In \zcref[range]{exer:5.5.22,exer:5.5.26} solve the
initial value problem.}
...
\exercisetext{In \zcref[range]{exer:5.5.27,exer:5.5.32} use the
principle
...
\exercisetext{In \zcref[range]{exer:5.5.33,exer:5.5.35}
...
\begin{align*}
 y_p'&=(A'+\omega B)\cos\omega x+(B'-\omega A)\sin\omega x\text{ and}\\
...
such that
\begin{align*}
\quad (c-a\omega^2)A+b\omega B+2a\omega B'+bA'+aA''&=P\phantom{.}\\
...
and, if $1\le r\le k$,
\begin{align*}
\phantom{-}(c-a\omega^2)A_{k-r}+b\omega B_{k-r}&=p_{k-r}+\cdots\phantom{.}\\
...
Conclude from this and \zcref{exer:5.5.36}(b) that
\[
y_p=A(x)\cos\omega x+B(x)\sin\omega x
\tag{B}
\]
...
\]
\item % (b)
Conclude from \zcref{exer:5.5.36}(c)
 that the equation
\[
...
\item\label{exer:5.5.38}Show that  \zcref{thmtype:5.5.1} implies the next theorem:
\begin{theorem*}
...
\item\label{exer:5.5.40}Use the method of \zcref{exer:5.5.39} to evaluate the integral.
\begin{alist*}[columns=2]
...
\item(2) $\int e^{-x}\left[(1+x^2)\cos x+(1-x^2)\sin x\right]\,dx$
\end{alist*}

\end{exerciselist}


\section{Reduction of Order}\label{sec:reduction_order}
\index{reduction of order}
\index{general solution!nonhomogeneous linear second order equation}
\index{linear second order equation!nonhomogeneous!reduction of order}
...
P_0(x)y''+P_1(x)y'+P_2(x)y=0.
\end{equation}
The method is called \indexterm{reduction of order} because it reduces
...
\zcref{sec:basic_concepts}, integrate the solution to obtain $u$, and then
...
\end{equation}
\item % (b)
As a byproduct of (a), find a fundamental set of solutions of
\eqref{eq:5.6.7}.
\end{alist}
\end{example}
...
\zcref{sec:basic_concepts}, we can now see that every solution of
\eqref{eq:5.6.9} is of the form
\[
z=vx\quad\text{where}\quad
v'x=xe^{-x}, \quad\text{so}\quad v'=e^{-x} \quad\text{and}\quad 
v=-e^{-x}+C_1.
\]
...
\item By letting $C_1=C_2=0$ in \eqref{eq:5.6.10}, we see that
...
solution of \eqref{eq:5.6.6}, \zcref{thmtype:5.1.6} implies that
...
\begin{alistSolution}
\item If $y=ux$, then $y'=u'x+u$ and $y''=u''x+2u'$, so
\begin{align*}
...
\[
z=\frac{v}{x^3}\quad\text{where}\quad
\frac{v'}{x^3}=\frac{1}{x}+\frac{1}{x^3}, \quad\text{so}\quad
 v'=x^2+1 \quad\text{and}\quad
 v=\frac{x^3}{3}+x+C_1.
\]
...
\zcref{example:5.6.1}(a), we  conclude that $y_1=x$ and $y_2=1/x$
...
\item Differentiating  \eqref{eq:5.6.16} yields
...
\exercises
In \zcref[range]{exer:5.6.1,exer:5.6.17} find the general solution,
...
\item\label{exer:5.6.2}$x^2y''+xy'-y=\dfrac{4}{x^2};   \quad y_1=x$

\item\label{exer:5.6.3}$x^2y''-xy'+y=x;   \quad y_1=x$

\item\label{exer:5.6.4}$y''-3y'+2y=\dfrac{1}{1+e^{-x}};   \quad y_1=e^{2x}$
...
\exercisetext{In \zcref[range]{exer:5.6.18,exer:5.6.30} find a
...
\exercisetext{In \zcref[range]{exer:5.6.31,exer:5.6.33} solve the
...
\exercisetext{In \zcref{exer:5.6.34,exer:5.6.35} solve the
...
$x^2y''+2xy'-2y=x^2, \quad   y(1)=\dfrac{5}{4},\;
y'(1)=\dfrac{3}{2};   \quad y_1=x$


\item\label{exer:5.6.35}\CGex
$(x^2-4)y''+4xy'+2y=x+2, \quad  y(0)=-\dfrac{1}{3},\quad y'(0)=-1;
\quad y_1=\dfrac{1}{x-2}$
...
$(a,b)$. \note{This exercise is related to \zcref{exer:5.1.9}.}
...
\]
is  a
\personHref{Riccati}{Riccati equation}.  (See
{Exercise~2.4.\ref{exer:2.4.55}}.) Assume that $p$ and $q$ are continuous.
\begin{alist}
\item %(a)
...
\item\label{exer:5.6.38}Use a method suggested by \zcref{exer:5.6.37}  to find all
...
\item\label{exer:5.6.39}Use a method suggested by \zcref{exer:5.6.37} and reduction of
...
\item %(e)
$x^2(y'+y^2)+xy+x^2- \dfrac{1}{4}=0; \quad y_1=-\tan x -\dfrac{1}{2x}$
...
\]
is  the
\personHref{Riccati}{generalized Riccati equation}\index{generalized Riccati equation}\index{Riccati equation!generalized}.
(See {Exercise~2.4.\ref{exer:2.4.55}}.)
...
\section{Variation of Parameters}\label{sec:variation_param}
\index{variation of parameters}
\index{linear second order equation!nonhomogeneous!variation of parameters}

In this section we give a method called
\emph{\color{blue}variation of parameters}\index{variation of parameters!linear second order equation}
for finding a particular
solution\index{particular solution!nonhomogeneous equation!linear second order}\index{nonhomogeneous linear second order equation!particular solution}
of
...
linear equations (\zcref{sec:higher_variation_param}) and  linear systems of
equations
(\zcref{sec:systems_variation_param}), while reduction of order doesn't.
...
idea of what it means from \zcref[range]{exer:5.7.37,exer:5.7.39}.
 \end{itemize}
...
on $(a,b)$, \zcref{thmtype:5.1.6}\; implies that the Wronskian
...
\[
u_1=-\frac{2}{7}x^{7/2}\quad\text{and}\quad u_2=\frac{2}{5}x^{5/2}.
\]
Therefore
\[
y_p=u_1x+u_2x^2
...
\[
y=\frac{4}{35}x^{9/2}+c_1x+c_2x^2.\qedhere
\]
\end{solution}
...
\zcref{exer:5.7.36} for a general discussion of this question.)
...
\[
-u_2'e^{-2x}=\frac{1}{1+e^x},\quad\text{so}\quad
u_2'=-\frac{e^{2x}}{1+e^x}.
 \]
...
\end{equation}
 given that
\[
y_1=\frac{1}{x-1}\quad\text{and}\quad y_2=\frac{1}{x+1}
\]
...
\end{align*}
as the solution of  \eqref{eq:5.7.21}. \zcref[S]{figure:5.7.1} is a graph
of the solution.
\end{solution}

\begin{figure}
  \centering
  \includegraphics[alt={A solution curve on -1<x<1 and -50<y<50 with vertical asymptotes at y=±1.  The curve begins near positive infinity when x=-1, crosses the x axis near 0.25, the y axis near y=-1, and goes to negative infinity as x approaches 1.},
  width=5.67in,height=3.3in,keepaspectratio]{fig050701}
  \caption{ $y=\dfrac{2\ln(x+1)}{x-1}+\frac{3x+1}{x^2-1}$}
  \label{figure:5.7.1}
\end{figure}


\boxit{Comparison of Methods}
\index{undetermined coefficients!comparison}
\index{reduction of order!comparison}
\index{variation of parameters!comparison}
\index{linear second order equation!nonhomogeneous!comparison of solving methods}
\index{nonhomogeneous linear second order equation!comparison of solving methods}
...
\exercises
 In \zcref[range]{exer:5.7.1,exer:5.7.6} use variation of
...
\item\label{exer:5.7.3}$y''-3y'+2y=\dfrac{4}{1+e^{-x}}$
\item\label{exer:5.7.4}$y''-2y'+2y=3e^x \sec x$
\item\label{exer:5.7.5}$y''-2y'+y=14x^{3/2}e^x$
\item\label{exer:5.7.6}$y''-y=\dfrac{4e^{-x}}{1-e^{-2x}}$
\end{exerciselistTwoColumn}
%
\textsl{In \zcref[range]{exer:5.7.7,exer:5.7.29} use variation
...
\item\label{exer:5.7.8}$xy''+(2-2x)y'+(x-2)y=e^{2x};   \quad y_1=e^x,
\quad y_2=\dfrac{e^x}{x}$
...
\exercisetext{In \zcref[range]{exer:5.7.30,exer:5.7.32} use variation
...
\exercisetext{In \zcref[range]{exer:5.7.33,exer:5.7.35} use variation
...
where
\[
A_0=k_0-\int_0^\infty f(t)\sin t\,dt\quad\text{and}\quad 
