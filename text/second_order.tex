...
x^4\right)-x\left(2\over x^3\right)-\frac{4}{x^2}=0
\]
...
x^2 & 1/x^2 \\[2\jot]
2x & -2/x^3
\end{array} \right|=x^2\left(-\frac{2}{x^3}\right)-2x\left(1\over x^2\right)=-\frac{4}{x}.
\end{equation}
...
\[
\int_{x_0}^x p(t)\,dt=\int_{x_0}^x \frac{dt}{t}=\ln\left(x\over
x_0\right),
\]
...
\left(y_2\over y_1\right)'={y_1y_2'-y_1'y_2\over y_1^2}=\frac{W}{y_1^2}.
\end{equation}
However, if $W(x_0)=0$,
...
$y_2$ on  $(a,b)$, since
\[
\left(y_1\over y_2\right)'={y_1'y_2-y_1y_2'\over y_2^2}=-\frac{W}{y_2^2}
\]
...
\end{equation}
 with
\[
\lambda = -\frac{b}{2a},\quad \omega = {\sqrt{4ac-b^2}\over 2a}.
\]
...
$4y''-4y'-3y=0, \quad  y(0)=\dst\frac{13}{12},\quad y'(0)=\dst{23
\over 24}$
...
Show that
\[
a\frac{\partial^2}{\partial^2 x}(e^{rx})+b{\partial \over\partial
...
x}(e^{rx})\right)+b\frac{\partial}{\partial r}\left({\partial
...
=1-\frac{u^2}{2!}+\frac{u^4}{4!}+\cdots+(-1)^n{u^{2n}\over(2n)!}
+\cdots,
\eqno{\rm (B)}
\]
and
...
\begin{eqnarray*}
\dst\frac{d}{dx}x^r\cos\omega x&=&-\omega x^r\sin\omega x+
rx^{r-1}\cos\omega x \\[2\jot]  \arraytext{and}
\dst\frac{d}{dx}x^r\sin\omega x&=&\phantom{-}\omega x^r\cos\omega x+
rx^{r-1}\sin\omega x.
\end{eqnarray*}
...
\emph{\color{blue}Suppose $\omega$ is a positive  number and $P$ and $Q$
...
{v'\over x^3}=\frac{1}{x}+\frac{1}{x^3}, \mbox{\quad so \quad}
 v'=x^2+1 \mbox{\quad
...
and
\begin{eqnarray*}
u_2&=&-\int{e^{2x}\over 1+e^x}\,dx=-\int \frac{v}{1+v}\,dv
...
-{u_1'\over(x-1)^2}-{u_2'\over(x+1)^2}&=&\frac{2}{(x+1)(x^2-1)}.\nonumber
\end{eqnarray}
...
Since
\[
\left[\frac{1}{x^2-1}-\frac{1}{(x+1)^2}\right]={(x+1)-(x-1)\over(x+1)(x^2-1)}
=\frac{2}{(x+1)(x^2-1)},
\]
...
+x\frac{1}{x+1} \\
&=&{2\ln(x+1)\over x-1}+x\left[\frac{1}{x+1}-\frac{1}{x-1}\right]
={2\ln(x+1)\over x-1}-\frac{2x}{(x+1)(x-1)}.
\end{eqnarray*}
However, since
\[
...
y={2\ln(x+1)\over x-1}+\frac{c_1}{x-1}+\frac{c_2}{x+1}.
\end{equation}
Differentiating this yields
\[
y'=\frac{2}{x^2-1}-{2\ln(x+1)\over(x-1)^2}-\frac{c_1}{(x-1)^2}-\frac{c_2}{(x+1)^2}.
\]
...
\begin{eqnarray*}
y&=&{2\ln(x+1)\over x-1}+\frac{2}{x-1}+\frac{1}{x+1}\\
&=&{2\ln(x+1)\over x-1}+\frac{3x+1}{x^2-1}
\end{eqnarray*}
...
\color{blue}
  \caption{ $y=\dst{2\ln(x+1)\over x-1}+\frac{3x+1}{x^2-1}$}
  \label{figure:5.7.1}
\end{figure}
...
%&\vspace*{-5pt} 
\item\label{exer:5.7.6} $y''-y=\dst{4e^{-x}\over
1-e^{-2x}}$
%\end{tabular}
\newpage
