...
From \zcref{exer:2.1.3}, the general solution of
\eqref{eq:4.1.1} is
\[
Q=ce^{at}
\]
...
\begin{table}
\caption{The effect of compound interest}
\label{table:4.1.1}
\centering
...
{(compoundings} &  (value after\\
{per year)} & 5 years)\\\midrule
%\hline
1 & \$133.82 \\
2 & \$134.39 \\
4 & \$134.68 \\
...
\item\label{exer:4.2.2}A fluid initially at $100^\circ$C is placed outside on a
...
\item\label{exer:4.2.3}At 12:00 {\sc pm} a thermometer reading $10^\circ$F is placed in
...
\item\label{exer:4.2.4}A thermometer initially reading $212^\circ$F is placed in a
...
\item\label{exer:4.2.5}An object with initial temperature $150^\circ$C is placed
...
the same as those in Example 3, \eqref{eq:4.3.10} does
...
\item\label{exer:4.3.1}A firefighter  who weighs 192 lb slides down an infinitely long
fire pole
...
\item\label{exer:4.3.2}A firefighter who weighs 192 lb slides down an infinitely long
fire pole
that exerts a frictional resistive force with magnitude proportional
...
\item\label{exer:4.3.3}A boat weighs 64,000 lb. Its propellor produces a constant thrust of
...
\item\label{exer:4.3.4}A constant horizontal force of 10 N pushes a 20 kg-mass through a
...
\item\label{exer:4.3.5}A stone weighing 1/2 lb is thrown upward from an initial height of 5
...
\item\label{exer:4.3.6}A 3200-lb car is moving at 64 ft/s down a 30-degree grade when it
...
\item\label{exer:4.3.7}A 96 lb weight is dropped from rest in a medium that exerts a
...
\item\label{exer:4.3.10}An object weighing 256 lb is dropped from rest in a medium that
...
\item\label{exer:4.3.16}A 64-lb object with initial velocity $v_0\le0$ falls through a dense
...
\item %(a)
Show that Eqn.~\eqref{eq:4.3.23}  can be rewritten as
\[
...
\]
\item %(c)
By requiring that $v(y_m)=0$, use Eqn.~\eqref{eq:4.3.22} to
...
\includegraphics[width=\linewidth,height=2.44in,keepaspectratio]{fig040408}
...
\begin{minipage}[t]{0.49\linewidth}
\tagstructbegin{tag=Sect}
\includegraphics[width=\linewidth,height=2.44in,keepaspectratio]{fig040409}
...
\dst\lim_{t\to\infty}v(t)&=-\infty, &\dst\lim_{t\to-\infty}v(t)&=\infty.
\end{align*}
...
\lim_{t\to\infty}v(t)&=0, &\lim_{t\to-\infty}v(t)&=\infty\qquad
\text{(on \part{a})}  \\
\lim_{t\to\infty}y(t)&=0, &\lim_{t\to-\infty}y(t)&=0,\\
\lim_{t\to\infty}v(t)&=0, &\lim_{t\to-\infty}v(t)&=0\qquad
\text{(on \part{b})}  \\
\lim_{t\to\infty}y(t)&=-\infty, &\lim_{t\to-\infty}y(t)&=0,\\
\lim_{t\to\infty}v(t)&=-\infty, &\lim_{t\to-\infty}v(t)&=0\qquad
\text{ (on \part{c})}.
\end{align*}
\end{example}
...
\tagstructbegin{tag=Sect}
\includegraphics[width=\linewidth,height=2.44in,keepaspectratio]{fig040411}
...
\begin{minipage}{0.49\linewidth}
\tagstructbegin{tag=Sect}
\includegraphics[width=\linewidth,height=2.44in,keepaspectratio]{fig040412}
\captionof{figure}{Figure~\ref{figure:4.4.11} with some trajectories added}
\label{figure:4.4.12}
\tagstructend
\end{minipage}
...
\begin{minipage}{0.49\linewidth}
\tagstructbegin{tag=Sect}
\includegraphics[width=\linewidth,height=2.44in,keepaspectratio]{fig040502}
...
\begin{minipage}{0.49\linewidth}
\tagstructbegin{tag=Sect}
\includegraphics[width=\linewidth,height=2.44in,keepaspectratio]{fig040503}
...
\item\label{exer:4.5.14}Use the method Example~\ref{example:4.5.6}{\bf (a)} to find the equations
...
\[
y=\dst\frac{y_0}{2}+\dst\frac{x}{2y_0}.\tag{B}
\]
\item % (b)
...
\item\label{exer:4.5.17}Use the results of Exercise \ref{exer:4.5.16} to find the equations of two
...
\item\label{exer:4.5.29}$\dst{y=\frac{ce^x}{x}}$
...
\item\label{exer:4.5.33}Use the result of Exercise~\ref{exer:4.5.32} to find a family of curves
that intersect every nonvertical line through the origin at the angle
$\alpha=\pi/4$.

\item\label{exer:4.5.34}Use the result of Exercise~\ref{exer:4.5.32} to find a family of curves
