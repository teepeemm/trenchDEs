...
\index{applications!of first order equations}
...
\index{exponential growth and decay}
\index{applications!of first order equations!growth and decay}
%\index{growth and decay!exponential}

One of the most common mathematical models for a physical process is
the \indexterm{exponential model}, where it's assumed that the rate
...
a
quantity $Q$ that satisfies this equation \emph{\color{blue}grows exponentially}\index{exponential growth} if
$a > 0$, or \emph{\color{blue}decays exponentially}\index{exponential decay} if $a < 0$
 (\zcref{figure:4.1.1}).
...
\index{exponential decay!radioactive decay}
...
$-k$, where
$k$ is a positive number that we'll call the \indexterm{decay constant}\index{constant!decay}
...
The \indexterm{half-life} $\tau$ of a radioactive material is defined to be
...
\index{compound interest!continuous}
\index{exponential growth!continuous compound interest}
\index{interest compounded continuously}
...
is \emph{\color{blue}compounded}\index{compound interest}. If the interest is compounded annually,  the
...
step in this direction is to \indexterm{compound continuously}\index{continuous compounding}, by which we
...
$5\frac{1}{2}$\% annual
interest compounded continuously,\index{compound interest!continuous} the value of the account after
$t$ years is
\[
Q(t)=150e^{.055t}
\]
...
\index{exponential growth and decay!mixed}
\index{mixed exponential growth and decay}
...
We say that $a/k$ is the \indexterm{steady state} value of $Q$. From
...
\boxit{Carbon Dating}\index{exponential decay!carbon dating}\index{carbon dating}

The fact that $Q$ approaches a steady state value in the
situation discussed in \zcref{example:4.1.4} underlies the method of 
\indexterm{carbon dating}, devised by  the American chemist and Nobel Prize Winner
...
\index{savings program}
\index{exponential growth!savings program}
...
predictions based on them with the actual outcome of experiments.\index{mathematical model!validity of}
\zcref[S]{example:4.1.6} is unusual in that we can compute the exact value of the
...
(This is called \indexterm{accelerated payment}.)
\begin{alist}
\item % (a)
...
\index{cooling and mixing}
\index{mixing and cooling}
\index{applications!of first order equations!cooling and mixing}

\boxit{Newton's Law of Cooling}\index{Newton's!law of cooling}
...
We'll call $k$  the \indexterm{temperature decay constant}\index{constant!temperature decay} of the medium.
...
phenomenon.\index{mathematical model!validity of}
Like most mathematical models it has its limitations. For
...
\indexterm{mixing problem}. To construct a tractable mathematical model for
...
solution drains out at the same rate.  Find the quantity $Q(t)$
...
\index{mathematical model!validity of}
...
\index{elementary mechanics}
\index{applications!of first order equations!elementary mechanics}
\index{elementary mechanics!Newton's second law of motion}

\boxit{Newton's Second Law of Motion}\index{Newton's!second law of motion}
...
Newton's second law of motion\index{Newton's!second law of motion}
...
\begin{table}
\centering
\caption{Units of measurement in various systems%
\index{cgs units}\index{mks units}\index{British units}}\index{elementary mechanics!units used}
\label{table:units}
\tagpdfsetup{table/header-rows={1}}
...
density,  Newton's law of
gravitation\index{Newton's!law of gravitation}\index{gravity!Newton's law of}
(discussed later in this section)
% WHERE?
asserts that the force exerted on an object by Earth's
...
Earth's surface, we may assume that the gravitational force\index{force!gravity} is
constant and equal to its value at the surface. The magnitude of this
force is $mg$, where $g$ is called the \indexterm{acceleration due to
gravity}\index{gravity!acceleration}. (To be completely accurate, $g$ should be called the
...
Gravitational Force}
\index{motion!through resisting medium and gravity}
\index{elementary mechanics!motion through resisting medium and gravity}
...
We call this limit the \indexterm{terminal velocity}\index{velocity!terminal}.
...
\caption{Solutions of \eqref{eq:4.3.15} for various $v_0>0$}
  \label{figure:4.3.4}
\end{figure}


\boxit{Escape Velocity}\index{escape velocity}\index{velocity!escape}
\index{elementary mechanics!escape velocity}
...
\indexterm{escape velocity}, with these properties:
...
\item\label{exer:4.3.3}A boat weighs \qty{64000}{\pound}. Its propeller produces a constant thrust of
...
is the \indexterm{bifurcation value} of $a$.) Try to find $a_0$ and
...
miles above Earth.  Determine its escape velocity\index{escape velocity|(}\index{elementary mechanics!escape velocity} in
...
\item\label{exer:4.3.20}In the situation considered in the discussion of escape velocity\index{escape velocity|)}, show
...
\index{autonomous second order equation}
\index{autonomous second order equation!conversion to first order}
\index{second order equation!autonomous}
\index{second order equation!autonomous!conversion to first order}
\index{first order equation!autonomous second order equation converted to}
\index{applications!of first order equations!autonomous second order equations}

A second order differential equation that can be written as
\begin{equation} \label{eq:4.4.1}
y''=F(y,y')
\end{equation}
where $F$ is independent of $t$, is said to be \emph{\color{blue}autonomous}\index{autonomous equation}.
...
\personHref{Poincare}{Poincaré phase plane}\index{Poincaré phase plane}\index{Poincaré, Henri} of \eqref{eq:4.4.1}. If $y$ is a solution of
\eqref{eq:4.4.1}
then $y=y(t), v=y'(t)$ is a parametric equation for an integral curve
of \eqref{eq:4.4.4}. We'll call these integral curves
\emph{\color{blue}trajectories}\index{trajectory!autonomous second order equation}
of \eqref{eq:4.4.1}, and we'll call
\eqref{eq:4.4.4} the \indexterm{phase plane equivalent} of \eqref{eq:4.4.1}.
...
law of
motion.\index{autonomous second order equation!Newton's second law of motion}\index{Newton's!second law of motion!autonomous second order equations}\index{second order equation!autonomous!Newton's second law of motion}
For example, suppose  $y$ is the displacement of a
...
are called \emph{\color{blue}damping}\index{damping!force}\index{force!damping} forces.) We  write this force as
...
law of
motion\index{Newton's!second law of motion}\index{second order equation!autonomous!Newton's second law of motion}
leads to \eqref{eq:4.4.5}.
...
that $\overline y$ is an \indexterm{equilibrium} of \eqref{eq:4.4.5} and
 $(\overline y,0)$ is a \indexterm{critical point} of the phase plane
equivalent equation \eqref{eq:4.4.6}. We say that the equilibrium and
the critical point are \emph{\color{blue}stable}\index{stable equilibrium} if, for any given $\epsilon>0$
...
say they are \emph{\color{blue}unstable}\index{unstable equilibrium}. To see if you really understand what
...
\index{undamped second order equation!autonomous}
\index{autonomous second order equation!undamped}
\index{second order equation!autonomous!undamped}
...
that it may model --- is \emph{\color{blue}undamped}\index{undamped model}.
...
total
energy of the object remains constant, or is \emph{\color{blue}conserved}\index{conservation of energy}. In
...
\index{autonomous second order equation!undamped!spring-mass system}
\index{undamped autonomous second order equation!spring-mass system}
\index{elementary mechanics!spring-mass system!undamped}
\index{spring-mass system!undamped}
...
Newton's second law of motion\index{Newton's!second law of motion} implies that
...
as \indexterm{simple harmonic motion}.)
\end{example}
...
\index{autonomous second order equation!undamped!pendulum}
\index{undamped autonomous second order equation!pendulum}
\index{elementary mechanics!pendulum!undamped}
\index{pendulum!undamped}
...
\begin{figure}
  \centering
  \includegraphics[alt={The y v plane with several solution curves that are symmetric about both axes.  Dashed separatrices connect (-π,0) to (π,0) by going through (0,±1).  Inside the separatrices, several ellipses are centered at the origin.  Outside the separatrices, solution curves move horizontally through the plane.},
  width=2.83in,height=1.83in,keepaspectratio]{fig040407}
\caption{Trajectories of the undamped pendulum}\index{separatrix}
  \label{figure:4.4.7}
\end{figure}
...
\index{stability condition!for second order undamped autonomous equation}
\index{instability condition!for second order undamped autonomous equation}
\index{second order undamped autonomous equation!stability condition}
\index{autonomous second order equation!stability condition}

It can be shown (\zcref{exer:4.4.23}) that an equilibrium
$\overline y$ of an undamped
equation\index{undamped autonomous second order equation!stability condition}
...
that contain different kinds of trajectories; again,
 we call this curve a \indexterm{separatrix}.
...
\index{damped!second order equation!autonomous}
\index{autonomous second order equation!damped}
\index{second order equation!autonomous!damped}
...
$c$ is called the
\emph{\color{blue}damping constant}\index{damping!constant}\index{constant!damping}.
In situations where
...
\index{damped spring-mass system}
\index{spring-mass system!damped}
\index{autonomous second order equation!damped!spring-mass system}
\index{second order equation!autonomous!damped}
\index{damped!autonomous second order equation!spring-mass system}
\index{elementary mechanics!spring-mass system!damped}
...
second law of motion\index{Newton's!second law of motion} implies that
\begin{equation} \label{eq:4.4.27}
my''+cy'+ky=0,
\end{equation}
where $c>0$ is the \indexterm{damping constant}\index{constant!damping}. Again, this
equation can
...
be similar to the dashed elliptic trajectory shown in
\zcref{figure:4.4.14}.
...
The corresponding solutions are said to be \emph{\color{blue}oscillatory}\index{oscillatory solution}
...
\index{autonomous second order equation!damped!pendulum}
\index{damped!autonomous second order equation!pendulum}
\index{elementary mechanics!pendulum!damped}
...
be called the \emph{\color{blue}critical value}\index{critical value!of trajectory} of $a$?}
...
The \personHref{Van_der_Pol}{van der Pol equation}\index{van der Pol equation}
\[
...
\indexterm{limit cycle}. Trajectories inside the limit cycle spiral
...
\personHref{Rayleigh}{Rayleigh's equation}\index{Rayleigh's equation}\index{Rayleigh, Lord},
\[
y''-\mu(1-(y')^2/3)y'+y=0
\]
...
\index{one-parameter family of curves}
\index{family of curves!one-parameter}
\index{applications!of first order equations!curves}
\index{curves!one-parameter family}
...
H(x,y,c)=0
\end{equation}
is said to define a \indexterm{one-parameter family of curves}\index{curves!one-parameter family!defined}\index{one-parameter family of curves!defined} if, for
...
differential equation is called an \indexterm{integral curve} of the
...
\index{curves!one-parameter family!differential equation for}
\index{one-parameter family of curves!differential equation for}
...
for the family}\index{differential equation!for family of curves}.
\end{definition}
...
\begin{example}\label{example:4.5.6}\index{tangent lines}
\begin{alist}
\item %(a)
...
tangent line is regarded as consisting of two \indexterm{tangent half
...
\index{geometric problems}\index{curves!geometric problems}
...
\index{orthogonal trajectories}\index{trajectories!orthogonal}
\index{curves!orthogonal trajectories}
...
\indexterm{orthogonal trajectory} of a given family of curves if it's
...
u(x,y)=c
\end{equation}
are called \indexterm{isothermal curves}\index{isothermals}\index{curves!isothermal}. The orthogonal trajectories of
this family are called \indexterm{heat-flow lines}, because at any given
...
curves \eqref{eq:4.5.16} are called \indexterm{equipotentials}\index{equipotential curves}\index{curves!equipotential}, and the
orthogonal trajectories are called \indexterm{lines of force}\index{force!lines of}.
...
\index{curves!orthogonal trajectories!finding}
...
\index{trajectories!orthogonal!finding}
...
$\alpha$ is the \emph{\color{blue}angle from $L$ to $L_1$}\index{angle between curves};   thus,
