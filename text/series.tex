...
\chapter{Series Solutions of Linear Second Order Equations}\label{ch:series}
...
\item
 Bessel's equation\index{Bessel's equation}
\[
x^2y''+xy'+(x^2-\nu^2)y=0,
\]
...
\item
\personHref{Airy}{Airy's equation},
\[
y''-xy=0,
\]
...
and $P_2$
in the case where $P_0(0)=0$, and deals with \emph{\color{blue}Euler's equation}\index{Euler equation}
\[
ax^2y''+bxy'+cy=0,
\]
...
called a
\indexterm{power series}\index{power series!defined}\index{series!power} in $x-x_0$. We say that the power series
\eqref{eq:7.1.1} \emph{\color{blue}converges}\index{convergent power series}\index{power series!convergent} for a given $x$ if the limit
\[
\lim_{N\to\infty}
\sum_{n=0}^Na_n(x-x_0)^n
\]
exists; otherwise, we say that the power series \emph{\color{blue}diverges}\index{divergent power series}\index{power series!divergent}
for the given $x$.
\end{definition}
...
In case~(iii) we say that $R$ is the
\emph{\color{blue}radius of convergence of power series}\index{convergence!of power series!radius of}\index{power series!radius of convergence}\index{radius of convergence}
of the power series. For convenience, we include the other two cases
in this definition by defining $R=0$ in case~(i) and $R=\infty$ in
case~(ii). We define the \indexterm{open interval of convergence}\index{convergence!open interval of} of
...
$\sum_{n=0}^\infty\alpha_n$ is said to \indexterm{converge absolutely}\index{convergence!absolute}\index{absolute convergence} if
...
\]
where $0\le L\le\infty$. Then the radius of
convergence\index{convergence!of power series!radius of}\index{power series!radius of convergence}\index{radius of convergence}
of $\sum_{n=0}^\infty a_n(x-x_0)^n$ is $R=1/ L$, which should be interpreted
...
\boxit{Taylor Series}\index{Taylor series}\index{series!Taylor}

If a function $f$ has derivatives of all orders at a point $x=x_0$,
then the
\personHref{Taylor}{Taylor series of  $f$  about} $x_0$  is
defined by
\[
 \sum_{n=0}^\infty\frac{f^{(n)}(x_0)}{n!}(x-x_0)^n.
\]
In the special case where $x_0=0$, this series is also called the
\personHref{Maclaurin}{Maclaurin series of}\index{Maclaurin series}\index{series!Maclaurin} $f$.
...
\index{differentiation of power series}
...
\index{uniqueness of power series}
\index{power series!uniqueness of}
...
series is the Taylor
series\index{Taylor series}\index{series!Taylor}\index{power series!Taylor series}
of $f$ about $x_0$.
...
\index{Taylor polynomial}\index{polynomial!Taylor}
\index{power series!Taylor polynomial}
...
\]
is the
\personHref{Taylor}{$N$-th Taylor polynomial of $f$ about $x_0$\/}.
...
\index{shifting summation index}
\index{summation index!shifting}
\index{power series!shifting summation index}
...
\end{equation}
where we start the new summation index\index{summation index} $k$ from zero so
that the first
...
\begin{center}
\bfseries Shifting the Summation Index in a Power Series
\end{center}
...
\index{linear combinations of power series}
\index{power series!linear combinations of}
\index{linear second order equation!series solution}
\index{series solution!linear second order equation}
...
value of $M$ for which there's no perceptible difference between the
two graphs.
 Choose the scale on the $y$-axis so that~$-75\le y\le75$.
\end{exerciselist}
%
\textsl{In \zcref[range]{exer:7.1.11,exer:7.1.15}
find a power series solution
$y(x)=\sum_{n=0}^\infty a_nx^n$.}
\begin{exerciselistTwoColumn}
\item\label{exer:7.1.11}$(2+x)y''+xy'+3y$
\item\label{exer:7.1.12}$(1+3x^2)y''+3x^2y'-2y$
...
\index{series solutions near ordinary point}
\index{ordinary point!series solutions near}
\index{linear second order equation!series solution!near ordinary point}
\index{series solution!linear second order equation!near ordinary point}
...
factors. Then we say that $x_0$ is an \indexterm{ordinary point} of
\eqref{eq:7.2.1} if $P_0(x_0)\ne0$, or a \indexterm{singular point} if
$P_0(x_0)=0$. For Legendre's equation\index{Legendre's equation},
\begin{equation}\label{eq:7.2.2}
(1-x^2)y''-2xy'+\alpha(\alpha+1)y=0,
\end{equation}
$x_0=1$ and $x_0=-1$ are singular points\index{Legendre's equation!singular points} and all other points are
ordinary points\index{Legendre's equation!ordinary points}. For Bessel's equation\index{Bessel's equation},
\[
x^2y''+xy'+(x^2-\nu^2)y=0,
\]
$x_0=0$ is a singular point\index{Bessel's equation!singular point} and all other points are ordinary points\index{Bessel's equation!ordinary point}.
If $P_0$ is a nonzero constant as in Airy's
equation,\index{Airy's equation}
\begin{equation}\label{eq:7.2.3}
y''-xy=0,
\end{equation}
then every point is an ordinary point\index{Airy's equation!ordinary point}.
...
We call \eqref{eq:7.2.6} a \emph{\color{blue}power series solution in $x-x_0$} \index{power series!solution of differential equation}\index{differential equation!power series solution} of
...
way \emph{\color{blue}the power series in $x-x_0$ for the general solution}\index{power series!general solution solution of differential equation}\index{differential equation!power series general solution solution} of
...
or more coefficients with lesser indices are called \indexterm{recurrence
relations}. When we use a recurrence relation to compute terms
of a sequence we're computing
\emph{\color{blue}recursively}\index{recursive sequence}\index{coefficients!computed recursively}\index{recurrence relation}.
...
\personHref{Chebyshev}{Chebyshev's equation}\index{Chebyshev's equation},
%\index{Chebyshev polynomials}
\[
(1-x^2)y''-xy'+\alpha^2 y=0,
\]
and
\personHref{Hermite}{Hermite's equation}\index{Hermite's equation},
\[
y''-2xy'+2\alpha y=0.
\]
Since
...
\textsl{In \zcref[range]{exer:7.2.16,exer:7.2.20}
 find the power series in $x-x_0$
for the general solution.}

\begin{exerciselistTwoColumn}
\item\label{exer:7.2.16}$y''-y=0;\quad   x_0=3$
\item\label{exer:7.2.17}$y''-(x-3)y'-y=0;\quad   x_0=3$
\item(2)\label{exer:7.2.18}$(1-4x+2x^2)y''+10(x-1)y'+6y=0;\quad  x_0=1$
\item(2)\label{exer:7.2.19}$(11-8x+2x^2)y''-16(x-2)y'+36y=0;\quad  x_0=2$
\item(2)\label{exer:7.2.20}$(5+6x+3x^2)y''+9(x+1)y'+3y=0;\quad  x_0=-1$
\end{exerciselistTwoColumn}
%
\textsl{In \zcref[range]{exer:7.2.21,exer:7.2.26}
 find $a_0$, \dots, $a_N$ for $N$ at least $7$ in the power series
$y=\sum_{n=0}^\infty a_n(x-x_0)^n$
...
\item\label{exer:7.2.27}\begin{alist}
\item % (a)
...
\item\label{exer:7.2.30}\begin{alist}
\item % (a)
...
$(-1,1)$ \emph{\color{blue}with respect to the weighting function}
$(1-x^2)^{b-1}$.)\index{orthogonal functions!with respect to a weighting function}
\end{alist}


\item\label{exer:7.2.31}\begin{alist}
\item % (a)
Use \zcref{exer:7.2.28}  to show that the power series in $x$
for the general solution
...
$e^{-x^2}$.)\index{orthogonal functions with respect to a weighting function}
\end{alist}
...
\end{exerciselist}
%
\textsl{In \zcref[range]{exer:7.2.33,exer:7.2.37}  use the method of
\zcref{exer:7.2.32}  to find the power series in
$x$ for the general solution.}

\begin{exerciselistTwoColumn}
\item\label{exer:7.2.33}$y''-xy=0$
\item\label{exer:7.2.34}$(1-2x^3)y''-10x^2y'-8xy=0$
...
\end{exerciselist}
%
\textsl{In \zcref[range]{exer:7.2.39,exer:7.2.44}  use the method of
...
\item\label{exer:7.3.30}\begin{alist}
\item % (a)
...
\emph{\color{blue}second order homogeneous linear difference equation}\index{difference equation}\index{linear difference equation!second order homogeneous}\index{second order homogeneous linear difference equation}.
The polynomial $p(r)=r^2+\alpha r+\beta$ is called the
\indexterm{characteristic polynomial}\index{polynomial!characteristic} of (B).
If $r_1$
...
\index{regular singular point!Euler equation}
\index{Euler equation!regular singular point}
\index{linear second order equation!series solution!Euler's equation}
\index{linear second order equation!regular singular point}
...
suppose  $P_0(x_0)=0$. Then $x_0$ is a \indexterm{regular singular
point}\index{singular point!regular} of the equation
...
$x_0$ is an \indexterm{irregular singular point}\index{singular point!irregular} of \eqref{eq:7.4.2}.
\end{definition}
...
it follows that $x_0=0$ is a regular singular point\index{Bessel's equation!singular point of} of
\eqref{eq:7.4.4}.
 \end{example}
...
equation\index{Legendre's equation}, and in \zcref{exer:7.4.23} for the general case.
...
An
\personHref{Euler}{Euler equation}\index{Euler equation}
...
\begin{aligned}
ax^2(x^r)''+bx(x^r)'+cx^r&=ax^2r(r-1)x^{r-2}+bx rx^{r-1}+cx^r\\
&=[ar(r-1)+br+c]x^r.
\end{aligned}
\end{equation}
The polynomial
...
p(r)=ar(r-1)+br+c
\]
is called the \indexterm{indicial polynomial}\index{polynomial!indicial} of \eqref{eq:7.4.6}, and
$p(r)=0$
is its \indexterm{indicial equation}. From \eqref{eq:7.4.7} we can
see that
...
\item\label{exer:7.4.2}$x^2y''-7xy'+7y=0$
...
\item\label{exer:7.4.7}$x^2y''+3xy'-3y=0$%&
\item\label{exer:7.4.8}$12x^2y''-5xy''+6y=0$
\item\label{exer:7.4.9}$4x^2y''+8xy'+y=0$%&
\item\label{exer:7.4.10}$3x^2y''-xy'+y=0$
\item\label{exer:7.4.11}$2x^2y''-3xy'+2y=0$%&
\item\label{exer:7.4.12}$x^2y''+3xy'+5y=0$
\item\label{exer:7.4.13}$9x^2y''+15xy'+y=0$%&
\item\label{exer:7.4.14}$x^2y''-xy'+10y=0$
\item\label{exer:7.4.15}$x^2y''-6y=0$%&
\item\label{exer:7.4.16}$2x^2y''+3xy'-y=0$
\item\label{exer:7.4.17}$x^2y''-3xy'+4y=0$%&
\item\label{exer:7.4.18}$2x^2y''+10xy'+9y=0$
\end{exerciselistTwoColumn}

\begin{exerciselist}
\item\label{exer:7.4.19}\begin{alist}
\item % (a)
...
\]
is said to be
\emph{\color{blue}oscillatory}\index{oscillatory solution}\index{solution!oscillatory}
on an
interval $(a,b)$ if it has infinitely many zeros on $(a,b)$.
Otherwise $y$ is said to be
\emph{\color{blue}nonoscillatory}\index{nonoscillatory solution}\index{solution!nonoscillatory}
on $(a,b)$.
...
\index{Frobenius!method of}
\index{Frobenius solutions}
\index{linear second order equation!series solution!Frobenius solution}
\index{series solution!linear second order equation!Frobenius solution}
...
singular point at $x_0=0$\index{regular singular point!at $x_0=0$},
...
\personHref{Frobenius}{the method of Frobenius},
and we'll call them Frobenius solutions.

It can be shown that the power series\index{Frobenius solutions!power series}
$\sum_{n=0}^\infty a_nx^n$ in a
...
\begin{align*}
\alpha x^2y''+\beta xy'+\gamma y&=0
\quad\text{(Euler equation\index{Euler equation})},\\
x^2y''+xy'+(x^2-\nu^2)y&=0
\quad\text{(Bessel's equation\index{Bessel's equation})},\\
\shortintertext{and}
xy''+(1-x)y'+\lambda y&=0
\quad\text{(\personHref{Laguerre}{Laguerre's equation}\index{Laguerre's equation})},
\end{align*}
...
\begin{proof}
If $\{a_n(r)\}$ is determined by the recurrence
relation\index{Frobenius solutions!recurrence relation}\index{recurrence relation!Frobenius solutions}
...
of $Ly=0$, we'll also say that $p_0$ is the \indexterm{indicial
polynomial}\index{polynomial!indicial} of \eqref{eq:7.5.2}, and that $p_0(r)=0$ is the 
\indexterm{indicial equation} of $Ly=0$. We'll consider only cases where the
...
Now suppose $r_1-r_2$ isn't an integer\index{Frobenius solutions!indicial equation with real roots differing by a noninteger}\index{indicial equation!with real roots differing by a noninteger}. From \eqref{eq:7.5.15},
\[
...
\end{equation}
form a fundamental set of Frobenius
solutions\index{coefficients!Frobenius solutions}
of \eqref{eq:7.5.16}.
...
\index{method of Frobenius!two term recurrence relation}
\index{recurrence relation!two term!method of Frobenius}
\index{Frobenius solutions!recurrence relation!two term}
\index{recurrence relation!two term}
...
possible to obtain explicit formulas for the coefficients of Frobenius
solutions.
...
find the coefficients in these series, we use the recurrence relations
...
closed form for the coefficients is essentially irrelevant;   recursive
...
\index{Frobenius solutions!verifying}
...
\]
is
\personHref{Bessel}{Bessel's equation of order~$\nu$}\index{Bessel's equation!of order $\nu$}\index{Bessel functions of order $\nu$}.
(Here $\nu$ is a parameter, and this use of ``order'' should not be
confused with its usual use as in ``the order of the equation.'') The
solutions of (A) are Bessel functions of order $\nu$.
\begin{alist}
\item % (a)
Assuming that $\nu$ isn't
...
\item\label{exer:7.5.54}\begin{alist}
\item % (a)
Verify that
\[
...
\item\label{exer:7.5.55}\begin{alist}
\item % (a)
...
\item\label{exer:7.5.58}\begin{alist}
\item % (a)
...
\item\label{exer:7.5.60}\begin{alist}
\item % (a)
...
\index{method of Frobenius}
\index{Frobenius!method of}
\index{Frobenius solutions!indicial equation with repeated root}
\index{indicial equation!with repeated root}
...
\]
is
\personHref{Bessel}{Bessel's equation of order zero}\index{Bessel's equation!of order zero}. (See
...
\index{method of Frobenius}
\index{Frobenius!method of}
\index{Frobenius solutions!indicial equation with real roots differing by an integer}
\index{indicial equation!with real roots differing by an integer}
...
\item\label{exer:7.7.41}\begin{alist}
\item % (a)
