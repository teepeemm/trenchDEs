...
\[
a_{n+2}=-\frac{\gamma+\alpha n}{n+2}\,a_{n+1}-{\beta n(n-1)+\delta
...
\begin{eqnarray*}
\frac{d^2Y}{dt^2}&=&{d\phantom{t}\over dt}\left(dY\over
dt\right)={d\phantom{t}\over dt}\left(x\frac{dy}{dx}\right)\\[2\jot]
...
so
\[
a_{2m}(1/2)=\frac{1}{8^mm!}\prod_{j=1}^m{(4j-1)(4j+1)\over4j-3}.
\]
Therefore
\[
y_2=x^{1/2}\sum_{m=0}^\infty
\frac{1}{8^mm!}\left(\prod_{j=1}^m{(4j-1)(4j+1)\over4j-3}\right)x^{2m}
\]
...
xq_1(x)\frac{\partial^2y}{\partial r\partial x}(x,r)+q_2(x){\partial
...
+xq_1(x)\frac{\partial^2 y}{\partial x\partial r}(x,r)+q_2(x){\partial
y\over \partial r}(x,r)=p'_0(r)x^r+p_0(r) x^r \ln x,
\]
or
\[
...
\[
{a'_n(r)\over a_n(r)}=\sum^n_{j=1} \left(\frac{1}{j+r}-\frac{2}{2j+2r-1}\right).
\]
Therefore
\[
...
a'_n(1/2)={(-1)^n\prod_{j=1}^n(2j+1)\over4^nn!}\left(\sum_{j=1}^n\frac{1}{j+1/2}-\sum_{j=1}^n\frac{1}{j}\right).
\end{equation}
Since
\[
\frac{1}{j+1/2}-\frac{1}{j}={j-j-1/2\over j(j+1/2)}=-\frac{1}{j(2j+1)},
\]
...
\eqref{eq:7.6.24},
\[
|a_{2m}(r)|=\frac{1}{2^m}\prod_{j=1}^m{|2j+r|\over|2j+r-1|}.
\]
...
Differentiating with respect to $r$ yields
\[
{a'_{2m}(r)\over a_{2m}(r)}=\sum^m_{j=1} \left(\frac{1}{2j+r}-\frac{1}{2j+r-1}\right).
\]
Therefore
\[
...
\[
y_2=y_1\ln
x-\frac{x}{2}\sum_{m=1}^\infty{\prod_{j=1}^m(2j+1)\over4^mm!}
...
Differentiating  with respect to $r$ yields
\[
{a'_n(r)\over a_n(r)}=\sum^n_{j=1} \left(\frac{1}{j+r-5}-\frac{2}{j+r-3}\right).
\]
Therefore
...
where
\[
J_n=\sum_{j=1}^n{p_1'(j+r_1-1)\over p_1(j+r_1-1)}-2\sum_{j=1}^n\frac{1}{j}.
\]
\item % (c)
...
Differentiating  with respect to $r$ yields
\[
{a'_n(r)\over a_n(r)}=\sum^n_{j=1} \left(\frac{1}{j+r}-\frac{2}{2j+2r-5}\right).
\]
Therefore
\[
...
Since
\[
\prod_{j=1}^n\frac{j+r+2}{j+r+1}={(r+3)(r+4)\cdots(n+r+2)\over
(r+2)(r+3)\cdots(n+r+1)}=\frac{n+r+2}{r+2}
\]
...
Differentiating  with respect to $r$ yields
\[
{a'_{2m}(r)\over a_{2m}(r)}=\sum^m_{j=1} \left(\frac{1}{2j+r}-\frac{1}{2j+r-3}\right).
\]
Therefore
\[
...
\[
a_{2m}'(3)=-\frac{3}{2}{(-1)^n\prod_{j=1}^m(2j+3)\over2^mm!}
...
&&\, +\frac{9}{32}
x^3\sum_{m=1}^\infty
{(-1)^m\prod_{j=1}^m(2j+3)\over2^mm!}\left(\sum_{j=1}^m\frac{1}{j(2j+3)}\right) x^{2m}.
\end{eqnarray*}
