...
In case~(iii) we say that $R$ is the \indexterm{radius of convergence}\index{convergence!radius of} of
...
\begin{align*}
\alpha x^2y''+\beta xy'+\gamma y&=0 \quad\text{(Euler's equation\index{Euler's equation})},\\
x^2y''+xy'+(x^2-\nu^2)y&=0 \quad\text{(Bessel's equation\index{Bessel's equation})},\\
\shortintertext{and}
xy''+(1-x)y'+\lambda y&=0,
