...
\sum_{n=10}^\infty (-1)^n \frac{x^n}{n!}\quad \mbox{\bf (c) }\quad
...
\begin{figure}[tbp]
  ...
  \includegraphics[bb=-78 148 689 643,width=5.67in,height=3.3in,keepaspectratio]{fig070101}
...
\end{equation}
Conversely, \zcref{thmtype:7.1.6}~\part{b} implies that if
$y=\sum_{n=0}^\infty a_nx^n$
...
\part{a} of \zcref{thmtype:7.1.6} imply that $Ly=0$ if and only if
...
the functions
\[
y_1=\frac{1}{1-r_1x}\quad\mbox{ and }\quad y_2=\frac{1}{1-r_2x}
\]
...
that the functions
\[
y_1=\frac{1}{1-r_1x}\quad\mbox{ and }\quad y_2=\frac{x}{(1-r_1x)^2}
\]
form a fundamental set of solutions of (A) on $(-\rho,\rho)$.
\item % (g)
Show that $\{y_1,y_2\}$ is
...
\[
y=\begin{cases}
c_1x^{r_1}+c_2x^{r_2}&\mbox{ if $r_1$ and $r_2$ are distinct real numbers;}\\
x^{r_1}(c_1+c_2\ln x)&\mbox{ if $r_1=r_2$;}\\
x^{\lambda}\left[c_1\cos\left(\omega\ln x\right)+
c_2\sin\left(\omega\ln x \right)\right]
&\mbox{ if $r_1,r_2=\lambda\pm i\omega$ with $\omega>0$}.
\end{cases}
\]
\end{theorem}
...
\[
Y(t)=\begin{cases}
c_1e^{r_1t}+c_2e^{r_2t}&\mbox{ if $r_1$ and $r_2$ are distinct real numbers;}\\
e^{r_1t}(c_1+c_2t)&\mbox{ if $r_1=r_2$;}\\
e^{\lambda t }\left(c_1\cos\omega t+c_2\sin\omega t \right)
&\mbox{ if $r_1,r_2=\lambda\pm i\omega$ with $\omega\ne0$}.
\end{cases}
\]
...
\[
y=\begin{cases}
c_1|x|^{r_1}+c_2|x|^{r_2}&\mbox{ if $r_1$ and $r_2$ are distinct
real numbers;} \\
|x|^{r_1}(c_1+c_2\ln|x|)&\mbox{ if $r_1=r_2$;}\\
|x|^{\lambda}\left[c_1\cos\left(\omega\ln|x|\right)+
c_2\sin\left(\omega\ln|x| \right)\right]
...
\zcref{thmtype:7.5.2}. Find $\rho$ and do experiments \part{a},
\part{b}, and
\part{c} of \zcref{exer:7.5.11} for each Frobenius solution, with
...
\part{a},
\part{b}, and \part{c} of \zcref{exer:7.5.11} for each Frobenius
solution,
...
p_1(r)=\alpha_1r(r-1)+\beta_1r+\gamma_1.
\]
\zcref[S]{thmtype:7.6.1}  and {Exercise~7.5.\ref{exer:7.5.55}}\part{a}
imply that if
\[
y(x,r)=x^r\sum_{n=0}^\infty a_n(r)x^n
\]
...
x^{2m}\\
&\quad{}-\frac{2}{4^kk!(k-1)!}\left(\frac{\gamma_2}{\alpha_0}\right)^k\left(y_1\ln x-
