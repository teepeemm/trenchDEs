...
This is the \emph{\color{blue}heat equation}.
...
\]
We call this the \emph{\color{blue}initial} condition.
 We must also specify \emph{\color{blue}boundary conditions}
that
$u$ must satisfy at the ends of the bar for all $t>0$.
We'll call this problem an
\emph{\color{blue}initial-boundary value problem}.
...
\emph{\color{blue}separation of variables} (not to be confused with method of separation of
...
we  call  a \emph{\color{blue}separation constant}, and write it
 as $-\lambda$;
 thus,
\[
...
The \emph{\color{blue}formal solution} of the initial-boundary value
problem
...
we say that it's an \emph{\color{blue}actual solution} of \eqref{eq:12.1.4}.
...
\emph{\color{blue}wave equation}. It is necessary to specify both $f$ and $g$
...
$u_1(x,t)=S_f(x+at)$ and $u_2(x,t)=S_f(x-at)$ are \emph{\color{blue}traveling
waves}.
...
it is also called \emph{\color{blue}the potential equation}.
We seek solutions of \eqref{eq:12.3.2}
in a region $R$ that satisfy specified conditions ---
called \emph{\color{blue}boundary conditions} --- on the boundary of $R$.
...
values.
This is called a
\personHref{Neumann_Carl}{Neumann condition},
...
the boundary conditions and the problem are \emph{\color{blue}mixed}.
