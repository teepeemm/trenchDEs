...
This is the \indexterm{heat equation}.
...
\]
We call this the \indexterm{initial condition}.
 We must also specify \indexterm{boundary conditions}
that
$u$ must satisfy at the ends of the bar for all $t>0$.
We'll call this problem an
\indexterm{initial-boundary value problem}.
...
\indexterm{separation of variables} (not to be confused with method of separation of
...
The \emph{\color{blue}formal solution}\index{formal solution!initial-boundary value problem} of the initial-boundary value
problem
...
we say that it's an \emph{\color{blue}actual solution}\index{actual solution!initial-boundary value problem} of \eqref{eq:12.1.4}.
...
\indexterm{wave equation}. It is necessary to specify both $f$ and $g$
...
wave equation under the following assumptions:\index{wave equation!assumptions}
...
As in \zcref{sec:heat_eq}, we use
 separation
...
\personHref{DAlembert}{d'Alembert's solution} of \eqref{eq:12.2.1}. Although
...
\index{Laplace's equation!rectangular coordinates}
...
\end{equation}
This is
\personHref{Laplace}{Laplace's equation}.  This equation also arises
...
called \indexterm{boundary conditions} --- on the boundary of $R$.
For example, we may require $u$ to assume prescribed values on the
boundary.
 This is called a
\personHref{Dirichlet}{Dirichlet condition},
and the problem is called a Dirichlet problem.
 Or, we may
 require the normal
...
and the problem is called a Neumann problem.
...
the boundary conditions and the problem are \emph{\color{blue}mixed}\index{mixed boundary conditions}.
...
0<r<\rho,\quad 0<\theta<\gamma,\\
 u(\rho,\theta)=f(\theta),\quad 0\le\theta\le\gamma,\\
