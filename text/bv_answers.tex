

\sectionanswers{11.1}

\answer{11.1.2}
$\lambda_n=n^2$, $y_n=\sin nx$, $n=1$, $2$, $3$, \dots


\answer{11.1.3}
$\lambda_0=0$, $y_0=1$;
$\lambda_n=n^2$, $y_n=\cos n x$, $n=1$, $2$, $3$, \dots


\answer{11.1.4}
$\lambda_n=\dst\frac{(2n-1)^2}{4}$,
$y_n=\dst\sin\frac{(2n-1) x}{2}$,
$n=1,2,3,\dots,$



\answer{11.1.5}
$\lambda_n=\dst\frac{(2n-1)^2}{4}$,
$y_n=\dst\cos\frac{(2n-1) x}{2}$,
$n=1$, $2$, $3$, \dots


\answer{11.1.6}
$\lambda_0=0$,  $y_0=1$,
$\lambda_n=n^2$, $y_{1n}=\cos nx$, $y_{2n}=\sin nx$,
$n=1$, $2$, $3$, \dots


\answer{11.1.7}
$\lambda_n=n^2\pi^2$, $y_n=\cos n\pi x$, $n=1$, $2$, $3$, \dots


\answer{11.1.8}
$\lambda_n=\dst\frac{(2n-1)^2\pi^2}{4}$,
$y_n=\dst\cos\frac{(2n-1)\pi x}{2}$,
$n=1$, $2$, $3$, \dots


\answer{11.1.9}
$\lambda_n=n^2\pi^2$, $y_n=\sin n\pi x$, $n=1$, $2$, $3$, \dots



\answer{11.1.10}
$\lambda_0=0$,  $y_0=1$,
$\lambda_n=n^2\pi^2$, $y_{1n}=\cos n\pi x$, $y_{2n}=\sin n\pi x$,
$n=1$, $2$, $3$, \dots


\answer{11.1.11}
$\lambda_n=\dst\frac{(2n-1)^2\pi^2}{4}$,
$y_n=\dst\sin\frac{(2n-1)\pi x}{2}$,
$n=1$, $2$, $3$, \dots


\answer{11.1.12}
$\lambda_0=0$,  $y_0=1$,
$\lambda_n=\dst\frac{n^2\pi^2}{4}$, $y_{1n}=\dst\cos\frac{n\pi x}{2}$,
$y_{2n}=\dst\sin{n\pi x\over2}$,
$n=1$, $2$, $3$, \dots





\answer{11.1.13}
$\lambda_n=\dst{n^2\pi^2\over4}$, $y_n=\dst\sin{n\pi x\over2}$,
$n=1$, $2$, $3$, \dots



\answer{11.1.14}
$\lambda_n=\dst{(2n-1)^2\pi^2\over36}$,
$y_n=\dst\cos{(2n-1)\pi x\over6}$,
$n=1$, $2$, $3$, \dots


\answer{11.1.15}
$\lambda_n=(2n-1)^2\pi^2$,
$y_n=\sin(2n-1)\pi x$,
$n=1$, $2$, $3$, \dots



\answer{11.1.16}
$\lambda_n=\dst\frac{n^2\pi^2}{25}$, $y_n=\dst\cos{n\pi x\over5}$,
$n=1$, $2$, $3$, \dots

\answer{11.1.23}
$\lambda_n=4n^2\pi^2/L^2$   $y_n=\dst\sin\frac{2n\pi x}{L}$,
$n=1$, $2$, $3$, \dots


\answer{11.1.24}
$\lambda_n=n^2\pi^2/L^2$    $y_n=\dst\cos\frac{ n\pi x}{L}$,
$n=1$, $2$, $3$, \dots

\answer{11.1.25}
$\lambda_n=4n^2\pi^2/L^2$    $y_n=\dst\sin\frac{2n\pi x}{L}$,
$n=1$, $2$, $3$, \dots

\answer{11.1.26}
$\lambda_n=n^2\pi^2/L^2$    $y_n=\dst\cos {n\pi
x\over L}$, $n=1$, $2$, $3$, \dots.


\sectionanswers{11.2}

\answer{11.2.2}
$F(x)=\dst2+\frac{2}{\pi}\sum_{n=1}^\infty{(-1)^n\over
n}\sin n\pi x$;  \quad
$F(x)=
\left\{\begin{array}{cl}
2,&\phantom{-}x=-1,\\2-x,&-1<x<1,\\2,&\phantom{-}x=1
\end{array}\right.$

\answer{11.2.3}
$F(x)=\dst-\pi^2-12\sum_{n=1}^\infty{(-1)^n\over n^2}\cos nx
-4\sum_{n=1}^\infty{(-1)^n\over n}\sin nx$;  \quad

\quad $F(x)=
\left\{\begin{array}{cl}
-3\pi^2,&\phantom{-}x=-\pi,\\2x-3x^2,&-\pi<x<\pi,\\
-3\pi^2,&\phantom{-}x=\pi
\end{array}\right.$

\answer{11.2.4}
$F(x)=-\dst\frac{12}{\pi^2}\sum_{n=1}^\infty(-1)^n\frac{\cos n\pi x}{n^2}$;  \quad $F(x)=1-3x^2$\quad$-1\le x\le 1$

\answer{11.2.5}
$F(x)=\dst\frac{2}{\pi}-\frac{4}{\pi}\sum_{n=1}^\infty{1\over4n^2-1}\cos2nx$;
\quad $F(x)=|\sin x|$, $-\pi\le x\le\pi$

\answer{11.2.6}
 $F(x)=\dst-{1\over2}\sin x+2\sum_{n=2}^\infty(-1)^n
\frac{n}{n^2-1}\sin nx$;;  \quad
$F(x)=x\cos x$, $-\pi\le x\le\pi$

\answer{11.2.7}
$F(x)=\dst-\frac{2}{\pi}+{\pi\over2}\cos x-\frac{4}{\pi}
\sum_{n=1}^\infty\frac{4n^2+1}{(4n^2-1)^2}\cos 2nx$;

\quad\,  $F(x)=|x|\cos x$, $-\pi\le x\le\pi$

\answer{11.2.8}
$F(x)=\dst1-{1\over2}\cos x-2\sum_{n=2}^\infty {(-1)^n\over n^2-1}\cos
nx$;  \quad $F(x)=x\sin x$, $-\pi\le x\le\pi$

\answer{11.2.9}
$F(x)=\dst{\pi\over2}\sin x-\frac{16}{\pi}
\sum_{n=1}^\infty\frac{n}{(4n^2-1)^2}\sin 2nx$;  \quad
 $F(x)=|x|\sin x$, $-\pi\le x\le\pi$

\answer{11.2.10}
$F(x)=\dst\frac{1}{\pi}+{1\over2}\cos\pi x-\frac{2}{\pi}\sum_{n=1}^\infty
{(-1)^n\over4n^2-1}\cos2n\pi x$;
\quad $F(x)=f(x)$, $-1\le x\le1$

\answer{11.2.11}
$F(x)=\dst{{1\over4\pi}\sin\pi x-\frac{8}{\pi^2}\sum_{n=1}^\infty
(-1)^n\frac{n}{(4n^2-1)^2}\sin2n\pi x}$;

\;  \qquad \qquad $\dst{-{1\over4\pi}\sum_{n=1}^\infty
 {(-1)^n\over n(n+1)}\sin(2n+1)\pi x}$   \quad
$F(x)=f(x)$, $-1\le x\le1$

\answer{11.2.12}
$F(x)=\dst{1\over2}\sin\pi x-\frac{4}{\pi}\sum_{n=1}^\infty
(-1)^n{n\over4n^2-1}\sin2n\pi x$;  \quad
$F(x)=
\left\{\begin{array}{cl}
\phantom{-}0,&-1\le x<{1\over2},\\
-{1\over2},&\phantom{-}x=-{1\over2},\\
\phantom{-}\sin\pi x,&-{1\over2}<x<{1\over2},\\
\phantom{-}{1\over2},&\phantom{-}x={1\over2},\\
\phantom{-}0,&\phantom{-}{1\over2}<x\le1\\
\end{array}\right.$

\answer{11.2.13}
$F(x)=\dst\frac{1}{\pi}+\frac{1}{\pi}\cos\pi x-\frac{2}{\pi}
\sum_{n=2}^\infty\frac{1}{n^2-1}\left(1-n\sin{n\pi\over2}\right)
\cos n\pi x$;

\quad \qquad $F(x)=
\left\{\begin{array}{cl}
0,&-1\le x<{1\over2},\\
{1\over2},&\phantom{-}x=-1,\\
|\sin\pi x|,&-{1\over2}<x<{1\over2},\\
{1\over2},&\phantom{-}x=1,\\
0,&\phantom{-}{1\over2}<x\le1
\end{array}\right.$

\answer{11.2.14}
$F(x)=\dst\frac{1}{\pi^2}+{1\over4\pi}\cos\pi
x+\frac{2}{\pi^2}\sum_{n=1}^\infty
(-1)^n\frac{4n^2+1}{(4n^2-1)^2}\cos2n\pi x$

\qquad\qquad\;    $\dst{+{1\over4\pi}\sum_{n=1}^\infty
(-1)^n\frac{2n+1}{n(n+1)}\cos(2n+1)\pi x}$;

\qquad\qquad $F(x)=
\left\{\begin{array}{cl}
0,&-1\le x<{1\over2},\\
{1\over4},&\phantom{-}x=-{1\over2},\\
x\sin\pi x,&-{1\over2}<x<{1\over2},\\
{1\over4},&\phantom{-}x={1\over2},\\
0,&\phantom{-}{1\over2}<x\le1,\\
\end{array}\right.$

\answer{11.2.15}
$\dst
F(x)=1-\frac{8}{\pi^2}\sum_{n=0}^\infty\frac{1}{(2n+1)^2}\cos{(2n+1)\pi
x\over4} -\frac{4}{\pi}\sum_{n=1}^\infty{(-1)^n\over n}\sin{n\pi
x\over4}$;

\qquad $F(x)=
\left\{\begin{array}{cl}
2,&\phantom{-}x=-4,\\
0,&-4<x<0,\\
x,&\phantom{-}0\le x<4,\\
2,&\phantom{-}x=4
\end{array}\right.$

\answer{11.2.16}
$F(x)=\dst{1\over2}+\frac{1}{\pi}\sum_{n=1}^\infty\frac{1}{n}\sin2n\pi x
+\frac{8}{\pi^3}\sum_{n=0}^\infty\frac{1}{(2n+1)^3}\sin(2n+1)\pi x$;

 \qquad $F(x)=
\left\{\begin{array}{cl}
{1\over2},&\phantom{-}x=-1,\\
x^2,&-1< x<0,\\
{1\over2},&\phantom{-}x=0,\\
1-x^2,&\phantom{-}0<x<1,\\
{1\over2},&\phantom{-}x=1
\end{array}\right.$

\answer{11.2.17}
$F(x)=\dst{3\over4}+\frac{1}{\pi}\sum_{n=1}^\infty
\frac{1}{n}\sin{n\pi\over2}\cos{n\pi x\over2}+\frac{3}{\pi}
\sum_{n=1}^\infty \frac{1}{n}\left(\cos
n\pi-\cos{n\pi\over2}\right)\sin{n\pi x\over2}$

\answer{11.2.18}
 $F(x)=\dst{5\over2}+\frac{3}{\pi}\sum_{n=1}^\infty
\frac{1}{n}\sin{2n\pi\over3}\cos{n\pi x\over3}+\frac{1}{\pi}
\sum_{n=1}^\infty \frac{1}{n}\left(\cos
n\pi-\cos{2n\pi\over3}\right)\sin{n\pi x\over3}$

\answer{11.2.20}
 $F(x)=\dst\frac{\sinh\pi}{\pi}\left(
1+2\sum_{n=1}^\infty{(-1)^n\over n^2+1}\cos
nx-2\sum_{n=1}^\infty{(-1)^nn\over n^2+1}\sin nx\right)$

\answer{11.2.21}
$F(x)=\dst-\pi\cos x-{1\over2}\sin x+2\sum_{n=2}^\infty
(-1)^n\frac{n}{n^2-1}\sin nx$

\answer{11.2.22}
$F(x)=\dst1-{1\over2}\cos x-\pi\sin x-2\sum_{n=2}^\infty
{(-1)^n\over n^2-1}\cos nx$

\answer{11.2.23}
$F(x)=-\dst\frac{2\sin k\pi}{\pi}\sum_{n=1}^\infty(-1)^n\frac{n}{n^2-k^2}\sin nx$

\answer{11.2.24}
$F(x)=\dst{\sin
k\pi\over\pi}\left[\frac{1}{k}-2k\sum_{n=1}^\infty{(-1)^n\over
n^2-k^2}\cos nx\right]$


\sectionanswers{11.3}

\answer{11.3.1}
$C(x)=\dst{L^2\over3}+\frac{4L^2}{\pi^2}\sum_{n=1}^\infty{(-1)^n\over
n^2}\cos\frac{n\pi x}{L}$

\answer{11.3.2}
$C(x)=\dst{1\over2}+\frac{4}{\pi^2}\sum_{n=1}^\infty\frac{1}{(2n-1)^2}
\cos(2n-1)\pi x$


\answer{11.3.3}
$C(x)=\dst-{2L^2\over3}+\frac{4L^2}{\pi^2}\sum_{n=1}^\infty\frac{1}{n^2}\cos\frac{n\pi x}{L}$

\answer{11.3.4}
$C(x)=\dst\frac{1-\cos k\pi}{k\pi}-\frac{2k}{\pi}\sum_{n=1}^\infty
{[1-(-1)^n\cos k\pi]\over n^2-k^2}\cos nx$.

\answer{11.3.5}
 $C(x)=\dst{1\over2}-\frac{2}{\pi}\sum_{n=1}^\infty
{(-1)^n\over2n-1}\cos{(2n-1)\pi x\over L}$

\answer{11.3.6}
$C(x)=\dst-{2L^2\over3}+\frac{4L^2}{\pi^2}\sum_{n=1}^\infty{(-1)^n\over
n^2}\cos\frac{n\pi x}{L}$


\answer{11.3.7}
$C(x)=\dst{1\over3}+\frac{4}{\pi^2}\sum_{n=1}^\infty\frac{1}{n^2}\cos
n\pi x$


\answer{11.3.8}
$C(x)=\dst\frac{e^\pi-1}{\pi}+\frac{2}{\pi}\sum_{n=1}^\infty
{[(-1)^ne^\pi-1]\over(n^2+1)}\cos nx$


\answer{11.3.9}
$\dst C(x)={L^2\over6}-\frac{L^2}{\pi^2}\sum_{n=1}^\infty\frac{1}{n^2}\cos\frac{2n\pi x}{L}$

\answer{11.3.10}
$C(x)=\dst-{2L^2\over3}+\frac{4L^2}{\pi^2}\sum_{n=1}^\infty\frac{1}{n^2}\cos\frac{n\pi x}{L}$

\answer{11.3.11}
$S(x)=\dst\frac{4}{\pi}\sum_{n=1}^\infty\frac{1}{(2n-1)}
\sin{(2n-1)\pi x\over L}$


\answer{11.3.12}
$S(x)=\dst\frac{2}{\pi}\sum_{n=1}^\infty\frac{1}{n}
\sin n\pi x$

\answer{11.3.13}
$S(x)=\dst\frac{2}{\pi}\sum_{n=1}^\infty
[1-(-1)^n\cos k\pi]\frac{n}{n^2-k^2}\sin nx$

\answer{11.3.14}
$S(x)=\dst\frac{2}{\pi}\sum_{n=1}^\infty\frac{1}{n}
\left[1-\cos{n\pi\over2}\right]\sin\frac{n\pi x}{L}$




\answer{11.3.15}
$S(x)=\dst\frac{4L}{\pi^2}\sum_{n=1}^\infty{(-1)^{n+1}\over(2n-1)^2}
\sin{(2n-1)\pi x\over L}$


\answer{11.3.16}
$S(x)=\dst{\pi\over2}\sin
x-\frac{16}{\pi}\sum_{n=1}^\infty\frac{n}{(4n^2-1)^2}\sin2nx$


\answer{11.3.17}
$S(x)=\dst-\frac{2}{\pi}\sum_{n=1}^\infty
{n[(-1)^ne^\pi-1]\over(n^2+1)}\sin nx$

\answer{11.3.18}
$C_M(x)=\dst-\frac{4}{\pi}\sum_{n=1}^\infty{(-1)^n\over2n-1}
\cos{(2n-1)\pi x\over2L}$

\answer{11.3.19}
$C_M(x)=\dst-\frac{4L^2}{\pi}\sum_{n=1}^\infty{(-1)^n\over2n-1}\left[
1-\frac{8}{(2n-1)^2\pi^2}\right]\cos{(2n-1)\pi x\over2L}$

\answer{11.3.20}
$C_M(x)=
-\dst\frac{4}{\pi}\sum_{n=1}^\infty\left[(-1)^n+\frac{2}{(2n-1)\pi}\right]
\cos{(2n-1)\pi x\over2}$.

\answer{11.3.21}
$C_M(x)=\dst-\frac{4}{\pi}\sum_{n=1}^\infty{1\over2n-1}\cos{(2n+1)\pi\over4}
\cos{(2n-1)\pi x\over 2L}$

\answer{11.3.22}
 $C_M(x)=
\dst\frac{4}{\pi}\sum_{n=1}^\infty(-1)^n\frac{2n-1}{(2n-3)(2n+1)}
\cos{(2n-1) x\over2}$

\answer{11.3.23}
$C_M(x)=
-\dst\frac{8}{\pi}\sum_{n=1}^\infty\frac{1}{(2n-3)(2n+1)}
\cos{(2n-1) x\over2}$


\answer{11.3.24}
$C_M(x)=-\dst\frac{8L^2}{\pi^2}\sum_{n=1}^\infty\frac{1}{(2n-1)^2}\left[
1+{4(-1)^n\over(2n-1)\pi}\right]\cos{(2n-1)\pi
x\over2L}$


\answer{11.3.25}
$S_M(x)=\dst\frac{4}{\pi}\sum_{n=1}^\infty\frac{1}{(2n-1)}
\sin{(2n-1)\pi x\over2L}$

\answer{11.3.26}
$S_M(x)=\dst-\frac{16L^2}{\pi^2}\sum_{n=1}^\infty\frac{1}{(2n-1)^2}\left[
(-1)^n+\frac{2}{(2n-1)\pi}\right]\sin{(2n-1)\pi x\over2L}$

\answer{11.3.27}
$S_M(x)=\dst\frac{4}{\pi}\sum_{n=1}^\infty{1\over2n-1}
\left[1-\cos{(2n-1)\pi)\over4}\right]
\sin{(2n-1)\pi x\over 2L}$

\answer{11.3.28}
$S_M(x)=\dst\frac{4}{\pi}\sum_{n=1}^\infty\frac{2n-1}{(2n-3)(2n+1)}\sin{(2n-1)
x\over2}$


\answer{11.3.29}
$S_M(x)=\dst\frac{8}{\pi}\sum_{n=1}^\infty{(-1)^n\over(2n-3)(2n+1)}\sin{(2n-1)
x\over2}$





\answer{11.3.30}
$S_M(x)=\dst\frac{8L^2}{\pi^2}\sum_{n=1}^\infty\frac{1}{(2n-1)^2}\left[
(-1)^n+\frac{4}{(2n-1)\pi}\right]\sin{(2n-1)\pi
x\over2L}$

\answer{11.3.31}
$C(x)=\dst-{7L^4\over5}-\frac{144L^4}{\pi^4}\sum_{n=1}^\infty{(-1)^n\over
n^4}\cos\frac{n\pi x}{L}$

\answer{11.3.32}
$C(x)=\dst-{2L^4\over5}-\frac{48L^4}{\pi^4}\sum_{n=1}^\infty{1+(-1)^n2\over
n^4}\cos\frac{n\pi x}{L}$

\answer{11.3.33}
$C(x)=\dst{3L^4\over5}-\frac{48L^4}{\pi^4}\sum_{n=1}^\infty{2+(-1)^n\over
n^4}\cos\frac{n\pi x}{L}$

\answer{11.3.34}
$\dst C(x)={L^4\over30}-\frac{3L^4}{\pi^4}\sum_{n=1}^\infty\frac{1}{n^4}\cos\frac{2n\pi x}{L}$

\answer{11.3.36}
$S(x)=\dst\frac{8L^2}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}
\sin{(2n-1)\pi x\over L}$


\answer{11.3.37}
$S(x)=\dst-\frac{4L^3}{\pi^3}\sum_{n=1}^\infty{(1+(-1)^n2)\over n^3}
\sin\frac{n\pi x}{L}$

\answer{11.3.38}
$S(x)=\dst-\frac{12L^3}{\pi^3}\sum_{n=1}^\infty{(-1)^n\over n^3}
\sin\frac{n\pi x}{L}$


\answer{11.3.39}
$S(x)=\dst\frac{96L^4}{\pi^5}\sum_{n=1}^\infty\frac{1}{(2n-1)^5}\sin{(2n-1)\pi
x\over L}$

\answer{11.3.40}
$S(x)=\dst-\frac{720L^5}{\pi^5}\sum_{n=1}^\infty{(-1)^n\over n^5}\sin{n\pi
x\over L}$

\answer{11.3.41}
$S(x)=\dst-\frac{240L^5}{\pi^5}\sum_{n=1}^\infty{1+(-1)^n2\over n^5}\sin{n\pi
x\over L}$

\answer{11.3.43}
$C_M(x)=-\dst\frac{64L^3}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n+\frac{3}{(2n-1)\pi}\right]\cos{(2n-1)\pi
x\over2L}$


\answer{11.3.44}
$C_M(x)=-\dst
\frac{32L^2}{\pi^3}\sum_{n=1}^\infty{(-1)^n\over
(2n-1)^3}\cos{(2n-1)\pi x\over2L}$

\answer{11.3.45}
$C_M(x)=-\dst\frac{96L^3}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n+\frac{2}{(2n-1)\pi}\right]\cos{(2n-1)\pi
x\over2L}$

\answer{11.3.46}
$C_M(x)=\dst\frac{96L^3}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n3+\frac{4}{(2n-1)\pi}\right]\cos{(2n-1)\pi
x\over2L}$

\answer{11.3.47}
$C_M(x)=\dst\frac{96L^3}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n5+\frac{8}{(2n-1)\pi}\right]\cos{(2n-1)\pi
x\over2L}$

\answer{11.3.48}
$C_M(x)=-\dst\frac{384L^4}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
1+{(-1)^n4\over(2n-1)\pi}\right]\cos{(2n-1)\pi
x\over2L}$

\answer{11.3.49}
$C_M(x)=-\dst\frac{768L^4}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
1+{(-1)^n2\over(2n-1)\pi}\right]\cos{(2n-1)\pi
x\over2L}$

\answer{11.3.51}
$S_M(x)=\dst
\frac{32L^2}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\sin{(2n-1)\pi x\over2L}$

\answer{11.3.52}
$S_M(x)
=\dst-\frac{96L^3}{\pi^3}\sum_{n=1}^\infty
\frac{1}{(2n-1)^3}\left[1+(-1)^n\frac{4}{(2n-1)\pi}\right]\sin{(2n-1)\pi x
\over2L}$

\answer{11.3.53}
$S_M(x)
=\dst\frac{96L^3}{\pi^3}\sum_{n=1}^\infty
\frac{1}{(2n-1)^3}\left[1+(-1)^n\frac{2}{(2n-1)\pi}\right]\sin{(2n-1)\pi
x\over2L}$

\answer{11.3.54}
$S_M(x)=\dst\frac{192L^3}{\pi^4}\sum_{n=1}^\infty{(-1)^n\over(2n-1)^4}
\sin{(2n-1)\pi x\over2L}$

\answer{11.3.55}
$S_M(x)=\dst\frac{1536L^4}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}
\left[(-1)^n+\frac{3}{(2n-1)\pi}\right]\sin{(2n-1)\pi x\over2L}$

\answer{11.3.56}
$S_M(x)=\dst\frac{384L^4}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}
\left[(-1)^n+\frac{4}{(2n-1)\pi}\right]\sin{(2n-1)\pi x\over2L}$


\sectionanswers{12.1}

\answer{12.1.8}
$u(x,t)=\dst\frac{8}{\pi^3}\sum_{n=1}^\infty
\frac{1}{(2n-1)^3}
e^{-(2n-1)^2\pi^2t}\sin(2n-1)\pi x$

\answer{12.1.9}
$u(x,t)=\dst\frac{4}{\pi}\sum_{n=1}^\infty\frac{1}{(2n-1)}
e^{-9(2n-1)^2\pi^2t/16}\sin{(2n-1)\pi x\over 4}$


\answer{12.1.10}
$u(x,t)=\dst{\pi\over2}e^{-3t}\sin
x-\frac{16}{\pi}\sum_{n=1}^\infty\frac{n}{(4n^2-1)^2}e^{-12n^2t}\sin2nx$


\answer{12.1.11}
$u(x,t)=\dst-\frac{32}{\pi^3}\sum_{n=1}^\infty
{(1+(-1)^n2)\over n^3}
e^{-9n^2\pi^2t/4}\sin\frac{n\pi x}{2}$

\answer{12.1.12}
$u(x,t)=\dst-\frac{324}{\pi^3}\sum_{n=1}^\infty{(-1)^n\over
n^3}
e^{-4n^2\pi^2t/9}\sin\frac{n\pi x}{3}$

\answer{12.1.13}
$u(x,t)=\dst\frac{8}{\pi^2}\sum_{n=1}^\infty{(-1)^{n+1}\over(2n-1)^2}
e^{-(2n-1)^2\pi^2t}\sin{(2n-1)\pi x\over 2}$


\answer{12.1.14}
$u(x,t)=\dst-\frac{720}{\pi^5}\sum_{n=1}^\infty{(-1)^n\over
n^5}e^{-7n^2\pi^2t}\sin n\pi x$

\answer{12.1.15}
$u(x,t)=\dst\frac{96}{\pi^5}\sum_{n=1}^\infty\frac{1}{(2n-1)^5}
e^{-5(2n-1)^2\pi^2t}\sin(2n-1)\pi x$


\answer{12.1.16}
$u(x,t)=\dst-\frac{240}{\pi^5}\sum_{n=1}^\infty{1+(-1)^n2\over
n^5}e^{-2n^2\pi^2t}\sin n\pi x$.

\answer{12.1.17}
$u(x,t)=\dst{16\over3}+\frac{64}{\pi^2}\sum_{n=1}^\infty{(-1)^n\over
n^2} e^{-9\pi^2n^2t/16}\cos\frac{n\pi x}{4}$

\answer{12.1.18}
$u(x,t)=\dst-{8\over3}+\frac{16}{\pi^2}\sum_{n=1}^\infty\frac{1}{n^2}e^{-n^2\pi^2t}\cos\frac{n\pi x}{2}$

\answer{12.1.19}
$u(x,t)=\dst{1\over6}-\frac{1}{\pi^2}\sum_{n=1}^\infty\frac{1}{n^2}e^{-36n^2\pi^2t}\cos 2n\pi x$


\answer{12.1.20}
$u(x,t)=\dst4-\frac{384}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}e^{-3(2n-1)^2\pi^2t/4}\cos{(2n-1)\pi x\over 2}$

\answer{12.1.21}
$u(x,y)=\dst-{28\over5}-\frac{576}{\pi^4}\sum_{n=1}^\infty{(-1)^n\over
n^4}e^{-5n^2\pi^2t/2}\cos\frac{n\pi x}{\sqrt2}$

\answer{12.1.22}
$u(x,t)=\dst-{2\over5}-\frac{48}{\pi^4}\sum_{n=1}^\infty{1+(-1)^n2\over
n^4}e^{-3n^2\pi^2t}\cos n\pi x$

\answer{12.1.23}
$u(x,t)=\dst{3\over5}-\frac{48}{\pi^4}\sum_{n=1}^\infty{2+(-1)^n\over
n^4}e^{-n^2\pi^2t}\cos n\pi x$

\answer{12.1.24}
$\dst u(x,t)={\pi^4\over30}-3\sum_{n=1}^\infty\frac{1}{n^4}e^{-4n^2t}\cos2nx$


\answer{12.1.25}
$u(x,t)=\dst{\frac{8}{\pi} \sum^\infty_{n=1}
{(-1)^n\over(2n+1)(2n-3)} e^{-(2n-1)^2\pi^2 t/4}\sin{(2n-1)\pi x\over
2}}$


\answer{12.1.26}
$u(x,t)=\dst8\sum_{n=1}^\infty\frac{1}{(2n-1)^2}\left[
(-1)^n+\frac{4}{(2n-1)\pi}\right]e^{-3(2n-1)^2t/4}\sin{(2n-1) x\over
2}$

\answer{12.1.27}
$u(x,t)=\dst
\frac{128}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}e^{-5(2n-1)^2t/16}\sin{(2n-1)\pi x\over4}$

\answer{12.1.28}
$u(x,t)
=\dst-\frac{96}{\pi^3}\sum_{n=1}^\infty
\frac{1}{(2n-1)^3}\left[1+(-1)^n\frac{4}{(2n-1)\pi}\right]
e^{-(2n-1)^2\pi^2t/4}\sin{(2n-1)\pi x\over2}$



\answer{12.1.29}
$u(x,t)
=\dst\frac{96}{\pi^3}\sum_{n=1}^\infty
\frac{1}{(2n-1)^3}\left[1+(-1)^n\frac{2}{(2n-1)\pi}\right]
e^{-(2n-1)^2\pi^2t/4}\sin{(2n-1)\pi x\over2}$


\answer{12.1.30}
$u(x,t)=\dst\frac{192}{\pi^4}\sum_{n=1}^\infty{(-1)^n\over(2n-1)^4}
e^{-(2n-1)^2\pi^2t/4}\sin{(2n-1)\pi x\over2}$


\answer{12.1.31}
$u(x,t)=\dst\frac{1536}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}
\left[(-1)^n+\frac{3}{(2n-1)\pi}\right]
e^{-(2n-1)^2\pi^2t/4}\sin{(2n-1)\pi x\over2}$



\answer{12.1.32}
$u(x,t)=\dst\frac{384}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}
\left[(-1)^n+\frac{4}{(2n-1)\pi}\right]
e^{-(2n-1)^2\pi^2t/4}\sin{(2n-1)\pi x\over2}$



\answer{12.1.33}
$u(x,t)=-\dst64\sum_{n=1}^\infty{e^{-3(2n-1)^2t/4}\over(2n-1)^3}\left[
(-1)^n+\frac{3}{(2n-1)\pi}\right]\cos{(2n-1)
x\over2}$


\answer{12.1.34}
$u(x,t)=-\dst\frac{16}{\pi} \sum^\infty_{n=1}
{(-1)^n\over2n-1} e^{-(2n-1)^2t}\cos{(2n-1)x\over4}$

\answer{12.1.35}
$u(x,t)=\dst-\frac{64}{\pi}\sum_{n=1}^\infty{(-1)^n\over2n-1}\left[
1-\frac{8}{(2n-1)^2\pi^2}\right]e^{-9(2n-1)^2\pi^2 t/64}\cos{(2n-1)\pi
x\over8}$

\answer{12.1.36}
$u(x,t)=
\dst\frac{8}{\pi^2}\sum_{n=1}^\infty\frac{1}{(2n-1)^2}
e^{-3(2n-1)^2\pi^2t/4}
\cos{(2n-1)\pi x\over2}$


\answer{12.1.37}
$u(x,t)=-\dst\frac{96}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n+\frac{2}{(2n-1)\pi}\right]e^{-(2n-1)^2\pi^2t/4}\cos{(2n-1)\pi
x\over2}$

\answer{12.1.38}
$u(x,t)=-\dst
\frac{32}{\pi}\sum_{n=1}^\infty{(-1)^n\over
(2n-1)^3}e^{-7(2n-1)^2t/4}\cos{(2n-1) x\over2}$

\answer{12.1.39}
$u(x,t)=\dst\frac{96}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n5+\frac{8}{(2n-1)\pi}\right]e^{-(2n-1)^2\pi^2t/4}\cos{(2n-1)\pi
x\over2}$

\answer{12.1.40}
$u(x,t)=\dst\frac{96}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n3+\frac{4}{(2n-1)\pi}\right]e^{-(2n-1)^2\pi^2t/4}\cos{(2n-1)\pi
x\over2}$

\answer{12.1.41}
$u(x,t)=-\dst\frac{768}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
1+{(-1)^n2\over(2n-1)\pi}\right]e^{-(2n-1)^2\pi^2t/4}\cos{(2n-1)\pi
x\over2}$

\answer{12.1.42}
$u(x,t)=-\dst\frac{384}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
1+{(-1)^n4\over(2n-1)\pi}\right]e^{-(2n-1)^2\pi^2t/4}\cos{(2n-1)\pi
x\over2}$


\answer{12.1.43}
$u(x,t)=\dst{1\over2}-\frac{2}{\pi}\sum_{n=1}^\infty
{(-1)^n\over2n-1}e^{-(2n-1)^2\pi^2a^2t/L^2}\cos{(2n-1)\pi x\over L}$

\answer{12.1.44}
$u(x,t)=\dst\frac{2}{\pi}\sum_{n=1}^\infty\frac{1}{n}
\left[1-\cos{n\pi\over2}\right]e^{-n^2\pi^2a^2t/L^2}\sin\frac{n\pi x}{L}$


\answer{12.1.45}
$u(x,t)=\dst\frac{4}{\pi}\sum_{n=1}^\infty{1\over2n-1}\sin{(2n-1)\pi\over4}
e^{-(2n-1)^2\pi^2a^2t/4L^2}\cos{(2n-1)\pi x\over 2L}$

\answer{12.1.46}
$u(x,t)=\dst\frac{4}{\pi}\sum_{n=1}^\infty{1\over2n-1}
\left[1-\cos{(2n-1)\pi)\over4}\right]
e^{-(2n-1)^2\pi^2a^2t/4L^2}\sin{(2n-1)\pi x\over 2L}$

\answer{12.1.48}
$u(x,t)=1-x+x^3+\dst\frac{4}{\pi}\sum_{n=1}^\infty{e^{-9\pi^2(2n-1)^2t/16}\over(2n-1)}
\sin{(2n-1)\pi x\over 4}$


\answer{12.1.49}
$u(x,t)=\dst1+x+x^2
-\dst\frac{8}{\pi^3}\sum_{n=1}^\infty
{e^{-(2n-1)^2\pi^2t}\over(2n-1)^3}
\sin(2n-1)\pi x$

\answer{12.1.50}
$u(x,t)=-1-x+x^3+
\dst\frac{8}{\pi^2}\sum_{n=1}^\infty\frac{1}{(2n-1)^2}
e^{-3(2n-1)^2\pi^2t/4}
\cos{(2n-1)\pi x\over2}$

\answer{12.1.51}
$u(x,t)=x^2-x-2\dst-\frac{64}{\pi}\sum_{n=1}^\infty{(-1)^n\over2n-1}\left[
1-\frac{8}{(2n-1)^2\pi^2}\right]e^{-9(2n-1)^2\pi^2 t/64}\cos{(2n-1)\pi
x\over8}$

\answer{12.1.52}
$u(x,t)=\sin\pi x+\dst{\frac{8}{\pi} \sum^\infty_{n=1}
{(-1)^n\over(2n+1)(2n-3)} e^{-(2n-1)^2\pi^2 t/4}\sin{(2n-1)\pi x\over
2}}$

\answer{12.1.53}
$u(x,t)=x^3-x+3+\dst\frac{32}{\pi^3}\sum_{n=1}^\infty{e^{-(2n-1)^2\pi^2t/4}\over
(2n-1)^3}\sin{(2n-1)\pi x\over2}$


\sectionanswers{12.2}

\answer{12.2.1}
$u(x,t)=\dst{4\over3\pi^3}\sum_{n=1}^\infty
{(-1)^{n+1}\over(2n-1)^3}\sin3(2n-1)\pi t\sin(2n-1)\pi x$

\answer{12.2.2}
$u(x,t)=\dst\frac{8}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}
\cos3(2n-1)\pi t\sin(2n-1)\pi x$


\answer{12.2.3}
$u(x,t)=\dst-\frac{4}{\pi^3}\sum_{n=1}^\infty{(1+(-1)^n2)\over n^3}
\cos n\sqrt7\,\pi t\,\sin n\pi x$

\answer{12.2.4}
$u(x,t)=\dst{8\over3\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}
\sin3(2n-1)\pi t\sin(2n-1)\pi x$

\answer{12.2.5}
$u(x,t)=\dst-\frac{4}{\sqrt7\,\pi^4}\sum_{n=1}^\infty{(1+(-1)^n2)\over
n^4}
\sin n\sqrt7\,\pi t\,\sin n\pi x$

\answer{12.2.6}
$u(x,t)=\dst\frac{324}{\pi^3}\sum_{n=1}^\infty{(-1)^n\over
n^3}\cos{8n\pi t\over3}\sin{n\pi x\over3}$

\answer{12.2.7}
$u(x,t)=\dst\frac{96}{\pi^5}\sum_{n=1}^\infty\frac{1}{(2n-1)^5}
\cos2(2n-1)\pi t\sin(2n-1)\pi x$

\answer{12.2.8}
$u(x,t)=\dst{243\over2\pi^4}\sum_{n=1}^\infty{(-1)^n\over
n^4}\sin{8n\pi t\over3}\sin{n\pi x\over3}$


\answer{12.2.9}
$u(x,t)=\dst\frac{48}{\pi^6}\sum_{n=1}^\infty\frac{1}{(2n-1)^6}
\sin2(2n-1)\pi t\sin(2n-1)\pi x$.




\answer{12.2.10}
$u(x,t)=\dst{\pi\over2}\cos\sqrt{5}\,t\sin x-\frac{16}{\pi}
\sum_{n=1}^\infty\frac{n}{(4n^2-1)^2}\cos2n\sqrt5\,t\sin 2nx$

\answer{12.2.11}
$u(x,t)=\dst-\frac{240}{\pi^5}\sum_{n=1}^\infty{1+(-1)^n2\over n^5}
\cos n\pi t\sin n\pi x$


\answer{12.2.12}
$u(x,t)=\dst{\pi\over2\sqrt5}\sin\sqrt{5}\,t\sin x-\frac{8}{\pi\sqrt5}
\sum_{n=1}^\infty\frac{1}{(4n^2-1)^2}\sin2n\sqrt5\,t\sin 2nx$



\answer{12.2.13}
$u(x,t)=\dst-\frac{240}{\pi^6}\sum_{n=1}^\infty{1+(-1)^n2\over n^6}
\sin n\pi t\sin n\pi x$


\answer{12.2.14}
$u(x,t)=\dst-\frac{720}{\pi^5}\sum_{n=1}^\infty{(-1)^n\over n^5}
\cos3n\pi t\sin n\pi x$


\answer{12.2.15}
$u(x,t)=\dst-\frac{240}{\pi^6}\sum_{n=1}^\infty{(-1)^n\over n^6}
\sin3n\pi t\sin n\pi x$








\answer{12.2.18}
$u(x,t)=-\dst
\frac{128}{\pi^3}\sum_{n=1}^\infty{(-1)^n\over
(2n-1)^3}\cos{3(2n-1)\pi t\over 4}\,\cos{(2n-1)\pi x\over4}$


\answer{12.2.19}
$u(x,t)=-\dst\frac{64}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n+\frac{3}{(2n-1)\pi}\right]\cos(2n-1)\pi t\,\cos{(2n-1)\pi
x\over2}$

\answer{12.2.20}
$u(x,t)=-\dst
{512\over3\pi^4}\sum_{n=1}^\infty{(-1)^n\over
(2n-1)^4}\sin{3(2n-1)\pi t\over 4}\,\cos{(2n-1)\pi x\over4}$

\answer{12.2.21}
$u(x,t)=-\dst\frac{64}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
(-1)^n+\frac{3}{(2n-1)\pi}\right]\sin(2n-1)\pi t\,\cos{(2n-1)\pi
x\over2}$

\answer{12.2.22}
$u(x,t)=\dst\frac{96}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n3+\frac{4}{(2n-1)\pi}\right]\cos{(2n-1)\sqrt5\,\pi
t\over2}\cos{(2n-1)\pi x\over2}$


\answer{12.2.23}
$u(x,t)=-\dst96\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n+\frac{2}{(2n-1)\pi}\right]\cos{(2n-1)\sqrt3\, t\over2}\cos{(2n-1)
x\over2}$


\answer{12.2.24}
$u(x,t)=\dst\frac{192}{\pi^4\sqrt5}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
(-1)^n3+\frac{4}{(2n-1)\pi}\right]\sin{(2n-1)\sqrt5\,\pi
t\over2}\cos{(2n-1)\pi x\over2}$

\answer{12.2.25}
$u(x,t)=-\dst\frac{192}{\sqrt3}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
(-1)^n+\frac{2}{(2n-1)\pi}\right]\sin{(2n-1)\sqrt3 t\over2}\sin{(2n-1)
x\over2}$


\answer{12.2.26}
$u(x,t)=-\dst\frac{384}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
1+{(-1)^n4\over(2n-1)\pi}\right]\cos{3(2n-1)\pi t\over2}\cos{(2n-1)\pi
x\over2}$

\answer{12.2.27}
$u(x,t)=\dst\frac{96}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n5+\frac{8}{(2n-1)\pi}\right]\cos{(2n-1)\sqrt7\,\pi
t\over2}\cos{(2n-1)\pi x\over2}$


\answer{12.2.28}
$u(x,t)=-\dst{768\over3\pi^5}\sum_{n=1}^\infty\frac{1}{(2n-1)^5}\left[
1+{(-1)^n4\over(2n-1)\pi}\right]\sin{3(2n-1)\pi t\over2}\cos{(2n-1)\pi
x\over2}$

\answer{12.2.29}
$u(x,t)=\dst\frac{192}{\pi^4\sqrt7}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
(-1)^n5+\frac{8}{(2n-1)\pi}\right]\sin{(2n-1)\sqrt7\,\pi
t\over2}\cos{(2n-1)\pi x\over2}$

\answer{12.2.30}
$u(x,t)=-\dst\frac{768}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\left[
1+{(-1)^n2\over(2n-1)\pi}\right]\cos{(2n-1)\pi t\over2}\cos{(2n-1)\pi
x\over2}$

\answer{12.2.31}
$u(x,t)=-\dst\frac{1536}{\pi^5}\sum_{n=1}^\infty\frac{1}{(2n-1)^5}\left[
1+{(-1)^n2\over(2n-1)\pi}\right]\sin{(2n-1)\pi t\over2}\cos{(2n-1)\pi
x\over2}$

\answer{12.2.32}
$u(x,t)=\dst{1\over2}[C_{M\!f}(x+at)+C_{M\!f}(x-at)]+
{1\over2a}\int_{x-at}^{x+at}C_{M\!g}(\tau)\,d\tau$


\answer{12.2.35}
$u(x,t)=\dst
\frac{32}{\pi}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\cos4(2n-1)t\,\sin{(2n-1)x\over2}$

\answer{12.2.36}
$u(x,t)
=\dst-\frac{96}{\pi^3}\sum_{n=1}^\infty
\frac{1}{(2n-1)^3}\left[1+(-1)^n\frac{4}{(2n-1)\pi}\right]
\cos{3(2n-1)\pi t\over2}\sin{(2n-1)\pi x\over2}$

\answer{12.2.37}
$u(x,t)=\dst
\frac{8}{\pi}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\sin4(2n-1)t\,\sin{(2n-1)x\over2}$



\answer{12.2.38}
$u(x,t)
=\dst-\frac{64}{\pi^4}\sum_{n=1}^\infty
\frac{1}{(2n-1)^4}\left[1+(-1)^n\frac{4}{(2n-1)\pi}\right]
\sin{3(2n-1)\pi t\over2}\sin{(2n-1)\pi x\over2}$

\answer{12.2.39}
$u(x,t)
=\dst\frac{96}{\pi^3}\sum_{n=1}^\infty
\frac{1}{(2n-1)^3}\left[1+(-1)^n\frac{2}{(2n-1)\pi}\right]
\cos{3(2n-1)\pi t\over2}
\sin{(2n-1)\pi x\over2}$


\answer{12.2.40}
$u(x,t)=\dst\frac{192}{\pi}\sum_{n=1}^\infty{(-1)^n\over(2n-1)^4}
\cos{(2n-1)\sqrt3\,t\over2}\sin{(2n-1) x\over2}$

\answer{12.2.41}
$u(x,t)
=\dst\frac{64}{\pi^4}\sum_{n=1}^\infty
\frac{1}{(2n-1)^4}\left[1+(-1)^n\frac{2}{(2n-1)\pi}\right]
\sin{3(2n-1)\pi t\over2}
\sin{(2n-1)\pi x\over2}$


\answer{12.2.42}
$u(x,t)=\dst\frac{384}{\sqrt3\,\pi}\sum_{n=1}^\infty{(-1)^n\over(2n-1)^5}
\sin{(2n-1)\sqrt3\,t\over2}\sin{(2n-1) x\over2}$


\answer{12.2.43}
$u(x,t)=\dst\frac{1536}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}
\left[(-1)^n+\frac{3}{(2n-1)\pi}\right]
\cos{(2n-1)\sqrt5\,\pi t\over2}\sin{(2n-1)\pi x\over2}$

\answer{12.2.44}
$u(x,t)=\dst\frac{384}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}
\left[(-1)^n+\frac{4}{(2n-1)\pi}\right]\cos(2n-1)\pi
t\sin{(2n-1)\pi x\over2}$

\answer{12.2.45}
$u(x,t)=\dst\frac{3072}{\sqrt5\,\pi^5}\sum_{n=1}^\infty\frac{1}{(2n-1)^5}
\left[(-1)^n+\frac{3}{(2n-1)\pi}\right]
\sin{(2n-1)\sqrt5\,\pi t\over2}\sin{(2n-1)\pi x\over2}$




\answer{12.2.46}
$u(x,t)=\dst\frac{384}{\pi^5}\sum_{n=1}^\infty\frac{1}{(2n-1)^5}
\left[(-1)^n+\frac{4}{(2n-1)\pi}\right]\sin(2n-1)\pi
t\sin{(2n-1)\pi x\over2}$

\answer{12.2.47}
$\dst u(x,t)={1\over2}[S_{M\!f}(x+at)+S_{M\!f}(x-at)]+
{1\over2a}\int_{x-at}^{x+at}S_{M\!g}(\tau)\,d\tau$


\answer{12.2.50}
$u(x,t)=\dst4-\frac{768}{\pi^4}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}\cos{\sqrt5(2n-1)\pi t\over2}\,\cos{(2n-1)\pi x\over 2}$

\answer{12.2.51}
$u(x,t)=\dst4t-\frac{1536}{\sqrt5\,\pi^5}\sum_{n=1}^\infty\frac{1}{(2n-1)^5}\sin{\sqrt5(2n-1)\pi t\over2}\,\cos{(2n-1)\pi x\over 2}$

\answer{12.2.52}
$u(x,t)=\dst-{2\pi^4\over5}-48\sum_{n=1}^\infty{1+(-1)^n2\over
n^4}\cos2nt\cos nx$

\answer{12.2.53}
$u(x,t)=\dst-{7\over5}-\frac{144}{\pi^4}\sum_{n=1}^\infty{(-1)^n\over
n^4}\cos n\sqrt7\,\pi t\cos n\pi x$

\answer{12.2.54}
$u(x,t)=\dst-{2\pi^4t\over5}-24\sum_{n=1}^\infty{1+(-1)^n2\over
n^5}\sin2n t\cos nx$

\answer{12.2.55}
$u(x,t)=\dst-{7t\over5}-\frac{144}{\pi^5\sqrt7}\sum_{n=1}^\infty{(-1)^n\over
n^5}\sin n\sqrt7\,\pi t\cos n\pi x$

\answer{12.2.56}
$u(x,t)=\dst{{\pi^4\over30}-3\sum_{n=1}^\infty\frac{1}{n^4}\cos8nt\cos2nx}$

\answer{12.2.57}
$u(x,t)=\dst{3\over5}-\frac{48}{\pi^4}\sum_{n=1}^\infty{2+(-1)^n\over
n^4}\cos n\pi t\cos n\pi x$

\answer{12.2.58}
$u(x,t)=\dst{{\pi^4t\over30}-{3\over8}\sum_{n=1}^\infty\frac{1}{n^5}\sin8nt\cos2nx}$

\answer{12.2.59}
$u(x,t)=\dst{3t\over5}-\frac{48}{\pi^5}\sum_{n=1}^\infty{2+(-1)^n\over
n^5}\sin n\pi t\cos n\pi x$

\answer{12.2.60}
$u(x,t)={1\over2}[C_f(x+at)+C_f(x-at)]+
{1\over2a}\int_{x-at}^{x+at}C_g(\tau)\,d\tau$

\answer{12.2.63}{\bf (c)}
$u(x,t)=\dst{f(x+at)+f(x-at)\over2}+
{1\over2a}\int_{x-at}^{x+at}g(u)\,du$

\answer{12.2.64}
$u(x,t)=x(1+4at)$
\answer{12.2.65}
$u(x,t)=x^2+a^2t^2+t$

\answer{12.2.66}
$u(x,t)=\sin(x+at)$
\answer{12.2.67}
$u(x,t)=x^3+6tx^2+3a^2t^2x+2a^2t^3$


\answer{12.2.68}
$u(x,t)=\dst x\sin x\cos at+at\cos x\sin at+\frac{\sin x\sin at}{a}$



\sectionanswers{12.3}

\answer{12.3.1}
$u(x,y)=\dst\frac{8}{\pi^3}\sum_{n=1}^\infty{\sinh(2n-1)\pi(1-y)
\over(2n-1)^3\sinh(2n-1)\pi}
\sin(2n-1)\pi x$

\answer{12.3.2}
$u(x,y)=\dst-\frac{32}{\pi^3}\sum_{n=1}^\infty{(1+(-1)^n2)\sinh
n\pi(3-y)/2\over n^3\sinh3n\pi/2}
\sin\frac{n\pi x}{2}$

\answer{12.3.3}
$u(x,y)=\dst\frac{8}{\pi^2}\sum_{n=1}^\infty(-1)^{n+1}{
\sinh(2n-1)\pi(1-y/2)\over(2n-1)^2\sinh(2n-1)\pi}
\sin{(2n-1)\pi x\over 2}$

\answer{12.3.4}
$u(x,y)=\dst{\pi\over2}{\sinh(1-y)\over\sinh1}\sin
x-\frac{16}{\pi}\sum_{n=1}^\infty{n\sinh2n(1-y)\over(4n^2-1)^2\sinh2n}\sin2nx$

\answer{12.3.5}
$u(x,y)=\dst3y+\frac{108}{\pi^3}\sum_{n=1}^\infty(-1)^n{\sinh n\pi
y/3\over n^3\cosh2n\pi/3}\cos\frac{n\pi x}{3}$


\answer{12.3.6}
$u(x,y)=\dst{y\over2}+\frac{4}{\pi^3}\sum_{n=1}^\infty{
\sinh(2n-1)\pi y\over(2n-1)^3\cosh2(2n-1)\pi}
\cos(2n-1)\pi x$

\answer{12.3.7}
$u(x,y)=\dst-{8y\over3}+\frac{32}{\pi^3}\sum_{n=1}^\infty(-1)^n{
\sinh n\pi y/2\over
n^3\cosh n\pi}\cos\frac{n\pi x}{2}$

\answer{12.3.8}
$u(x,y)=\dst{y\over3}+\frac{4}{\pi^3}\sum_{n=1}^\infty\frac{\sinh n\pi y}{n^3\cosh n\pi}\cos n\pi x$

\answer{12.3.9}
$u(x,y)=\dst
\frac{128}{\pi^3}\sum_{n=1}^\infty{\cosh(2n-1)\pi(x-3)/4\over
(2n-1)^3\cosh3(2n-1)\pi/4}
\sin{(2n-1)\pi y\over4}$

\answer{12.3.10}
$u(x,y)
=\dst-\frac{96}{\pi^3}\sum_{n=1}^\infty
\left[1+(-1)^n\frac{4}{(2n-1)\pi}\right]
{\cosh(2n-1)\pi(x-2)/2\over(2n-1)^3\cosh(2n-1)\pi}
\sin{(2n-1)\pi y\over2}$


\answer{12.3.11}
$u(x,y)
=\dst\frac{768}{\pi^3}\sum_{n=1}^\infty
\left[1+(-1)^n\frac{2}{(2n-1)\pi}\right]
{\cosh(2n-1)\pi(x-2)/4\over(2n-1)^3\cosh(2n-1)\pi/2}
\sin{(2n-1)\pi y\over4}$

\answer{12.3.12}
$u(x,y)
=\dst\frac{96}{\pi^3}\sum_{n=1}^\infty
\left[3+(-1)^n\frac{4}{(2n-1)\pi}\right]
{\cosh(2n-1)\pi(x-3)/2\over(2n-1)^3\cosh3(2n-1)\pi/2}
\sin{(2n-1)\pi y\over2}$

\answer{12.3.13}
$u(x,y)=
-\dst\frac{16}{\pi}\sum_{n=1}^\infty{\cosh(2n-1)x/2\over
(2n-3)(2n+1)(2n-1)\sinh(2n-1)/2}
\cos{(2n-1) y\over2}$

\answer{12.3.14}
$u(x,y)=-\dst\frac{432}{\pi^3}\sum_{n=1}^\infty
\left[1+{4(-1)^n\over(2n-1)\pi}\right]
{\cosh(2n-1)\pi x/6\over(2n-1)^3\sinh(2n-1)\pi/3}
\cos{(2n-1)\pi
y\over6}$

\answer{12.3.15}
$u(x,y)=-\dst
\frac{64}{\pi}\sum_{n=1}^\infty(-1)^n{\cosh(2n-1)x/2\over
(2n-1)^4\sinh(2n-1)/2}\cos{(2n-1) y\over2}$.

\answer{12.3.16}
$u(x,y)=-\dst\frac{192}{\pi^4}\sum_{n=1}^\infty
{\cosh(2n-1)\pi x/2\over(2n-1)^4\sinh(2n-1)\pi/2}
\left[(-1)^n+\frac{2}{(2n-1)\pi}\right]
\cos{(2n-1)\pi y\over2}$



\answer{12.3.17}
$\dst u(x,y)=\sum_{n=1}^\infty\alpha_n{\sinh n\pi y/a\over
\sinh n\pi b/a}\sin
\frac{n\pi x}{a},\quad
\alpha_n=\frac{2}{a}\int_0^a f(x)\sin
\frac{n\pi x}{a}\,dx$



\quad \quad $u(x,y)=\dst\frac{72}{\pi^3}\sum_{n=1}^\infty
{\sinh(2n-1)\pi y/3\over(2n-1)^3\sinh2(2n-1)\pi/3}
\sin{(2n-1)\pi x\over 3}$


\answer{12.3.18}
$\dst
u(x,y)=\alpha_0(1-y/b)+\sum_{n=1}^\infty\alpha_n{\sinh n\pi(b-y)/
a\over\sinh n\pi b/a}\cos
\frac{n\pi x}{a}$, \quad
$\dst\alpha_0=\frac{1}{a}\int_0^a f(x)\,dx$,

\quad \quad$\dst\alpha_n=\frac{2}{a}\int_0^a f(x)\cos
\frac{n\pi x}{a}\,dx,\quad n\ge1$


\quad \quad$\dst u(x,y)={8(1-y)\over15}-\frac{48}{\pi^4}\sum_{n=1}^\infty\frac{1}{n^4}{\sinh n\pi(1-y)\over\sinh n\pi}\cos n\pi x$


\answer{12.3.19}
$\dst
u(x,y)=\sum_{n=1}^\infty\alpha_n{\sinh(2n-1)\pi(b-y)/2a
\over\sinh(2n-1)\pi b/2a}\cos
{(2n-1)\pi x\over2a}$,

\quad \quad$\dst\alpha_n=\frac{2}{a}\int_0^a
f(x)\cos {(2n-1)\pi x\over2a}\,dx$


\quad \quad$u(x,y)=\dst
\frac{288}{\pi^3}\sum_{n=1}^\infty{\sinh(2n-1)\pi(2-y)/6\over
(2n-1)^3\sinh(2n-1)\pi/3}\sin{(2n-1)\pi x\over6}$

\answer{12.3.20}
$\dst
u(x,y)=\sum_{n=1}^\infty\alpha_n{\sinh(2n-1)\pi(b-y)/2a\over
\sinh(2n-1)\pi b/2a}\sin
{(2n-1)\pi x\over2a}$,

\quad \quad$\dst\alpha_n=\frac{2}{a}\int_0^a
f(x)\sin {(2n-1)\pi x\over2a}\,dx$



\quad \quad$u(x,y)=\dst\frac{32}{\pi^3}\sum_{n=1}^\infty\left[
(-1)^n5+\frac{18}{(2n-1)\pi}\right]
{\sinh(2n-1)\pi(2-y)/2\over(2n-1)^3\sinh(2n-1)\pi}
\cos{(2n-1)\pi x\over2}$.


\answer{12.3.21}
$\dst u(x,y)=\sum_{n=1}^\infty\alpha_n{\cosh n\pi(y-b)/a\over
\cosh n\pi b/a}\sin
\frac{n\pi x}{a}$, \quad
$\dst\alpha_n=\frac{2}{a}\int_0^a f(x)\sin
\frac{n\pi x}{a}\,dx$

\quad \quad$u(x,y)=\dst-12\sum_{n=1}^\infty(-1)^n{\cosh n(y-2)\over n^3\cosh2n}
\sin nx$


\answer{12.3.22}
$\dst u(x,y)=\alpha_0+\sum_{n=1}^\infty\alpha_n{\cosh n\pi y/a
\over\cosh n\pi b/a}\cos
\frac{n\pi x}{a}$,
$\dst\alpha_0=\frac{1}{a}\int_0^a f(x)\,dx$,

\quad \quad$\dst\alpha_n=\frac{2}{a}\int_0^a f(x)\cos
\frac{n\pi x}{a}\,dx,\quad n\ge1$

\quad \quad$\dst u(x,y)={\pi^4\over30}-3\sum_{n=1}^\infty\frac{1}{n^4}\frac{\cosh2ny}{\cos2n}\cos 2nx$


\answer{12.3.23}
$\dst u(x,y)=\frac{a}{\pi}\sum_{n=1}^\infty\alpha_n{\sinh n\pi
(y-b)/a\over n\cosh n\pi b/a}\sin
\frac{n\pi x}{a}$, \quad
$\dst\alpha_n=\frac{2}{a}\int_0^a
f(x)\sin \frac{n\pi x}{a}\,dx$

\quad \quad$u(x,y)=\dst\frac{4}{\pi}\sum_{n=1}^\infty(-1)^{n+1}
{\sinh(2n-1)(y-1)\over(2n-1)^3\cosh(2n-1)}
\sin(2n-1)x$


\answer{12.3.24}
$\dst u(x,y)=\sum_{n=1}^\infty\alpha_n{\cosh n\pi x/b\over
\cosh n\pi a/b}
\sin\frac{n\pi y}{b}$,\quad
$\dst\alpha_n=\frac{2}{b}\int_0^b g(y)\sin
\frac{n\pi y}{b}\,dy$


\quad \quad$u(x,y)=\dst\frac{96}{\pi^5}\sum_{n=1}^\infty{\cosh(2n-1)\pi x
\over(2n-1)^5\cosh(2n-1)\pi}\sin(2n-1)\pi y$.







\answer{12.3.25}
$\dst u(x,y)=\sum_{n=1}^\infty\alpha_n{\cosh(2n-1)\pi x/2b\over
\cosh(2n-1)\pi a/2b}
\cos{(2n-1)\pi y\over2b}$,\quad

\quad \quad$\dst\alpha_n=\frac{2}{b}\int_0^b g(y)\cos
{(2n-1)\pi y\over2b}\,dy$


\quad \quad$u(x,y)=-\dst
\frac{128}{\pi^3}\sum_{n=1}^\infty(-1)^n{\cosh(2n-1)\pi x/4\over
(2n-1)^3\cosh(2n-1)\pi/2}\cos{(2n-1)\pi y\over4}$.

\answer{12.3.26}
$\dst u(x,y)=\frac{b}{\pi}\sum_{n=1}^\infty\alpha_n
{\cosh n\pi x/b\over n \sinh n\pi a/b}
\sin\frac{n\pi y}{b}$,\quad
$\dst\alpha_n=\frac{2}{b}\int_0^b g(y)\sin
\frac{n\pi y}{b}\,dy$


\quad \quad$u(x,y)=\dst\frac{64}{\pi^3}\sum_{n=1}^\infty(-1)^{n+1}
{\cosh(2n-1)\pi x/4\over(2n-1)^3\sinh(2n-1)\pi/4}
\sin{(2n-1)\pi y\over 4}$


\answer{12.3.27}
$\dst
u(x,y)=-\frac{2b}{\pi}\sum_{n=1}^\infty\alpha_n{\cosh(2n-1)\pi(x-a)/2b
\over(2n-1)\sinh(2n-1)\pi a/2b}
\sin{(2n-1) y\over2b}$,\quad

\quad \quad$\dst\alpha_n=\frac{2}{b}\int_0^b g(y)\sin
{(2n-1)\pi y\over2b}\,dy$


\quad \quad$u(x,y)=\dst192\sum_{n=1}^\infty
\left[1+(-1)^n\frac{4}{(2n-1)\pi}\right]
{\cosh(2n-1)(x-1)/2\over(2n-1)^4\sinh(2n-1)/2}
\sin{(2n-1)y\over2}$.


\answer{12.3.28}
$\dst
u(x,y)=\alpha_0(x-a)+\frac{b}{\pi}\sum_{n=1}^\infty\alpha_n{\sinh
n\pi(x-a)/b
\over n\cosh n\pi a/b}
\cos\frac{n\pi y}{b}$,\quad
$\dst\alpha_0=\frac{1}{b}\int_0^bg(y)\cos\frac{n\pi y}{b}\,dy$,

\quad \quad$\dst\alpha_n=\frac{2}{b}\int_0^b g(y)\cos
\frac{n\pi y}{b}\,dy$

\quad \quad$u(x,y)=\dst{\pi(x-2)\over2}-\frac{4}{\pi}\sum_{n=1}^\infty
{\sinh(2n-1)(x-2)\over(2n-1)^3\cosh2(2n-1)}
\cos(2n-1)y$.

\answer{12.3.29}
$u(x,y)=\dst\alpha_0+\sum_{n=1}^\infty\alpha_n e^{-n\pi y/a}\cos
\frac{n\pi x}{a}$,\quad
$\alpha_0=\dst\frac{1}{a}\int_0^a f(x)\,dx$,

\quad \quad$\alpha_n=\dst\frac{2}{a}\int_0^a f(x)\cos
\frac{n\pi x}{a}\,dx,\quad n\ge1$


\quad \quad$u(x,y)={\pi^3\over2}-\frac{48}{\pi}\sum_{n=1}^\infty\frac{1}{(2n-1)^4}e^{-(2n-1)y}\cos(2n-1)x$

\answer{12.3.30}
$u(x,y)=\dst\sum_{n=1}^\infty\alpha_n e^{-(2n-1)\pi y/2a}\cos
{(2n-1)\pi x\over2a}$,\quad
$\alpha_n=\dst\frac{2}{a}\int_0^a
f(x)\cos {(2n-1)\pi x\over2a}\,dx$



\quad \quad$u(x,y)=-\dst
\frac{288}{\pi^3}\sum_{n=1}^\infty{(-1)^n\over
(2n-1)^3}e^{-(2n-1)\pi y/6}\cos{(2n-1)\pi x\over6}$


\answer{12.3.31}
$u(x,y)=\dst\sum_{n=1}^\infty\alpha_ne^{-(2n-1)\pi y/2a}\sin
{(2n-1)\pi x\over2a}$,\quad
$\alpha_n=\dst\frac{2}{a}\int_0^a
f(x)\sin {(2n-1)\pi x\over2a}\,dx$

\quad \quad$u(x,y)=\dst
\frac{32}{\pi}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}e^{-(2n-1)y/2}\sin{(2n-1) x\over2}$.

\answer{12.3.32}
$u(x,y)=-\dst\frac{a}{\pi}\sum_{n=1}^\infty\frac{\alpha_n}{n} e^{-n\pi
y/a}\sin
\frac{n\pi x}{a}$, \quad
$\alpha_n=\dst\frac{2}{a}\int_0^a
f(x)\sin \frac{n\pi x}{a}\,dx$

\quad \quad$u(x)=\dst4\sum_{n=1}^\infty{(1+(-1)^n2)\over n^4}
e^{-ny}\sin nx$

\answer{12.3.33}
$u(x,y)=-\dst\frac{2a}{\pi}\sum_{n=1}^\infty{\alpha_n\over2n-1}
e^{-(2n-1)\pi y/2a}\cos
{(2n-1)\pi x\over2a}$,\quad
$\alpha_n=\dst\frac{2}{a}\int_0^a
f(x)\cos {(2n-1)\pi x\over2a}\,dx$

\quad \quad$u(x,y)=\dst\frac{5488}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
1+{4(-1)^n\over(2n-1)\pi}\right]e^{-(2n-1)\pi y/14}\cos{(2n-1)\pi
x\over14}$


\answer{12.3.34}
$u(x,y)=\dst-\frac{2a}{\pi}\sum_{n=1}^\infty{\alpha_n\over2n-1}e^{-(2n-1)\pi
y/2a}\sin
{(2n-1)\pi x\over2a}$,\quad
$\alpha_n=\dst\frac{2}{a}\int_0^a
f(x)\sin {(2n-1)\pi x\over2a}\,dx$


\quad \quad$u(x,y)=-\dst\frac{2000}{\pi^3}\sum_{n=1}^\infty\frac{1}{(2n-1)^3}\left[
(-1)^n+\frac{4}{(2n-1)\pi}\right]e^{-(2n-1)\pi y/10}\sin{(2n-1)\pi
x\over10}$



\answer{12.3.35}
$u(x,y)=\dst
\sum_{n=1}^\infty{A_n\sinh n\pi(b-y)/a+B_n\sinh n\pi y/a\over \sinh
n\pi b/a}\sin\frac{n\pi x}{a}\\
\hspace*{6.25em}+\sum_{n=1}^\infty
{C_n\sinh n\pi(a-x)/b+D_n\sinh n\pi x/b\over \sinh n\pi a/b}\sin\frac{n\pi y}{b}$


\answer{12.3.36}
$\dst u(x,y)=C+
\frac{a}{\pi}\sum_{n=1}^\infty{B_n\cosh n\pi y/a-A_n\cosh
n\pi(y-b)/a\over n\sinh n\pi b/a}\cos\frac{n\pi x}{a}\\
\hspace*{6.5em}+\frac{b}{\pi}\sum_{n=1}^\infty{D_n\cosh n\pi
x/b-C_n\cosh n\pi(x-a)/b\over n\sinh n\pi a/b}\cos\frac{n\pi y}{b}$


\sectionanswers{12.4}

\answer{12.4.1}
$u(r,\theta)=\dst\alpha_0\frac{\ln r/\rho}{\ln\rho_0/\rho}+
\sum_{n=1}^\infty
\frac{r^n\rho^{-n}-\rho^nr^{-n}}
{\rho_0^n\rho^{-n}-\rho^n\rho_0^{-n}}
(\alpha_n\cos
n\theta+\beta_n\sin n\theta)$
$\dst\alpha_0=\frac{1}{2\pi}\int_{-\pi}^\pi f(\theta)\,d\theta$,

\quad\quad and
$\dst\alpha_n=\frac{1}{\pi}\int_{-\pi}^\pi f(\theta)\cos
n\theta\,d\theta$,\quad
$\dst\beta_n=\frac{1}{\pi}\int_{-\pi}^\pi f(\theta)\sin
n\theta\,d\theta$,\;
 $n=1$, $2$, $3$, \dots

\answer{12.4.2}
$\dst u(r,\theta)=\sum_{n=1}^\infty\alpha_n
\dst\frac{\rho_0^{-n\pi/\gamma}r^{n\pi/\gamma}-\rho_0^{n\pi/\gamma}
r^{-n\pi/\gamma}}{\rho_0^{-n\pi/\gamma}\rho^{n\pi/\gamma}-\rho_0^{n\pi/\gamma}
\rho^{-n\pi/\gamma}}
\sin\frac{n\pi\theta}{\gamma}$   \quad

\quad \quad
$\alpha_n=\dst\frac{1}{\gamma}\int_0^\gamma
f(\theta)\sin\frac{n\pi\theta}{\gamma}\,d\theta$, $n=1$, $2$,
$3$,\dots

\answer{12.4.3}
$\dst u(r,\theta)=
\rho\alpha_0\dst\ln\frac{r}{\rho_0}+\frac{\rho\gamma}{\pi}\sum_{n=1}^\infty\frac{\alpha_n}{n}
\dst\frac{\rho_0^{-n\pi/\gamma}r^{n\pi/\gamma}-\rho_0^{n\pi/\gamma}
r^{-n\pi/\gamma}}
{\rho_0^{-n\pi/\gamma}\rho^{n\pi/\gamma}+\rho_0^{n\pi/\gamma}
\rho^{-n\pi/\gamma}}
\dst\cos\frac
{n\pi\theta}{\gamma}$


\quad \quad $\alpha_0=\dst\frac{1}{\gamma}\int_0^\gamma
f(\theta)\,d\theta$,\quad
$\alpha_n=\dst\frac{2}{\gamma}\int_0^\gamma
f(\theta)\cos\frac{n\pi\theta}{\gamma}\,d\theta$, $n=1$, $2$,
$3$,\dots

\answer{12.4.4}
$u(r,\theta)=\dst\sum_{n=1}^\infty\alpha_n
\dst\frac{r^{(2n-1)\pi/2\gamma}}{\rho^{(2n-1)\pi/2\gamma}}
\cos\frac
{(2n-1)\pi\theta}{2\gamma}$  \quad

\quad \quad
$\alpha_n=\dst\frac{2}{\gamma}\int_0^\gamma
f(\theta)\cos\frac{(2n-1)\pi\theta}{2\gamma}\,d\theta$,\;
  $n=1$, $2$, $3$,\dots


\answer{12.4.5}
$u(r,\theta)=\dst\frac{2\gamma\rho_0}{\pi}\sum_{n=1}^\infty
\frac{\alpha_n}{2n-1}
\frac{\rho^{-(2n-1)\pi/2\gamma}r^{(2n-1)\pi/2\gamma}+
\rho^{(2n-1)\pi/2\gamma}r^{-(2n-1)\pi/2\gamma}}
{\rho^{-(2n-1)\pi/2\gamma}\rho_0^{(2n-1)\pi/2\gamma}-
\rho^{(2n-1)\pi/2\gamma}\rho_0^{-(2n-1)\pi/2\gamma}}
\dst\sin\frac
{(2n-1)\pi\theta}{2\gamma}$,

\quad \quad
$\alpha_n=\dst\frac{2}{\gamma}\int_0^\gamma
g(\theta)\sin\frac{(2n-1)\pi\theta}{2\gamma}\,d\theta$,\quad
 $n=1$, $2$, $3$,\dots

\answer{12.4.6}
$\dst{u(r,\theta)=\alpha_0+\sum_{n=1}^\infty\alpha_n
\dst\frac{r^{n\pi/\gamma}}{\rho^{n\pi/\gamma}}\cos\frac{n\pi\theta}{\gamma}}$
\quad
$\alpha_0=\dst\frac{1}{\gamma}\int_0^\gamma f(\theta)\,d\theta$,\quad

\quad \quad
$\alpha_n=\dst\frac{2}{\gamma}\int_0^\gamma
f(\theta)\cos\frac{n\pi\theta}{\gamma}\,d\theta$,\;
 $n=1$, $2$, $3$,\dots

\answer{12.4.7}
$\dst{
v_n(r,\theta)=\frac{r^n}{n\rho^{n-1}}(\alpha_n\cos
n\theta+\sin n\theta)}$

\quad \quad $u(r,\theta)=\dst{
c+\sum_{n=1}^\infty\frac{r^n}{n\rho^{n-1}}(\alpha_n\cos
n\theta+\beta_n\sin n\theta)}$
$\alpha_n=\dst{\frac{1}{\pi}\int_{-\pi}^\pi f(\theta)\cos
n\theta\,d\theta}$,\quad

\quad \quad
$\dst{\beta_n=\frac{1}{\pi}\int_{-\pi}^\pi f(\theta)\sin
n\theta\,d\theta}$, $n=1$, $2$, $3$,\dots


\sectionanswers{13.1}

\answer{13.1.2} $y=-x+\dst{\frac{2}{e-1}\left(e^{x}-e^{(x-1)}\right)}$\;
\answer{13.1.3}
 $y=x^{2}-\dst{\frac{x^{3}}{3}}+cx$ with $c$ arbitrary

\answer{13.1.4}$y=-x+2e^{x}+e^{-(x-1)}$\;
\answer{13.1.5} $y= \dst{\frac{1}{4}+\frac{11}{4}\cos2x+\frac{9}{4}\sin2x}$

\smallskip
\answer{13.1.6}  $y=(x^2+13-8x)e^{x}$\;
\answer{13.1.7}  $y=2e^{2x}+\dst{\frac{3(5e^{3x}-4e^{4x})}{e(15-16e)}
+\frac{2e^{4}(4e^{3(x-1)}-3e^{4(x-1)})}{16e-15}}$

\answer{13.1.8} $\dst{\int_{a}^{b}tF(t)\,dt=0}$\;
$y=\dst{-x\int_{x}^{1}F(t)\,dt-\int_{0}^{x}tF(t)\,dt+c_{1}x}$ with $c_{1}$
arbitrary

\smallskip
\answer{13.1.9} \part{a}\;   $b-a\ne k\pi$\;    ($k=$ integer)

\smallskip
\qquad\; \;  $y=\dst{\frac{\sin(x-a)}{\sin(b-a)}\int_{x}^{b}F(t)\sin(t-b)\,dt
+\frac{\sin(x-b)}{\sin(b-a)}\int_{a}^{x}F(t)\sin(t-a)\,dt}$
\smallskip

\quad\;  \;  \part{b}\;    $\dst{\int_{a}^{b}F(t)\sin(t-a)\,dt=0}$

\qquad\; \;  \,
$y=-\dst{\sin(x-a)\int_{x}^{b}F(t)\cos(t-a)\,dt
-\cos(x-a)\int_{a}^{x}F(t)\sin(t-a)\,dt}$

\smallskip
\qquad\; \;  \,
$+c_{1}\sin(x-a)$ with  $c_{1}$ arbitrary

\answer{13.1.10} \part{a}\;    $b-a\ne(k+1/2)\pi$\;    ($k=$ integer)

\qquad\; \;  \,
$y=-\dst{ \frac{\sin(x-a)}{\cos(b-a)}\int_{x}^{b}F(t)\cos(t-b)\,dt-
 \frac{\cos(x-b)}{\cos(b-a)}\int_{x}^{b}F(t)\sin(t-a)\,dt}$

\quad\;  \;  \part{b}\;    $\dst{\int_{a}^{b}F(t)\sin(t-a)\,dt=0}$

\qquad\; \;  \,
$y=-\dst{\sin(x-a)\int_{x}^{b}F(t)\cos(t-a)\,dt
-\cos(x-a)\int_{a}^{x}F(t)\sin(t-a)\,dt}$

\qquad\; \;  \,
$+c_{1}\sin(x-a)$ with  $c_{1}$ arbitrary

\smallskip

\answer{13.1.11} \part{a}\;    $b-a\ne k\pi$ ($k=$ integer)

\qquad\; \;  \,
$y=\dst{\frac{\cos(x-a)}{\sin(b-a)}\int_{x}^{b}F(t)\cos(t-b)\,dt+
\frac{\cos(x-b)}{\sin(b-a)}\int_{a}^{x}F(t)\cos(t-a)\,dt}$
\smallskip

\qquad\; \;  \,
\part{b} $\dst{\int_{a}^{b}F(t)\cos(t-a)\,dt=0}$

\smallskip

\qquad\; \;  \,
$y=\dst{\cos(x-a)\int_{x}^{b} F(t)\sin(t-a)\,dt +\sin(x-a)\int_{a}^{x}
F(t)\cos(t-a)\,dt}$
\smallskip

\qquad\; \;  \,
\quad  $+c_{1}\cos(x-a)$  with $c_{1}$ arbitrary

\smallskip

\answer{13.1.12}
$y=\dst{\frac{\sinh(x-a)}{\sinh(b-a)}\int_{x}^{b}F(t)\sinh(t-b)\,dt
+\frac{\sinh(x-b)}{\sinh(b-a)}\int_{a}^{x}F(t)\sinh(t-a)\,dt}$

\smallskip

\answer{13.1.13}
$y=\dst{-\frac{\sinh(x-a)}{\cosh(b-a)}\int_{x}^{b}F(t)\cosh(t-b)\,dt
-\frac{\cosh(x-b)}{\cosh(b-a)}\int_{a}^{x}F(t)\sinh(t-a)\,dt}$

\smallskip

\answer{13.1.14}
$y=-\dst{\frac{\cosh(x-a)}{\sinh(b-a)}\int_{x}^{b}F(t)\cosh(t-b)\,dt
-\frac{\cosh(x-b)}{\sinh(b-a)}\int_{a}^{x}F(t)\cosh(t-a)\,dt}$
\smallskip

\answer{13.1.15}
$y=-\dst{\frac{1}{2}\left(
e^{x}\int_{x}^{b}e^{-t}F(t)\,dt+e^{-x}\int_{a}^{x}e^{t}F(t)\,dt\right)}$

\smallskip

\rm
\answer{13.1.16} If $\omega$ isn't  a positive integer, then

\smallskip
\quad\;  \;  \,
$y=\dst{\frac{1}{\omega\sin\omega\pi}
\left(\sin\omega x\int_{x}^{\pi}F(t)\sin\omega(t-\pi)\,dt
+\sin\omega(x-\pi)\int_{0}^{x}F(t)\sin\omega t\,dt\right)}$.

\smallskip \quad\;   \,
If $\omega=n$ (positive integer), then $\dst{\int_{0}^{\pi}F(t)\sin
nt\,dt=0}$ is necessary for existence

\quad\;   \,
of a solution. In this case,

\smallskip

\quad\;   \,
$y=-\dst{\frac{1}{n}\left(\sin nx\int_{x}^{\pi}F(t)\cos nt\,dt
+\cos nx\int_{0}^{x} F(t)\sin nt\,dt\right) +c_{1}\sin nx}$

\smallskip \quad\;   \,
 with $c_{1}$ arbitrary.

\smallskip
\answer{13.1.17} If $\omega\ne n+1/2$\;   ($n=$ integer), then
$$
y=-\frac{\sin\omega x}{\omega\cos\omega
\pi}\int_{x}^{\pi}F(t)\cos\omega(t-\pi)\,dt
-\frac{\cos\omega(x-\pi)}{\omega\cos\omega\pi}\int_{0}^{x}F(t)\sin\omega
t\,dt.
$$

\smallskip \quad\;   \,
If $\omega= n+1/2$ ($n=$ integer), then
$\dst{\int_{0}^{\pi}F(t)\sin(n+1/2)t\,dt=0}$
is necessary

 \quad\;   \,
 for existence of a solution. In this case,

\smallskip \quad\;   \,
$y=-\dst{\frac{\sin(n+1/2)x}{n+1/2}\int_{x}^{\pi}F(t)\cos(n+1/2)t\,dt}$

\smallskip \quad\;   \,   \qquad \qquad
$-\dst{\frac{\cos(n+1/2)x}{n+/2}\int_{0}^{x}F(t)\sin(n+1/2)t\,dt
+c_{1}\sin(n+1/2)x}$

\quad\;   \,
with $c_{1}$ arbitrary,

\smallskip

\answer{13.1.18} If $\omega\ne n+1/2$\;   ($n=$ integer), then
$$
y=\frac{\cos\omega x}{\omega\cos\omega
\pi}\int_{x}^{\pi}F(t)\sin\omega(t-\pi)\,dt
+\frac{\sin\omega(x-\pi)}{\omega\cos\omega\pi}\int_{0}^{x}F(t)\cos\omega
t\,dt.
$$

\smallskip \quad\;   \,
If $\omega= n+1/2$ ($n=$ integer), then
$\dst{\int_{0}^{\pi}F(t)\cos(n+1/2)t\,dt=0}$
is necessary

 \quad\;   \,
 for existence of a solution. In this case,

\smallskip \quad\;   \,
$y=\dst{\frac{\cos(n+1/2)x}{n+1/2}\int_{x}^{\pi}F(t)\sin(n+1/2)t\,dt}$

\smallskip \quad\;   \,   \qquad \qquad
$+\dst{\frac{\sin(n+1/2)x}{n+/2}\int_{0}^{x}F(t)\cos(n+1/2)t\,dt
+c_{1}\cos(n+1/2)x}$

\quad\;   \,
with $c_{1}$ arbitrary.

\smallskip

\answer{13.1.19} If $\omega$ isn't  a positive integer, then

\smallskip
\quad \quad
$y=\dst{\frac{1}{\omega\sin\omega\pi}
\left(\cos\omega x\int_{x}^{\pi}F(t)\cos\omega(t-\pi)\,dt
+\cos\omega(x-\pi)\int_{0}^{x}F(t)\cos\omega t\,dt\right)}$.

\smallskip \quad\;   \,
If $\omega=n$ (positive integer), then $\dst{\int_{0}^{\pi}F(t)\cos
nt\,dt=0}$ is necessary for existence

\quad\;   \,
of a solution. In this case,

\smallskip

\quad\;   \,
$y=-\dst{\frac{1}{n}\left(\cos nx\int_{x}^{\pi}F(t)\sin nt\,dt
+\sin nx\int_{0}^{x} F(t)\cos nt\,dt\right) +c_{1}\cos nx}$

\smallskip \quad\;   \,
 with $c_{1}$ arbitrary.

\medskip

\answer{13.1.20}  $y_{1}=B_{1}(z_{2})z_{1}-B_{1}(z_{1})z_{2}$

\smallskip

\answer{13.1.21} \part{a}
$G(x,t)=
\begin{cases}
\dst{\frac{(t-a)(x-b)}{b-a}} &a\le t\le x,\\[.11in]
\dst{\frac{(x-a)(t-b)}{b-a)}}&x\le t\le b
\end{cases}$

\quad  \quad\;
$y=\dst{\frac{1}{b-a}\left((x-a)\int_{x}^{b}(t-b)F(t)\,dt+(x-b)\int_{a}^{x}(t-a)F(t)\,dt\right)}$

\smallskip
\quad\;
\part{b}\;
$G(x,t)=
\begin{cases}
a-t & a\le t\le x\\
a-x &x\le t\le b
\end{cases}$  \quad
$y=\dst{(a-x)\int_{x}^{b}F(t)\,dt+\int_{a}^{x}(a-t)F(t)\,dt}$

\smallskip
\quad\;
\part{c}\;
$G(x,t)=
\begin{cases}
x-b & a\le t\le x\\
t-b &x\le t\le b
\end{cases}$  \quad
$y=\dst{\int_{x}^{b}(t-b)F(t)\,dt+(x-b)\int_{a}^{x}F(t)\,dt}$

\smallskip
\quad\;
\part{d}\;
$\dst{\int_{a}^{b}F(t)\,dt=0}$ is a necessary condition for existence of a
solution. Then

\quad  \qquad  \qquad
      $y=\dst{\int_{x}^{b}tF(t)\,dt+x\int_{a}^{x}F(t)\,dt+c_{1}}$
with $c_{1}$ arbitrary.

\smallskip
\answer{13.1.22}
$G(x,t)=
\begin{cases}
\dst{-\frac{(2+t)(3-x)}{5}},& 0\le t\le x,\\[.11in]
-\dst{\frac{(2+x)(3-t)}{5}}, &x\le t\le 1
\end{cases}$
\;  \part{a} $y=\dst{\frac{x^{2}-x-2}{2}}$
\;  \part{b} $y=\dst{\frac{5x^{2}-7x-14}{30}}$

\medskip
\quad \quad
\part{c}$\, y=\dst{\frac{5x^{4}-9x-18}{60}}$

\smallskip
\answer{13.1.23}
$G(x,t)=
\begin{cases}
\dst{\frac{\cos t\sin x}{t^{3/2}\sqrt{x}}}, &\dst{\frac{\pi}{2}}\le t\le
x,\\[.11in]
\dst{\frac{\cos x\sin t}{t^{3/2}\sqrt{x}}}, &x\le t\le \pi
\end{cases}$

\smallskip
\quad \quad
\;  \part{a}\;   $y=\dst{\frac{1+\cos x-\sin x}{\sqrt x}}$
\;  \part{b}\;   $y=\dst{\frac{x+\pi \cos x-\pi/2\sin x}{\sqrt x}}$

\smallskip
\answer{13.1.24}
$G(x,t)=
\begin{cases}
\dst{\frac{(t-1)x(x-2)}{t^{3}}},\;  1\le t\le x,\\[.11in]
\dst{\frac{x(x-1)(t-2)}{t^{3}}},\;   x\le t\le 2
\end{cases}$

\smallskip
\quad \quad
\part{a} $y=x(x-1)(x-2)$\;
\part{b} $y=x(x-1)(x-2)(x+3)$

\smallskip
\answer{13.1.25}
$G(x,t)=
\begin{cases}
-\dst{\frac{1}{22}\left(3+\frac{1}{t^{2}}\right)\left(x+\frac{4}{x}\right)},&\;  1\le x\le t,\\[.11in]
-\dst{\frac{1}{22}\left(3x+\frac{1}{x}\right)\left(1+\frac{4}{t^{2}}\right)},&\;   x\le t\le 2
\end{cases}$

\smallskip

\quad \quad
\part{a}
$y=\dst{\frac{x^{2}-11x+4}{11x}}$\;
\part{b}
$y=\dst{\frac{11x^{3}-45x^{2}-4}{33x}}$\;
\part{c}
$y=\dst{\frac{11x^{4}-139x^{2}-28}{88x}}$

\smallskip
\answer{13.1.26} $\alpha(\rho+\delta)-\beta\rho\ne0$   \quad
$G(x,t)=
\begin{cases}
\dst{\frac{(\beta-\alpha t)(\rho+\delta-\rho x)}
{\alpha(\rho+\delta)-\beta\rho}}, \quad  0\le t\le x, \\[.11in]
\dst{\frac{(\beta-\alpha x)(\rho+\delta-\rho t)}
{\alpha(\rho+\delta)-\beta\rho}},\quad  x\le t\le 1
\end{cases}$

\smallskip
\answer{13.1.27}
$\alpha\delta-\beta\rho\ne0$  \quad
$G(x,t)=
\begin{cases}
\dst{\frac{(\beta\cos t-\alpha\sin t)(\delta \cos x-\rho\sin
x)}
{\alpha\delta-\beta\rho}},\;    &0\le t\le x,\\[.11in]
\dst{\frac{(\beta\cos x-\alpha\sin x)(\delta \cos t-\rho\sin t)}
{\alpha\delta-\beta\rho}},\;   &x\le t\le \pi
\end{cases}$

\smallskip
\answer{13.1.28} $\alpha\rho+\beta\delta\ne0$ \;
$G(x,t)=
\begin{cases}
\dst{\frac{(\beta\cos t-\alpha\sin t)(\rho\cos x+\delta\sin x)}
{\alpha\rho+\beta\delta}}\;   & x\le t\le \pi  \\[.11in]
\dst{\frac{(\beta\cos x-\alpha\sin x)(\rho\cos t+\delta\sin t)}
{\alpha\rho+\beta\delta}}\;
&0\le t\le x
\end{cases}$

\smallskip
\answer{13.1.29} $\alpha\delta-\beta\rho\ne0$ \;
$G(x,t)=
\begin{cases}
\dst{\frac{e^{x-t)}(\beta\cos t-(\alpha+\beta)\sin t)
(\delta\cos x-(\rho+\delta)\sin x)}{\alpha\delta-\beta\rho}}\;
&0\le t\le x, \\[.11in]
\dst{\frac{e^{x-t}(\beta\cos x-(\alpha+\beta)\sin x)
(\delta\cos t-(\rho+\delta)\sin t)}{\alpha\delta-\beta\rho}},\;
&x\le t\le \pi
\end{cases}$

\smallskip
\answer{13.1.30} $\beta \delta+(\alpha+\beta)(\rho+\delta)\ne0$ \;
$G(x,t)=
\begin{cases}
\dst{\frac{e^{x-t}(\beta\cos t-(\alpha+\beta)\sin t)
((\rho+\delta)\cos x+\delta \sin x)}
{\beta \delta+(\alpha+\beta)(\rho+\delta}},& 0\le t\le  x, \\[.11in]
\dst{\frac{e^{x-t}(\beta\cos x-(\alpha+\beta)\sin x)
((\rho+\delta)\cos t+\delta \sin t)}
{\beta \delta+(\alpha+\beta)(\rho+\delta}},  &x\le t\le \pi/2
\end{cases}$

\smallskip
\answer{13.1.31}
$(\rho+\delta)(\alpha-\beta)e^{(b-a)}-(\rho-\delta)(\alpha+\beta)e^{(a-b)}\ne0$

\smallskip
\quad \quad
$G(x,t)=
\begin{cases}
\dst{\frac{((\alpha-\beta)e^{(t-a)}-(\alpha+\beta)e^{-(t-a)})
((\rho-\delta)e^{(x-b)}-(\rho+\delta)e^{-(x-b)})}
{2[(\rho+\delta)(\alpha-\beta)e^{(b-a)}-(\rho-\delta)(\alpha+\beta)e^{(a-b)}]}},\;
&0\le t\le x,\\[.11in]
\dst{\frac{((\alpha-\beta)e^{(x-a)}-(\alpha+\beta)e^{-(x-a)})
((\rho-\delta)e^{(t-b)}-(\rho+\delta)e^{-(t-b)})}
{2[(\rho+\delta)(\alpha-\beta)e^{(b-a)}-(\rho-\delta)(\alpha+\beta)e^{(a-b)}]}}\;
&x\le t\le \pi
\end{cases}$


\sectionanswers{13.2}


\answer{13.2.1} $(e^{bx}y')'+ce^{bx}y=0$\;
\answer{13.2.2} $\dst{(xy')'+\left(x-\frac{\nu^{2}}{x}\right)y=0}$\;
\answer{13.2.3} $\dst{(\sqrt{1-x^{2}}y')'+\frac{\alpha^{2}}{\sqrt{1-x^{2}}}y=0}$

\smallskip

\answer{13.2.4} $(x^{b}y')'+cx^{b-2}y=0$\;
\answer{13.2.5} $(e^{-x^{2}}y')'+2\alpha e^{-x^{2}}y =0$\;
\answer{13.2.6} $(xe^{-x}y')'+\alpha e^{-x}y=0$

\smallskip
\answer{13.2.7} $((1-x^{2})y')'+\alpha(\alpha+1)y=0$\;

\smallskip

\answer{13.2.9}  $\lambda_{n}=n^{2}\pi^{2}$,\;    $y_{n}=e^{-x}\sin n\pi x$\, ($n=$
positive integer)

\smallskip

\answer{13.2.10} $\lambda_{0}=-1$, $y_{0}=1$   $\lambda_{n}=n^{2}\pi^{2}$,
$y_{n}=e^{-x}(n\pi\cos n\pi x+\sin n\pi x)$  ($n=$ positive

\quad \quad  integer)

\smallskip

\answer{13.2.11} \part{a} $\lambda=0$ is an eigenvalue   $y_{0}=2-x$\;
\part{b} none\;    \part{c} $5.0476821$, $14.9198790$,

\quad \quad
$29.7249673$,
$49.4644528$   $y=2\sqrt{\lambda}\cos\sqrt{\lambda}\,x-\sin\sqrt{\lambda} x$

\smallskip

\answer{13.2.12} \part{a} $\lambda=0$ isn't  an eigenvalue\;   \part{b}
 $-0.5955245$   $y=\cosh \sqrt{-\lambda}\,x$\;
\part{c} $8.8511386$,

\quad \quad
$38.4741053$,  $87.8245457$, $156.9126094$
$y=\cos\sqrt\lambda x$

\smallskip

\answer{13.2.13} \part{a} $\lambda=0$ isn't  an eigenvalue\;    \part{b} none\;
\part{c} $0.
1470328$, $1.4852833$, $4.5761411$,

\quad \quad
$9.6059439  $\;
$y=\sqrt\lambda\cos\sqrt\lambda x+\sin\sqrt\lambda x$

\smallskip

\answer{13.2.14} \part{a} $\lambda=0$ isn't  an eigenvalue\;   \part{b} $-0.1945921$

\quad \quad
$y=2\sqrt{-\lambda}\,\cosh \sqrt{-\lambda}\,x\;
-\sinh\sqrt{-\lambda}\,x$
\part{c} $1.9323619$, $5.9318981$, $11.9317920$,

\quad \quad  $19.9317507$
$y=2\sqrt{\lambda}\,\cos \sqrt{\lambda}\,x-\sin\sqrt{\lambda}\,x$

\smallskip

\answer{13.2.15} \part{a} $\lambda=0$ isn't  an eigenvalue\;
 \part{b} $-1.0664054$   $y=\cosh\sqrt{-\lambda}\,x$\;
\part{c} $1.5113188$,

\quad \quad
$8.8785880$, $21.2104662$, $38.4805610$
$y=\cos\sqrt{\lambda}\,x$
\smallskip

\answer{13.2.16} \part{a} $\lambda=0$ isn't  an eigenvalue\;
\part{b}  $-1.0239346$\;

\quad \quad
$y=\sqrt{-\lambda}\,\cosh \sqrt{-\lambda}\,x\;
-\sinh\sqrt{-\lambda}\,x$
\part{c} $2.0565705$, $9.3927144$, $21.7169130$,

\quad \quad
$38.9842177$  \,
$y=\sqrt{\lambda}\,\cos \sqrt{\lambda}\,x-\sin\sqrt{\lambda}\,x$

\smallskip

\answer{13.2.17} \part{a} $\lambda=0$ isn't  an eigenvalue\;
 \part{b} $-0.4357577$,

\quad \quad
$y=2\sqrt{-\lambda}\,\cosh \sqrt{-\lambda}\,x\;
-\sinh\sqrt{-\lambda}\,x$
\part{c} $0.3171423$, $3.7055350$, $9.1970150$,

\quad \quad
 $16.8760401$\;
$y=2\sqrt{\lambda}\,\cos \sqrt{\lambda}\,x-\sin\sqrt{\lambda}\,x$
\smallskip

\answer{13.2.18} \part{a} $\lambda=0$ isn't  an eigenvalue\;
\part{b} $-2.1790546$, $-9.0006633$

\quad \quad
$y=\sqrt{-\lambda}\,\cosh\sqrt{-\lambda}\,x-3\sinh\sqrt{-\lambda}\,x$

\quad \quad
\part{c} $5.8453181$, $17.9260967$, $35.1038567$, $57.2659330$
$y=\sqrt{\lambda}\,\cos\sqrt{\lambda}\,x-3\sin\sqrt{\lambda}\,x$

\smallskip

\answer{13.2.19} \part{a} $\lambda=0$ is  an eigenvalue\;   $y_{0}=2-x$\;
 \part{b} $-1.0273046  $

\quad \quad
$y=2\sqrt{-\lambda}\,\cosh\sqrt{-\lambda}\,x-\sinh\sqrt{-\lambda}\,x$
\part{c} $8.8694608$, $16.5459202$, $26.4155505$,

\quad \quad
$38.4784094$
$y=2\sqrt{\lambda}\,\cos\sqrt{\lambda}\,x-\sin\sqrt{\lambda}\,x$

\smallskip

\answer{13.2.20} \part{a} $\lambda=0$ isn't  an eigenvalue\;   \part{b} $-7.9394171$,
$-3.1542806$

\quad \quad
$y=2\sqrt{-\lambda}\,\cosh\sqrt{-\lambda}\,x-5\sinh\sqrt{-\lambda}\,x$
\part{c} $29.3617465$, $78.777456$, $147.8866417$,

\quad \quad
$236.7229622$
$y=2\sqrt{\lambda}\,\cos\sqrt{\lambda}\,x-5\sin\sqrt{\lambda}\,x$

\smallskip

\answer{13.2.21} $\lambda=0$, $y=xe^{-x}$   $20.1907286$, $118.8998692$,
$296.5544121$,
$553.1646458$

\quad \quad
$y=e^{-x}\sin\sqrt{\lambda}\,x$

\answer{13.2.22}  $\lambda_{n}=n^{2}\pi^{2}$, $y_{n}=x\sin n\pi(x-2)$\;    ($n=$
positive integer)

\smallskip

\answer{13.2.23} $\lambda=0$, $y=x(2-x)$   $20.1907286$, $118.8998692$, $296.5544121$

\quad \quad
$553.1646458$, $y=x\sin\sqrt{\lambda}\,(x-2)$

\smallskip

\answer{13.2.24}  $3.3730893$, $23.1923372$, $62.6797232$, $121.8999231$,
$200.8578309$

\quad \quad
$y=x\sin\sqrt{\lambda}\,(x-1)$

\smallskip
\answer{13.2.25} \part{a} $-L<\delta<0$\;   \part{b} $\delta=-L$

\answer{13.2.26} $\lambda_{0}=-1/\alpha^{2}$ $y_{0}=e^{-x/\alpha}$  \,
 $\lambda_{n}=n^{2}$,
$y_{n}=n\alpha \cos nx-\sin nx$,\;   $n=1$, $2$, \dots

\smallskip
\answer{13.2.27}  \part{a} $y=x-\alpha$ \, \part{b} $y=\alpha k\cosh kx-\sin kx$
\part{c}  $y=\alpha k\cos kx-\sin kx$

\smallskip
\answer{13.2.29} \part{b} $\lambda=-\alpha^{2}/\beta^{2}$   $y=e^{-\alpha x/\beta}$

