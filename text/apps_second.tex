...
in
radians. \zcref[S]{eq:6.1.10} is the \emph{\color{blue}amplitude-phase
...
\emph{\color{blue}time-varying amplitude} of the motion, the quantity $\omega_1$ is
...
\emph{\color{blue}quasi-period}. A typical graph of \eqref{eq:6.2.3} is shown in
...
\emph{\color{blue}transient component} of the solution $y$.
...
the \emph{\color{blue}resistance} of the resistor.
...
According to
\personHref{Kirchhoff}{Kirchoff's law}, the sum of
...
Newton's second law of motion ($\vect{F}=m\vect{a}$) says that the polar
coordinates
...
is known as
\personHref{Kepler}{Kepler's Second Law}.
...
the origin (\zcref{exer:6.4.1}). The nonnegative constant $e$ is
...
values of $r$  are
\begin{align*}
r_{\min}&=\frac{\rho}{1+e}\quad\text{(the \emph{\color{blue}perihelion
...
The point $P$ on the orbit where $r=r_{\min}$ is the \emph{\color{blue}perigee} 
and the point $A$ where $r=r_{\max}$ is the \emph{\color{blue}apogee}.
...
\]
(This is
\personHref{Kepler}{Kepler's third law};   $T$ is called the
